...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Parallel-cylinder regularized CSG retains certified overlap, contact, common-support, and manifold internal-tangency families with Full-check, X_T, rollback, properties, and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses and four finite axial-bound topologies. Graph SSI merges those bounds into Outside, Whole, or non-wrapping Open sheet states; every Open edge retains source-ordered nonlinear pcurves, independent subrange residual proof, and exact caller-bound/root evidence. Public Section now atomically maps those roots to topology-owned cap rings, keeps physical roots separate from guarded residual endpoints, and stitches four bounded procedural spans with four cap rulings into two closed components across world/oblique frames, replay, and swap; exact N/N-1 work and malformed-final-endpoint refusal publish no prefix. Cap-disk arrangements consume the topology-owned ruling chords, and the periodic Boolean adapter recognizes procedural endpoints only behind certified embedding evidence and cross-checks trim identity. Kgraph reissues all 256 guarded nonlinear pcurve cells plus two strict physical-root corridors with stored/exact UV and derivatives; kops now seals those corridors beside each caller-ordered Open residual at exact 260 work. Contact/corner roots, seam-spanning walks, unsafe cases, persistence, Section embedding, Boolean composition, and containing-minus-contained pinches remain typed gaps. Next: preserve the root-bound certificate through Section orientation, certify both periodic side embeddings, then compose regularized skew Cylinder/Cylinder CSG.)*
