...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Parallel-cylinder CSG now certifies strict overlaps, coincident caps, strict radial/axial separation, exact zero-gap cap contact, and exact external radial tangency through positive axial overlap. At zero axial gap, strict radial secancy retains two proof-joined bounded cap arcs with dual source-ring root identity while local coincidence gaps keep the global Section graph honestly Indeterminate. Contact Intersect is regularized ProvenEmpty and ordered Subtract Full-commits a whole minuend copy. Contact Unite Full-commits strict internal disk containment as one 5F/4E/0V annulus-connected shell, exactly coincident disks as one 3F/2E/0V coalesced band, strict secancy as one 6F/6E/2V split-ring shell, and exact external tangency as two atomically copied 3F/2E/0V bodies. Positive-overlap external tangency uses the same empty-Intersect, minuend-copy Subtract, and two-copy Unite contract for nested, partial, and shared-end axial intervals. The slices retain independent analytic properties, Full reports, complete DerivedFrom/Merge/Split lineage, deterministic X_T/Fast self-import, rollback, and exact N/N-1 work across world/translated-Pythagorean direction/order/repeat matrices. Division-free exact support proof requires both stored axes and an error-free representable radius sum; one-ULP inward, internal-tangent, unsafe, unrepresentable-sum, and rounded all-nonzero oblique cases remain boundary refusals. Analytic-shell batches now preflight and emit ordered Face Split lineage across result bodies plus binary endpoint-free circle Edge Merge lineage, with allocation-free refusal, Full-valid joint commit, and rollback replay. Next: certify exact common-cylinder-support interval CSG with one generic four-endpoint axial cell sweep using that lineage contract; internal tangency and skew SSI remain typed gaps.)*
