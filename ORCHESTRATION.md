...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Parallel-cylinder CSG now certifies strict overlaps, coincident caps, strict radial/axial separation, exact zero-gap cap contact, exact external radial tangency, and exact common-cylinder support through positive axial overlap. Contact Intersect/Subtract/Unite retain regularized empty, source-copy, or connected-shell contracts across their certified radial classes. Common support uses one certified four-endpoint total preorder and open-cell sweep for crossing, nesting, shared-end, and equal intervals; it atomically Full-commits zero, one, or two canonical 3F/2E/0V bands with complete DerivedFrom/Merge/Face-Split lineage. The slices retain analytic properties, Full reports, deterministic X_T/Fast self-import, rollback/replay, and exact 64/63 relation, 420/419 one-band, 840/839 two-band, and 3953/3952 property work across world/translated-Pythagorean direction/order/repeat matrices. Exact endpoint equality alone emits cap/ring Merge, while a one-ULP endpoint neighbor stays distinct; one-ULP radius drift, internal tangency, unsafe arithmetic, unrepresentable tangent sums, and rounded all-nonzero oblique cases remain boundary refusals. Next: exact internal radial tangency over certified axial intervals; skew SSI remains a typed gap.)*
