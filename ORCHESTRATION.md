...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Parallel-cylinder CSG now certifies strict overlaps, coincident caps, strict radial/axial separation, and exact zero-gap cap contact. At zero axial gap, strict radial secancy retains two proof-joined bounded cap arcs with dual source-ring root identity while local coincidence gaps keep the global Section graph honestly Indeterminate. Contact Intersect is regularized ProvenEmpty and ordered Subtract Full-commits a whole minuend copy. Contact Unite Full-commits strict internal disk containment as one 5F/4E/0V annulus-connected shell, exactly coincident disks as one 3F/2E/0V coalesced band, strict secancy as one 6F/6E/2V split-ring shell, and exact external tangency as two atomically copied 3F/2E/0V bodies. The slices retain independent analytic properties, Full reports, complete DerivedFrom/Merge/Split lineage, deterministic X_T/Fast self-import, rollback, and exact N/N-1 work. External tangency covers equal/unequal radii across world/Pythagorean-tilted direction/order/repeat matrices; one-ULP and rounded all-nonzero oblique near cases remain boundary refusals. Next: certify exact external radial tangency with positive axial overlap for the same source-copy set semantics; internal tangency remains a typed gap.)*
