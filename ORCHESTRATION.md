...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Parallel-cylinder regularized CSG retains certified overlap, contact, common-support, and manifold internal-tangency families with Full-check, X_T, rollback, properties, and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses and four finite axial-bound topologies. Graph SSI merges those bounds into Outside, Whole, or non-wrapping Open sheet states; every Open edge retains source-ordered nonlinear pcurves, independent residual proof, and caller-bound/root evidence. Public Section maps physical roots to cap rings separately from residual guards and stitches four procedural spans with four cap rulings into two closed components. Kgraph reissues all 256 guarded pcurve cells plus two physical-root corridors; kops root-binds them at exact 260 work; Section preserves them through operand/reversal orientation. The periodic theorem now certifies both cylinder-side embeddings from every stored/source cell and corridor, proving global injectivity, transverse joins, orientation, zero winding, and deterministic Boolean-adapter arrangements across world/oblique frames, replay, and swap at exact 67,811 Section work. Malformed evidence publishes no prefix. Contact/corner roots, seam-spanning walks, unsafe cases, persistence, Boolean composition, and containing-minus-contained pinches remain typed gaps. Next: compose the certified side and cap arrangements into regularized skew Cylinder/Cylinder CSG.)*
