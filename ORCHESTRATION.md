...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Parallel-cylinder regularized CSG retains certified overlap, contact, common-support, and manifold internal-tangency families with Full-check, X_T, rollback, properties, and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses and four finite axial-bound topologies. Graph SSI merges those bounds into Outside, Whole, or non-wrapping Open sheet states; every Open edge retains source-ordered nonlinear pcurves, residual proof, and caller-bound/root evidence. Public Section keeps physical cap roots separate from guards and stitches four procedural spans with four cap rulings into two components. Kgraph reissues 256 guarded pcurve cells plus two root corridors; kops root-binds them at exact 260 work; Section preserves them through reversal. The periodic theorem certifies both side embeddings, global injectivity, transverse joins, orientation, and zero winding across world/oblique frames, replay, and swap at exact 67,811 work. A read-only Boolean bridge now conserves all eight Section fragments once per operand across two periodic, two disk-cap, and two whole-cap arrangements, classifies every open cell, and has an exact 56/55 frontier. It deliberately stops before planning: physical-root carrier trims, persistent composite skew pcurves, shell realization, contact/corner roots, seam walks, unsafe cases, and containing-minus-contained pinches remain typed gaps. Next: certify physical-root trim authority and a planning-only complete-fragment validator, then add persistent skew realization.)*
