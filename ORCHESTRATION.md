...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Exact-parallel/antiparallel strict-secant Cylinder/Cylinder graph ruling proof is landed; public Section closes each admitted same-signed strict positive axial overlap with two uniquely owned physical ends as one topology-owned component. Public partial-overlap Intersect Full-commits at 4F/6E/4V, and both ordered Subtract meanings Full-commit at 5F/7E/4V through an alternating periodic graph theorem and structural cap-reaching notch proof. The world/oblique, order, and repeat matrix has independent volume/area/centroid/inertia, deterministic X_T/Fast self-import, retained sources, whole-part rollback, and exact realization/property/shell N/N-1 refusal. Nested Intersect, connected Unite, and nested Subtract remain Full-green. Tangent/miss/coincident/skew remain typed graph gaps; equal-height/shared-end cap overlap and antiparallel public realization remain typed gaps. Next: partial-overlap Unite at 6F/8E/4V through a structural two-host axial-chain shell proof.)*
