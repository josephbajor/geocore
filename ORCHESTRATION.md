...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Parallel-cylinder CSG now certifies strict overlaps, coincident caps, strict radial/axial separation, and exact zero-gap cap contact. At zero axial gap, strict radial secancy now retains two proof-joined bounded cap arcs with dual source-ring root identity while local coincidence gaps keep the global Section graph honestly Indeterminate. Contact Intersect is regularized ProvenEmpty; ordered Subtract Full-commits a whole minuend copy across the exact frame/direction/order/repeat matrix, with Full reports, analytic properties, lineage, deterministic X_T/Fast self-import, rollback, and exact 64/63 relation plus 26/25 copy-work refusal. Next: Full-commit zero-gap Unite through proof-owned shared-cap arrangement, interface removal, and shell closure.)*
