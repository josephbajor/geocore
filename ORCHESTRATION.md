...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Parallel-cylinder regularized CSG retains certified overlap, contact, common-support, and manifold internal-tangency families with Full-check, X_T, rollback, properties, and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses and four finite axial-bound topologies. Graph SSI now merges those bounds into Outside, Whole, or non-wrapping Open sheet states; each Open edge retains source-ordered nonlinear pcurves, an independent subrange residual proof, exact caller-bound/root endpoint evidence, replay/swap stability, seam refusal, and atomic 256/255 per-span work. Public Section still publishes only root-free Whole sheets because its fragment/component contract cannot express bounded procedural carriers. Contact/corner roots, seam-spanning walks, unsafe cases, persistence, and containing-minus-contained pinches remain typed gaps. Next: publish graph-certified Open spans through bounded procedural Section fragments/components.)*
