...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   Parallel-cylinder regularized CSG retains certified overlap, contact, common-support, and internal-tangency families. Exact nonparallel Cylinder/Cylinder Section seals four bounded procedural spans plus four cap rulings into two components, and a complete finite-window family binds every retained span to exact occupancy, ordinal, endpoint, root, and source evidence. A specialized family-driven theorem now Full-certifies each 4F/6E/4V bounded-skew lobe from exact cap-slab/radial traversal, adjacent retained members, periodic replay, and strict omitted-member separation. Public exact-axis dispatch enables bounded-cylinder-minus-transverse-cylinder Subtract: two lobes commit atomically across world/oblique frames at 9,235 post-selection work, 2,914 shell-proof work, and eight aggregate vertices; N/N-1 and every operation/order path fail closed. Evidence: `public_bounded_skew_subtract_require_valid_commits_two_four_face_lobes`, `bounded_skew_public_dispatch_is_proof_gated_across_operation_order_matrix`, and `bounded_skew_public_budgets_accept_n_and_refuse_n_minus_one_atomically`. Certified weighted properties now reuse the lobe theorem and fixed-cell enclosures to bound volume, centroid, area, and inertia under rigid frames at an exact 1,463,426 analytic-work N/N-1 frontier. Evidence: `public_bounded_skew_lobes_have_certified_rigid_invariant_properties`. Deterministic X_T now lowers each persistent edge to a curve-less tolerant edge with paired degree-1 SP-curves over the exact 256-cell plus two-corridor partition; parsed transport, rigid replay, and atomic self-import pass. Evidence: `persistent_skew_edges_emit_certificate_partitioned_degree_one_sp_curves` and `bounded_skew_lobes_have_stable_xt_fast_self_import_twice_and_rigid_replay`. Licensed-host replay is queued under R5. Proof-safe rigid copy reissues the complete family, residuals, corridors, pcurves, and endpoints; source/copy Full replay, deterministic repeated copy, lineage, and exact N/N-1 rollback pass across world/oblique frames. Exact common-root clustering now retains repeated contacts and isolated corner events, with a 12-bit/32-work query plan and persistent v2 family accounting `896 + 32·popcount(Q) + 260M`; kops charges the exact plan before failing closed on unpublished zero-dimensional topology. Evidence: `two_exact_corner_clusters_retain_two_spans_and_two_isolated_points`, `exact_corner_clusters_are_persisted_and_accounted_without_zero_length_members`, and `root_cluster_work_has_exact_n_and_atomic_n_minus_one_boundary`. Isolated-point publication and full mixed-family stitching are next; through-contact publication, seam walks, unsafe cases, and containing-minus-contained pinches remain typed gaps.)*
...
Composed subtraction supports separated through cuts and strict nested replacement of existing holes; evidence: `composed_cuts_replace_nested_holes_and_preserve_unaffected_material`. Next application slice: crossing/touching source trims and broader result reuse; keep unsupported layouts failure-atomic and queue final X_T bytes under R5.
