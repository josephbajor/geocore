...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Exact-parallel/antiparallel strict-secant Cylinder/Cylinder graph ruling proof is landed; public Section closes each admitted same-signed strict positive axial overlap with two uniquely owned physical ends as one topology-owned component. Public partial-overlap Intersect Full-commits at 4F/6E/4V, both ordered Subtract meanings at 5F/7E/4V, and Unite at 6F/8E/4V through alternating periodic graph authority plus structural cap-reaching and two-host axial-chain shell theorems. The world/oblique, order, and repeat matrix has independent volume/area/centroid/inertia, deterministic X_T/Fast self-import, retained sources, whole-part rollback, and exact realization/property/shell N/N-1 refusal; unequal strict-secant radii and asymmetric tails validate general representation authority. Nested cases remain Full-green. Tangent/miss/coincident/skew remain typed graph gaps; equal-height/shared-end cap overlap awaits coincident planar-cap cell ownership/dedup. Next: antiparallel positive-overlap realization by normalizing both source boundary orders onto one certified common axial coordinate.)*
