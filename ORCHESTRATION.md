...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Parallel-cylinder CSG now certifies strict overlaps, coincident caps, strict radial/axial separation, and exact zero-gap cap contact. At zero axial gap, strict radial secancy retains two proof-joined bounded cap arcs with dual source-ring root identity while local coincidence gaps keep the global Section graph honestly Indeterminate. Contact Intersect is regularized ProvenEmpty and ordered Subtract Full-commits a whole minuend copy. Contact Unite now Full-commits strict internal disk containment as one 5F/4E/0V annulus-connected shell and exactly coincident disks as one 3F/2E/0V coalesced band across exact world/oblique direction/order/repeat matrices, with Full reports, independent analytic properties, complete DerivedFrom/Merge lineage, deterministic X_T/Fast self-import, rollback, exact 64/63 relation work, 1092/1091 internal or 420/419 coincident realization refusal, and 1188/1187 internal shell-proof refusal. Next: Full-commit strict-secant zero-gap Unite from the published dual-root arcs through shared-cap arrangement and split-ring closure; tangency remains boundary-only.)*
