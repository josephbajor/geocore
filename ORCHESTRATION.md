...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Parallel-cylinder regularized CSG retains certified overlap, contact, common-support, and manifold internal-tangency families with Full-check, X_T, rollback, properties, and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses and four finite axial-bound topologies. Graph SSI merges those bounds into Outside, Whole, or non-wrapping Open sheet states; every Open edge retains source-ordered nonlinear pcurves, residual proof, and caller-bound/root evidence. Public Section seals each physical cap root as endpoint identity, an exact Section-oriented carrier enclosure, and a topology-owned point strictly outside its residual guard; four procedural spans and four cap rulings form two components. Kgraph reissues 256 guarded pcurve cells plus two root corridors; kops root-binds them at exact 260 work; Section preserves them through reversal. The periodic theorem certifies both side embeddings, global injectivity, transverse joins, orientation, and zero winding across world/oblique frames, replay, and swap at exact 67,811 work. The Boolean bridge conserves all eight Section fragments across two periodic, two disk-cap, and two whole-cap arrangements and classifies every cell at exact 56/55 work. Generic truth now reaches read-only plans for all three set operations; each retains all eight Section edges and 12/14 coalesced physical edges with two opposed distinct-face uses at exact 3,086/3,088/3,728/3,730 N/N-1 post-selection and composite-precharge work. Kgraph persists each bounded Open span as a normalized nonperiodic `[0,1]` carrier with paired pcurves, source dependencies, reissued proof, sealed tolerance, and stored/exact-source directed chart-integral enclosures accumulated in the same 256-cell pass. Its O(1) exact-geometry `DisjointRange` proof uses full root enclosures and retains exact 260 work. Kgraph now also seals the complete finite-window family from exact strict-positive admission and the four axial-bound outcomes: occupancy, member ordinals, endpoint tags, and lifted root intervals derive only from that sweep; exact geometry, source permutation, residual, and corridor replay fail closed at unchanged `896 + 260M` work (1,936 for four members). Section bit-exactly binds each authored slab and projective root through reversal to its unique family member, and persistent materialization requires that membership. Analytic-shell source-matches paired Cylinder uses independently of face IDs, inserts exact graph dependencies over `[0,1]`, and journals aggregate edge-tolerance growth. A topology-owned complete circular-cap lineage witness restores oblique Plane/Cylinder ruling-family proof while preserving signed-axis, strict-secancy, and whole-range residual checks. Ktopo resolves each normalized composite to both live Cylinder/pcurve dependencies and O(1)-certifies all four whole-fin pcurve/surface incidences with fail-closed tamper, tolerance, and stale-dependency rejection. The 2F/2E/2V scaffold remains Fast-valid and Full-indeterminate, and `RequireValid` rejects with exact rollback. The kernel plan-to-component adapter consumes all eight persisted Section fragments and certifies the four physical composites once at exact 1,040 pre-certifier work. Component inputs Fast-preflight across world/oblique frames, both operand orders, and Unite/Intersect/Subtract with exact N/N-1 refusal. The real non-swapped Subtract input has four ordered disjoint Upper spans and two 4F/6E/4V lobe targets. Evidence: `exact_topology_mints_complete_deterministic_family_without_added_work`, `persistence_input_preserves_graph_root_order_across_section_reversal`, `bounded_skew_planning_is_complete_replay_stable_and_exactly_accounted`, `disjoint_ranges_certify_distinct_handles_replay_swap_and_tamper_rejection`, and `persistent_skew_whole_fin_incidence_certifies_both_sources_and_all_fins`. Public dispatch remains disabled; `RequireValid`, rigid copy, and X_T export remain gated. Contact/corner roots, seam walks, unsafe cases, and containing-minus-contained pinches remain typed gaps. Next: land the specialized 4F theorem and enable the proof-gated nonparallel public dispatch.)*
