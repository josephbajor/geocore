...
4. Extend the same ladder to block/cylinder (introduces curved SSI pcurves).
   *(active queue head — public cylinder construction, certified finite-cylinder
   ...
   same scope without treating zero lower bounds as interference. Exact-parallel/antiparallel strict-secant Cylinder/Cylinder graph ruling proof is landed; public Section closes each admitted same-signed strict positive axial overlap with two uniquely owned physical ends as one topology-owned component. Public partial-overlap Intersect now Full-commits its 4F/6E/4V lens prism across world/oblique frames, swaps, and repeats with independent volume/area/centroid/inertia, deterministic X_T/Fast self-import, retained sources, whole-part rollback, and exact N/N-1 refusal. A proper boundary-returning annulus-trace theorem and seam-lifted simple disk cut cover the mixed end ownership from exact incidence/winding without layout cases. Nested Intersect, connected Unite, and both ordered Subtract meanings remain Full-green. Tangent/miss/coincident/skew remain typed graph gaps; equal-height/shared-end cap overlap and antiparallel public realization remain typed gaps. Next: Full-check both ordered partial-overlap Subtract meanings at 5F/7E/4V through a structural cap-reaching notch proof.)*
