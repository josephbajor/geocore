...
| [`crates/kernel`](crates/kernel) | Supported native facade | Session/part lifecycle, opaque semantic IDs and views, block/cylinder/profile-extrusion construction, rigid copy, certified block/block plus axial, zero-cut whole-source/contained-cylinder cavity, one-ring union/pocket, two-port hole, and two-ring two-sided union Booleans, checking, tessellation, and typed X_T import/export without raw topology leakage. |
