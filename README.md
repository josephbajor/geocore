...
| [`crates/kernel`](crates/kernel) | Supported native facade | Session/part lifecycle, opaque semantic IDs and views, block/cylinder/profile-extrusion construction, rigid copy, certified block/block plus axial, zero-cut whole-source, one-ring connected-union/blind-pocket, and two-port through-hole block/cylinder Booleans, checking, tessellation, and typed X_T import/export without raw topology leakage. |
