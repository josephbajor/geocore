...
| [`crates/ktopo`](crates/ktopo) | L2 topology | Parasolid entity hierarchy (body→region→shell→face→loop→fin→edge→vertex) over generational arenas, independent per-fin pcurve uses with explicit parameter maps, pcurve-aware Euler variants, pcurve-authoring primitive constructors, shared edge/pcurve/surface incidence validation, and pcurve-driven whole-body watertight tessellation |
...
- M2.5 is in progress and remains the next architecture gate. The per-fin pcurve storage,
  ...
  Euler creation, and pcurve-driven body tessellation slices have landed. Operation
  caller migration, curve-less tolerant edges and conforming X_T SP-curves, face domains/
  tolerances, a procedural geometry graph, transactions/rollback, deterministic lineage
  journals, enforced topology mutation, richer errors/tolerance rules, and checker v2
