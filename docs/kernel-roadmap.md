...
Composed subtraction reuses Full-valid Plane/Cylinder results for separated through cuts through the shared arrangement/planner. Evidence: `composed_cuts_reuse_results_preserve_holes_and_replay_under_rigid_frames` and `composed_cuts_budget_boundaries_are_exact_and_failure_atomic`. Open: interacting retained trims, tilted oblique through cuts, broader composition, and licensed replay of `composed_drilled_plate.x_t`.
