...
Parallel-cylinder Section and regularized CSG retain their certified overlap/contact/common-support/internal-tangency contracts and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses plus all four axial-bound topologies, then atomically classifies each sheet as Outside, Whole, or non-wrapping Open. Open graph edges carry independent subrange residual proof, source-ordered pcurves, exact caller-bound/root endpoint evidence, replay/swap stability, seam refusal, and exact 256/255 per-span work; evidence: `nonwrapping_axial_clip_publishes_one_open_upper_span_replay_and_swap_stably` and `open_span_certificate_work_has_exact_n_and_atomic_n_minus_one_boundary`. Public Section still publishes only root-free Whole sheets because its fragment/component contract cannot represent bounded procedural carriers. Contact/corner roots, seam-spanning walks, unsafe cases, persistence, and the pinched containing-minus-contained difference remain typed gaps. Next: add bounded procedural Section fragments/components and map graph endpoint proofs to source cap rings.
