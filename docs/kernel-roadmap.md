...
Certified transverse annulus traces, exact nested line-cycle planar cells, and disk cells now share finite source-arc planning/materialization; a count-independent chord-portal theorem Full-certifies cap-crossing Unite and block-minus-cylinder at 9F/18E/12V, while Intersect and cylinder-minus-block commit at 4F/6E/4V across both rigid frames/orders with literal-derived volume, deterministic X_T, and Fast self-import. Public `body_properties` returns outward volume, centroid, area, and centroidal inertia for Full-valid exact Plane/Cylinder solids. Public `body_distance` Full-validates two pure solids and encloses whole-material minimum distance: finite face-domain support projections prove the lower endpoint and a retained topology-owned pcurve/surface point pair proves the upper endpoint without claiming a minimizer. Primitive, rigid, containment, cavity, cap-crossing, refusal, determinism, and exact N/N-1 evidence are green. The sixteen-payload bundle is stale pending one licensed-host replay. Next is certified clearance/clash.
...
`analytic_work_budget_accepts_exactly_n_and_rejects_n_minus_one`, `public_distance_certifies_block_and_cylinder_clearance_under_rigid_motion_and_swap`, and `public_distance_preserves_material_semantics_for_containment_and_a_boolean_cavity`.
