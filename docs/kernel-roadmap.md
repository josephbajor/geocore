...
|---|---|---|
| M0 Foundations | IMPLEMENTED SLICE | Deterministic math, robust exact-fallback `orient2d`/`orient3d`/`incircle`, exact cyclic `polygon_orientation2d` plus its non-copying streaming companion `polygon_orientation2d_iter`, bounded exact `quadratic_discriminant`, shared power-of-two-normalized harmonic half-angle roots, interval-first exact `affine_dot3` with a six-product expansion fallback, exact `squared_distance_difference3` with an eleven-product expansion fallback, overflow-safe finite `Vec3` normalization inherited by `Line::new` and guarded scale-aware `Frame::new`/`Frame::from_z` construction, overflow-safe finite direction canonicalization for `Line2d::new` and `Circle2d::new` under their retained computed-zero UV policy, and fail-open original-source Curve2d affine ranges, intervals, tolerances, arenas with copy-on-write undo frames, deterministic map primitives, exact-sign consumers in strict first-chart SSI polygon convexity, coincident Plane/Plane monotone-hull construction, oblique-extrusion direction, exact streaming `kgeom` trim cleaning and outer/hole winding, exact paired-chart `kops` polygonal-region winding, exact `ktopo` ordinary-face outer-loop selection, source-certified periodic side-loop vertical ordering, strict exact planar straight-loop checker orientation with containment-derived outer identity, exact identity/nonzero-constant gates for planar conic/Plane and circle/Sphere, provenance-preserving positive-width harmonic window partitions for contained conic/primitive pairs, bounded source-world exact pseudo-Sturm quartic topology plus nonidentity and exact coefficient-axis distance-extremum coverage and uniquely replayed Frame-authored tilted-axis bounded-clearance certification and center-witness discovery for line/Torus, certified one-period oblique spherical seam-root topology in `kgraph`, source-preserving projection errors, and source-preserving graph-certificate errors through the typed rigid-copy facade path exist; rounded trim signed area, checker sampled/magnitude areas, periodic pcurve sample means, rounded transformed-quadratic signs, rounded affine point subtraction, tolerance-sized conic amplitude, and tolerance-sized classified-root/window collapse are not authority in those migrated decisions. `insphere`, an `incircle` production decision consumer when required, generic curved-pcurve signed line integrals and broader curved or periodic containment, affine/squared-distance/harmonic fallbacks outside their reviewed exponent envelopes, full source-exact harmonic discriminant construction beyond coefficient agreement, amplitude/containment policy in the remaining higher conic/primitive families, general NURBS and higher-polynomial root/window classification beyond the bounded line/Torus slice, tilted line/Torus semantic-axis relations outside exact coefficient identity or unique stored-value Frame replay, ambiguous replay plateaus without an independent exact identity, broader source-world exact affine-axis proof, contextual replacement of fixed line/Torus isolation caps, raw extreme-scale `Vec3` and `Vec2` `norm`/`norm_sq`/dot/cross/distance/subtraction behavior, callers that derive UV spans before analytic-curve construction, other solver-local error migrations, legacy compatibility wrappers that still flatten typed sources, the broader topological-decision audit, and broader conformance debt remain. |
...
| M4 Intersections/profile ops | PROVISIONAL / GATED | Broad analytic special cases, explicit `Complete`/`Indeterminate` result evidence, exact NURBS patch subdivision/BVH, analytic implicit-surface exclusion, deterministic recursive candidate covers with structured limits and proof-bearing miss exits, source-range-certified curve-pair exclusion bounds, bounded cell-local curve-pair polishing, several exact-cell interval-certified root/overlap slices, and a bounded NURBS/Plane migration whose original source owns slab/window proofs while derived controls are numeric-only exist; its exact-source contacts and overlaps remain `Indeterminate` under static node/depth caps. Rigid complete-body copy and the first checked polygonal-profile extrusion also exist; general root discovery and boolean-ready paired-pcurve branches do not. |
...
Periodic side-face tessellation now separates its `+u` bottom and `-u` top
winding loops with source evidence whenever pcurves exist.
`Curve2d::source_affine_range(range, linear, bias)` is object-safe and fail-open:
Line2d uses outward affine interval evaluation, Circle2d uses an authored
global harmonic bound over its stored axis, and NurbsCurve2d scans only active
original Euclidean controls under the positive-weight convex-hull property.
Invalid definitions retain InvalidGeometry/StaleHandle before capability
dispatch.

Complete all-pcurve loops on a non-`v`-periodic chart with zero vertical chart
shift union those fin ranges. `face_case_b` accepts only
`top.lo > bottom.hi`; a certified reversed order is invalid geometry, while
overlapping or unavailable evidence returns
`ktopo.tessellation.periodic-loop-vertical-separation`. Mean sampled `v`
survives only when both loops are wholly pcurve-less. Mixed coverage,
vertical-periodic charts, nonzero vertical shifts, and unsupported pcurve
classes remain fail-open. The degree-five hostile pcurve has authored
`u = 0, τ/5, …, τ`, controls
`v = [1,-43/5,109/5,-99/5,53/5,1]`, all five legacy seed values equal to one,
and `v(1/8) = -41/64`; source ranges now stop the real `tess_face` path above a
bottom `v = 0` loop. This scan deliberately inherits the prior uncharged
ordering check; explicit operation-stage accounting remains follow-up policy.

The `ktopo` checker now gives orientation authority only to represented planar
straight-loop layouts. Every fin must expose a whole-active-interval-certified
line use on a plane; its UV segments must be finite and nonzero, close with
bit-identical endpoint coordinates, have no exact robust segment intersection
or zero-tolerance adjacent overlap, and have a nonzero
`polygon_orientation2d_iter` sign. Exact robust point location and pairwise
disjointness then establish exactly one outer containing every pairwise-disjoint,
unnested hole; loop order and signed-area magnitude play no role. This removes
the former fixed samples, raw `area != 0`/`area > 0` tests,
largest-absolute-area outer proxy, and periodic nearest-sample unwrapping from
checker authority. Fast emits `WrongLoopOrientation` only when every loop sign
and the unique outer are certified. Unsupported curved, periodic,
nonlinear-chart, tolerance-joined, exact-zero, or non-finite loops are silent in
Fast; Full adds `VerificationGapKind::LoopOrientation` to each unsupported loop,
while unresolved multi-loop outer/hole roles remain the separate
`LoopContainment` gap. Zero-loop sphere and torus faces have no loop-orientation
obligation.

Private evidence uses outer and reversed-hole coordinates near `2^52` whose raw
shoelace sums are both zero, yet exact signs and outer containment survive
repeat, cyclic rotation, reversal, and loop-storage permutation. Exact-zero,
NaN, infinity, and tolerance-joined rings stay outside the strict authority even
when the existing tolerance-aware simplicity proof accepts the join. Public
evidence preserves ordinary holed-sheet results across loop and fin rotation,
storage permutation, and repetition; reversing only the hole faults that loop.
Block, planar sheet, holed sheet, ordinary and oblique polygonal extrusion, and
copied holed-sheet Full-valid evidence is unchanged. All primitives remain Fast
checker-clean and watertight; the ordinary curved cylinder is Fast-valid without
an orientation fault and Full-indeterminate with a `LoopOrientation` gap.
Generic curved-pcurve signed line-integral and curved/periodic containment proofs
remain open.
...
The bounded quadratic/harmonic root-count gate is also migrated.
`quadratic_discriminant(a, b, c)` first asks outward interval arithmetic to
certify the exact dyadic sign of `b² - 4ac`. Inconclusive cancellation uses
expansion products and sums only while all components and scale-by-four
intermediates remain finite and normal; non-finite input or a fallback outside
that conservative envelope returns `None`. Every successful result carries the
exact sign and a finite sign-consistent approximation for later numeric root
construction, with exact algebraic zero represented by `0.0`.

`harmonic_half_angle_roots(cosine, sine, constant)` shares that substrate after
an exactly reversible power-of-two normalization. It classifies root count from
the original identity `cosine² + sine² - constant²`, not from the rounded
half-angle coefficients whose exact sign can disagree after transformation.
An exactly zero leading homogeneous coefficient owns the projective
`y = infinity`, `t = π` root; the all-zero harmonic remains a distinct identity
case. Interval-certified ordinary secants retain the prior quadratic-formula
bits and deterministic order, while exact-positive fallback uses the stable
`q = -0.5 * (b + copysign(sqrt(D), b))` construction. Classification or finite
root representation failure is incomplete evidence: every `kops` planar-conic,
sphere, cylinder, cone, and torus consumer returns `Indeterminate`, while the
geometry graph returns the typed
`IntersectionCertificateError::HarmonicRootClassification`.

Evidence covers exact positive, zero, and negative quadratic discriminants
under large cancellation; the `2^52` harmonic whose rounded transformed
discriminant is zero but exact root count is two; a fixed coefficient bit
pattern whose rounded half-angle transform has the opposite exact sign from the
original harmonic;
the two transverse roots of `cos(t) + 0.991 = 0` under a `0.01` contact
tolerance; exact normalization at `2^700` and `2^-700`; repeat parity for
...
The first bounded higher-polynomial exception is line/Torus. Its exact
source-world quartic is assembled from point/center squared distance and
axis/direction dot-difference expansions under the semantic unit-Line and
orthonormal-Frame contracts. A degree-at-most-four signed pseudo-remainder
Sturm chain isolates distinct roots without tolerance degree trimming or
parameter deduplication; the `gcd(P,P')` layer identifies repeated roots.
When the auxiliary polynomials are nonidentity, completion additionally
isolates the exact squared torus-distance stationary quartic, rejects its
opposite-sign factor with an unsquared interval check, isolates the radial-axis
quadratic, and checks both requested line endpoints. Every rejected critical
bracket needs an outward torus-distance lower bound strictly above the linear
tolerance. When the squared-stationarity and radial-axis coefficient sets are
both exact identities, a dedicated axis branch uses the center-plane linear
polynomial over the tolerance-expanded line range plus the requested endpoints;
world and exact signed-permutation frames pin that admission. A strict distance
lower bound greater than tolerance proves a full-window empty miss. When the
requested line and surface windows admit the center-plane candidate and its
residual is at or below tolerance, one tolerance-only tangent witness is
retained with deterministic longitude and the result remains `Indeterminate`.
A one-sided identity mismatch fails closed. Exact root representation
collision, finite arithmetic outside the reviewed expansion envelope, fixed
isolation cell/depth exhaustion, partial torus windows, unit/orthonormal
invariant failure caught by defensive solver checks, or a distinct
tolerance-only discovery returns `Indeterminate`. Rounded frame-local
polynomial roots survive only as discovery evidence. Distinct algebraic roots
remain separate through isolation, while final world contacts retain
linear-resolution consolidation. A separate narrow Frame-semantic authored-axis
certificate admits a tilted line only when its direction components equal
`±frame.z`, its stored origin replays `Frame::point_at(0, 0, w)`, every
replaying coordinate quotient agrees, and neither adjacent floating-point
offset replays the same point. The exact center polynomial is `w ± t`; with
`e = line.origin - frame.origin - w frame.z`, the bounded empty proof uses exact
radical-free inequalities equivalent to
`sqrt(R² + min(h²)) > r + tolerance + ||e||`. Full-period misses below that
strict bound are `Complete`; admitted tolerance-only center witnesses remain
`Indeterminate`, and partial windows never upgrade. Reversed direction,
center-outside-range, partial-latitude exclusion, a helper-level replay-error
boundary, one-ULP controls, repeat/direct/generic parity, and in-model replay
plateaus pin the contract. Ambiguous general replay skips the General exact-quartic
completion path and fails closed, while an independently exact
coefficient-axis identity remains a valid fallback. This consumes the existing
semantic-orthonormal Frame contract; it is not a source-world exact affine-axis
proof.
This does not migrate the remaining conic/primitive quartics, generic
higher-degree roots, tilted semantic-axis relations outside exact coefficient
identity or the unique stored-value Frame replay contract, ambiguous replay
plateaus without an independent exact identity, a broader source-world exact
affine-axis proof, or contextual replacement of the fixed isolation caps.

`Vec3::normalized` now rejects every non-finite component, retains the previous
division result whenever the ordinary norm is finite, and applies homogeneous
scaling only when finite squared-length overflow makes that norm infinite.
The existing linear-resolution floor remains authoritative. `Line::new`
inherits those semantics. `Frame::new` and `Frame::from_z` return their
previous construction bits when the ordinary candidate satisfies the
orthonormal-frame contract; otherwise a bounded homogeneous cross/cross
projection applies physical and angular degeneracy floors before direct
unitization and a final orthonormality gate. Evidence covers `2^700` and
near-maximum finite vectors, projection overflow, parallel and near-parallel
hints at ordinary and extreme scales, the unchanged geometry determinism
golden in debug and release, downstream rigid-copy/facade suites, and line/line
plus line/Torus parity. This closes the constructor-specific extreme-finite
direction debt, not raw extreme-scale `norm`/`norm_sq`, upstream
dot/cross/distance/subtraction overflow.

`Line2d::new` and `Circle2d::new` now use one private finite nonzero direction
canonicalizer. The ordinary finite-norm path returns the previous
`direction / direction.norm()` bits. Only an infinite norm from finite
squared-length overflow enters homogeneous maximum-component scaling; zero,
non-finite, and norm-underflow-to-zero inputs retain their previous rejection.
This UV-space contract intentionally does not adopt `LINEAR_RESOLUTION`:
`1e-9` remains a valid direction. Evidence covers `2^700` and `f64::MAX`
axes, exact ordinary graph and Plane/Sphere proof parity, and a verified
aligned Plane/Sphere circle whose extreme Line2d/Circle2d pcurves survive
rigid copy in both operand orders. Raw `Vec2` arithmetic and callers that
compute UV lengths or parameter spans before construction remain ordinary
floating arithmetic.
...
parameter after fitting, classification fails closed. Contained planar-conic,
cylinder, cone, torus, and sphere window partitions retain canonical
coefficient/root-index provenance at each numeric cut. Distinct numeric roots
therefore remain separate even below parameter tolerance; different equations
may share an exact chart corner, while different roots of one equation at one
numeric cut return `Indeterminate`. Only exact zero-width source ranges use the
legacy point path. A bit-exact full-period periodic surface axis contributes one
canonical seam equation, while a near-full window retains both bounds.
Model-space contact emission still merges points within linear tolerance as the
explicit physical-coincidence policy rather than an algebraic-root-count
promise.

The `kgraph` oblique spherical seam-root certifier now verifies source-derived
coefficient signs against the finite harmonic, retains per-root provenance
through fitting, and assigns exact derivative directions. Its bounded
algebraic x-side proof certifies strictly positive or negative sides and fails
closed as a singular chart when the evidence contains zero. Exact and clamped
representatives remain distinguished; only exact endpoint roots create
one-sided anchors.
Aliases of the same periodic root may merge, distinct close roots remain
separate, exact numeric collisions fail typed, and carrier ranges wider than
one period are unsupported rather than silently aliased. Evidence
...
finite dyadic `f64` values directly as
`normal · point - normal · origin + bias`, while preserving the legacy rounded
approximation bits on ordinary certified cases. Cancellation builds an exact
expansion from all six coordinate products
`normal[i] * point[i]` and `-normal[i] * origin[i]`, then adds the exact bias;
rounded `point - origin` never becomes decision authority. Non-finite input,
subnormal/overflowing products or expansion components, and fallback outside
the conservative exact-product envelope return `None`. The evidence includes
...
homogeneous curve over the requested range and, when interval cancellation is
inconclusive, use exact affine signs over only the active original Euclidean
controls. The symmetric plane slab and asymmetric plane-`u`/plane-`v` affine
bands are therefore source-provenanced; controls produced by restriction,
Bezier extraction, or splitting are numeric subdivision and sign-variation
guidance only. Invalid or unavailable proof remains `Candidate`.
...
Graph-owned intersection-certificate errors now have a complete owner-defined
classification table, thirteen stable codes, and capabilities for unsupported
trace/carrier parameterizations, regular sphere charts, chart-window enclosure,
lossless harmonic-root representation, and finite residual bounds.
`GraphSurfaceIntersectionError` retains its published aggregate
`kops.intersect.branch-certificate-failure` code and adapter-owned class matrix:
singular/out-of-window sphere charts and harmonic-root representation failures
are `Unsupported`; solver-constructed map/trace/range/residual failures remain
`InternalInvariant`. The exact `IntersectionCertificateError`, leaf code, and
leaf metadata remain available through `std::error::Error::source`.

Rigid copy now has a typed lower boundary,
`ktopo::BodyCopyError::{Kernel, Certificate}`. All six verified/transmitted
certificate reissuers retain `IntersectionCertificateError` rather than mapping
it to `InvalidGeometry`. `Transaction::copy_body_rigid_with_source` exposes
that boundary; the historical `copy_body_rigid` method remains a
`kcore::Result` compatibility wrapper. `PartEdit::copy_body_rigid` uses the
typed entry, and the facade-safe `kernel::BodyCopyError` plus
`KernelError::BodyCopy` preserve class, code, capability, limit, exact nested
sources. The typed failure remains paired with the single operation report, and
...
- Audit classification decisions so exact predicates or interval-certified signs govern
  ...
  decisions are also migrated. The checker sampled-loop sign and
  ...
  decisions and periodic side-loop vertical ordering from original pcurve
  ranges. Concrete remaining targets include generic curved-pcurve signed line
  integrals and broader curved or periodic containment, amplitude/containment
  ...
  landed coefficient sign/zero agreement, higher-polynomial root and
  window-cut classification beyond the bounded line/Torus quartic slice,
  general NURBS roots, and other raw classification branches. Continue the
...
- Continue replacing catch-all `InvalidGeometry` mappings with stable categories
  ...
  ellipse/ellipse projection and typed graph-certificate/rigid-copy paths are
  migrated. The legacy `Transaction::copy_body_rigid` compatibility wrapper and
  other solver-local collapses remain explicit debt.
...
- The seven-row mixed-store affected-root baseline holds a four-body dependent
  ...
  remain unchanged, including their output digests. The distinct four-row
  production-clean v2 ladder holds an unchanged `primitive_mix` at
  4/16/64/256 total bodies. Each timed sample executes exactly one ordinary
  `commit_checked(&[])` scope with zero edits, affected/refreshed roots,
  selected checker obligations, or mutations; commits; pins equal before/after
  store and installed-index snapshots; and requires repeat equality. It
  records one graph-validation start with 61/228/805/3,204 primary graph-node
  starts, one candidate clone with cloned footprint/body-order cardinalities
  equal to total bodies, zero candidate refresh starts and body-order refresh
  entries, two affected-root selection starts examining zero mutation items,
  and zero Fast checker starts. Only the four tagged output digests change.
  These are boundary/cardinality counters rather than elapsed-work measures
  and do not isolate every validation subphase; candidate clone/refresh
  counters exclude validate-all full rebuilds. Optimize where measurements
  require it; add full-rebuild phase instrumentation, production-assembly, and
  broader heterogeneous production-edit-footprint ladders; and retain full
...
- Landed substrate: NURBS-to-Bezier surface subdivision, positive-weight control-hull
  ...
  The bounded clamped NURBS/Plane path now uses the original curve as the only
  geometric proof authority. Original homogeneous range evaluation plus exact
  affine signs over the active original Euclidean controls classify the
  tolerance slab and both asymmetric plane-window affine bands; rounded
  restricted curves, Bezier extraction, and recursive split controls guide
  subdivision and numeric sign variation only. Endpoint, leaf, and
  single-parameter candidates are re-evaluated on the original source and must
  pass exact `affine_dot3` slab signs plus the shared residual admission.
  Bisection maintains strict exact source-sample signs and fails open when a
  sign is unavailable. `Candidate` clipping leaves emit no claimed overlap, and
  the static depth-72 and node-65,536 root/clipping caps never certify
  completion: the solver always returns diagnostic `Indeterminate`. Contained
  overlap ranges merge only for actual interval overlap or bit-exact endpoint
  contact, so a tolerance-sized gap remains separate while a nested range
  canonicalizes into its owner. The ordinary crossing pins exact `t = 0.5`
  and repeat bits. Three legacy-failing fixtures cover an oblique raw dot that
  rounds to zero and formerly became a false overlap together with zero-range
  rejection and curve/normal reversal, rounded split controls that lose a real
  exact midpoint plane contact, and rounded split controls that hide an
  original-source excursion outside a finite UV window; a private
  separated/touching/nested merge regression pins exact-contact ownership.
  Complete root isolation, context-accounted rather than static node/depth
  limits, proof of unresolved UV-boundary cells, affine fallback outside the
  reviewed exponent envelope, and broader NURBS/higher-polynomial roots beyond
  the bounded line/Torus quartic remain open.
...
- The Q2 mixed-store cohort matrix separates total bodies from affected roots
  ...
  Those prior 39 Q2 rows and output digests remain unchanged. The distinct
  production-clean v2 ladder holds unchanged `primitive_mix` totals of
  4/16/64/256 and times one ordinary `commit_checked(&[])` scope with zero
  edits or scoped bodies. It pins committed success, equal before/after store
  and index snapshots, repeat equality, one graph-validation start with
  61/228/805/3,204 primary graph-node starts, one candidate clone with cloned
  footprints/body-order entries equal to total bodies, zero candidate
  refresh/body-order-refresh work, two zero-item affected-root selections, and
  zero Fast starts. Only the four phase-tagged output digests change. These
  counters isolate named invocation boundaries and cardinalities, not elapsed
  work or every graph-validation subphase; clone/refresh counters exclude
  validate-all full rebuilds. `checked_bodies` is selected obligations rather
  than actual Fast starts, and `last_commit` is the latest terminal decision
  recording evidence, so early Full errors may retain prior evidence. Phase
  optimization, full-rebuild phase instrumentation, broader heterogeneous
  production edit footprints, and production assemblies are still explicit
