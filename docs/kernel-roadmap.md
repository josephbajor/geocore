...
Parallel-cylinder Section and regularized CSG retain their certified overlap/contact/common-support/internal-tangency contracts and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses plus all four axial-bound topologies, then atomically classifies each sheet as Outside, Whole, or non-wrapping Open. Public Section maps every non-wrapping Open endpoint proof to a topology-owned cap-ring root, retains its guarded residual endpoint separately, publishes bounded procedural fragments, and stitches four spans plus four certified cap rulings into two closed components across world/oblique frames, replay, and swap. Exact graph and Section N/N-1 work plus a malformed-final-endpoint test deny partial publication; evidence: `two_axial_bounds_publish_four_nonwrapping_upper_spans`, `four_open_span_certificates_are_one_atomic_work_debit`, `bounded_skew_spans_close_with_cap_rulings_and_topology_owned_roots`, `bounded_skew_section_work_accepts_n_and_refuses_n_minus_one_atomically`, and `malformed_final_open_endpoint_refuses_the_face_pair_without_a_prefix`. Contact/corner roots, seam-spanning walks, unsafe cases, persistence, Boolean consumption, and the pinched containing-minus-contained difference remain typed gaps. Next: consume bounded procedural components in mixed face arrangements and regularized skew Cylinder/Cylinder CSG.
