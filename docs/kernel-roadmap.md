...
Certified transverse annulus traces, exact nested line-cycle planar cells, and disk cells now share finite source-arc planning/materialization; a count-independent chord-portal theorem Full-certifies cap-crossing Unite and block-minus-cylinder at 9F/18E/12V, while Intersect and cylinder-minus-block commit at 4F/6E/4V across both rigid frames/orders with literal-derived volume, deterministic X_T, and Fast self-import. Public `body_properties` now consumes one Full-valid exact Plane/Cylinder B-rep and returns outward volume, centroid, and surface-area enclosures without tessellation. Area uses one oriented `-v du` domain integral per face before its Plane/Cylinder Jacobian, preserving holes, full-period bands, and cavity-boundary addition. Primitive, large rigid-placement, cavity, all cap-crossing meanings/orders, unsupported sphere/sheet refusal, repeated-query determinism, and exact N/N-1 work evidence are green. The sixteen-payload bundle is stale pending one licensed-host replay. Next is certified centroidal inertia; distance and clash remain queued.
