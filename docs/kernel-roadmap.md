...
- The seven-row mixed-store affected-root baseline holds a four-body dependent
  ...
  affected roots at 64 total bodies. Seven affected-production-solid
  `primitive_mix` rows form a crossed grid: 1/4/16/64 affected roots at 64
  total bodies and one affected root across 4/16/64/256 total bodies, sharing
  the 64/1 row. The total-size ladder uses one ordinary checked operation scope
  ...
  scope, digest, and index-equality evidence. Elapsed time remains advisory.
  Add production-assembly and broader production-edit-footprint ladders, then
  measure and optimize full graph validation, committed-index cloning,
  deterministic body-order/rank refresh, and global ordinary-commit cost if
  required. Retain full reconstruction as the debug/audit oracle.
...
- The Q2 mixed-store cohort matrix separates total bodies from affected roots
  ...
  production-solid-footprint matrix is now a seven-row crossed `primitive_mix`
  grid: 1/4/16/64 affected roots at 64 total bodies and one affected root at
  4/16/64/256 total bodies. It pins exact ordinary-operation-scope,
  tolerance-journal, affected/order, before/after-store/output, and
  installed-index evidence. Broader production edit footprints, production
  assemblies, full graph validation, committed-index cloning, body-order
  refresh, and remaining global ordinary-commit costs are still explicit
