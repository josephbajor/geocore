...
Parallel-cylinder Section certifies strict positive overlaps and exterior radial separation; at exact axial contact, strict radial secancy retains two proof-joined bounded cap arcs with dual source-ring root identity while tangent/internal/coincident/skew remain typed graph gaps. Public regularized CSG Full-commits strict overlaps, coincident caps, strict separation, exact zero-gap contact, exact external radial tangency, and exact common-cylinder support through positive axial overlap. Common support uses one certified four-endpoint total preorder and open-cell sweep for crossing, nesting, shared-end, and equal intervals; all three operations atomically return zero, one, or two canonical 3F/2E/0V bands. Exact endpoint equality emits cap/ring Face/Edge Merge, nested subtraction emits one ordered cross-body Face Split, and distinct boundaries remain DerivedFrom. The 20-row semantic table plus world/translated-Pythagorean direction/order/repeat matrix retain Full reports, analytic properties, deterministic X_T/Fast self-import, rollback/replay, and exact 64/63 relation, 420/419 one-band, 840/839 two-band, and 3953/3952 property work. Evidence: `exact_common_support_retains_all_four_boundaries_and_endpoint_equalities`, `exact_common_support_executes_the_complete_twenty_row_interval_table`, `exact_common_support_crossing_is_deterministic_across_rigid_frames_and_orders`, `exact_shared_endpoint_merges_are_not_inferred_across_one_ulp`, and `exact_common_support_relation_and_realization_work_have_atomic_frontiers`. One-ULP radius drift and rounded all-nonzero oblique cases remain boundary refusals. Next: exact internal radial tangency over certified axial intervals; skew SSI remains a typed gap.
