...
Parallel-cylinder Section and regularized CSG retain their certified overlap/contact/common-support/internal-tangency contracts and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses plus all four axial-bound topologies, then atomically classifies each sheet as Outside, Whole, or non-wrapping Open. Public Section maps physical roots to topology-owned cap rings separately from guarded residual endpoints and stitches four procedural spans plus four cap rulings into two closed components. Kgraph reissues all 256 stored/exact UV and derivative cells plus two physical-root corridors; kops root-binds them at exact 260 work; Section preserves them through operand/reversal orientation. Both cylinder-side periodic embeddings now consume every cell and corridor to prove complete-domain monotonicity, nonadjacent separation, unique transverse joins, interval orientation, and zero winding. The Boolean adapter arranges both sides deterministically across world/oblique frames, replay, and swap. Exact Section work is 67,811 with N/N-1 refusal; missing and malformed attached proof evidence fail closed. Evidence: `bounded_procedural_embeddings_are_certified_and_arrangeable`, `attached_certificate_with_mismatched_carrier_fails_closed`, `pair_precharge_accepts_n_refuses_n_minus_one_and_checks_overflow`, and `bounded_skew_section_work_accepts_n_and_refuses_n_minus_one_atomically`. Contact/corner roots, seam-spanning walks, unsafe cases, persistence, Boolean composition, and the pinched containing-minus-contained difference remain typed gaps. Next: compose the certified side and cap arrangements into regularized skew Cylinder/Cylinder CSG.
