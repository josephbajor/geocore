...
Parallel-cylinder Section and regularized CSG retain their certified overlap/contact/common-support/internal-tangency contracts and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses plus all four axial-bound topologies, then atomically classifies each sheet as Outside, Whole, or non-wrapping Open. Public Section maps every non-wrapping Open endpoint proof to a topology-owned cap-ring root, retains its guarded residual endpoint separately, publishes bounded procedural fragments, and stitches four spans plus four certified cap rulings into two closed components across world/oblique frames, replay, and swap. Cap-disk arrangements consume the real two-chord topology across world/oblique frames and swap; the periodic Boolean adapter recognizes procedural endpoint identity only behind a certified embedding and refuses the current nonlinear gap. Kgraph reissues indexed stored/exact UV and derivative enclosures for all 256 guarded pcurve cells and two strict physical-root corridors; kops now binds those corridors to the matching caller-ordered endpoint proofs and retains one sealed Open certificate at exact 260 work. Exact graph and Section N/N-1 work plus malformed-evidence tests deny partial publication; evidence: `indexed_pcurve_cells_cover_guarded_range_and_enclose_independent_derivative_oracle`, `physical_root_corridors_reprove_root_uv_and_reject_wrong_guard_or_chart`, `bounded_skew_spans_close_with_cap_rulings_and_topology_owned_roots`, `section_adapter_arranges_bounded_skew_cap_chords_exactly_and_fail_closed`, and `uncertified_procedural_embeddings_still_fail_closed`. Contact/corner roots, seam-spanning walks, unsafe cases, persistence, Section embedding, Boolean composition, and the pinched containing-minus-contained difference remain typed gaps. Next: preserve the sealed proof through Section orientation, certify both periodic side arrangements, then implement regularized skew Cylinder/Cylinder CSG.
