...
Parallel-cylinder Section and regularized CSG retain their certified overlap/contact/common-support/internal-tangency contracts and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses plus all four axial-bound topologies, then atomically classifies each sheet as Outside, Whole, or non-wrapping Open. Public Section keeps physical cap roots separate from guards and stitches four procedural spans plus four cap rulings into two components. Kgraph reissues 256 stored/exact UV and derivative cells plus two root corridors; kops root-binds them at exact 260 work; Section preserves them through reversal. Both periodic side embeddings consume every cell/corridor to prove complete-domain monotonicity, separation, transverse joins, orientation, and zero winding across world/oblique frames, replay, and swap at exact 67,811 Section work. A read-only Boolean bridge now conserves all eight graph fragments once per operand across exactly two periodic, two two-chord/three-cell disk, and two whole-cap arrangements; it classifies every cell deterministically and precharges exact 56/55 work. Wrong-part, duplicate, and indeterminate evidence refuse before publication. Evidence: `bounded_skew_preparation_is_complete_and_rigid_replay_stable`, `bounded_skew_boundary_work_accepts_exact_n_and_refuses_n_minus_one_atomically`, `periodic_evidence_selection_refuses_indeterminate_and_duplicate_matches`, and `wrong_part_refuses_before_boundary_work`. Physical-root carrier trims, persistent composite skew pcurves, planning/realization, contact/corner roots, seam walks, unsafe cases, and the pinched containing-minus-contained difference remain typed gaps. Next: certify physical-root trim authority and planning-only complete-fragment coverage, then add persistent skew realization.
