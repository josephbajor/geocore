...
Parallel-cylinder CSG retains certified overlap/contact/common-support/internal-tangency contracts. Exact nonparallel Cylinder/Cylinder Section publishes a sealed four-member finite-window family; a topology-owned theorem Full-certifies the two 4F/6E/4V bounded-skew lobes from exact family adjacency, cap/radial traversal, periodic replay, and omitted-member separation. Public exact-axis dispatch now atomically commits bounded-cylinder-minus-transverse-cylinder Subtract across world/oblique frames at exact 9,235 post-selection, 2,914 shell-proof, and eight-vertex frontiers; unsupported operation/order meanings retain Full gap evidence and roll back. Evidence: `public_bounded_skew_subtract_require_valid_commits_two_four_face_lobes`, `bounded_skew_public_dispatch_is_proof_gated_across_operation_order_matrix`, and `bounded_skew_public_budgets_accept_n_and_refuse_n_minus_one_atomically`. Certified weighted properties reuse the lobe theorem and fixed-cell enclosures for volume/centroid/area/inertia under rigid frames at exact 1,463,426 analytic-work N/N-1; evidence: `public_bounded_skew_lobes_have_certified_rigid_invariant_properties`. X_T lowers persistent edges to curve-less tolerant transport with paired degree-1 SP-curves over the exact proof partition; parsed invariants, rigid replay, and atomic self-import pass. Evidence: `persistent_skew_edges_emit_certificate_partitioned_degree_one_sp_curves` and `bounded_skew_lobes_have_stable_xt_fast_self_import_twice_and_rigid_replay`; licensed-host replay is queued under R5. Rigid copy reissues every complete-family dependency and passes independent source/copy Full replay, deterministic repeated copy, lineage, and exact N/N-1 rollback across world/oblique frames. Evidence: `complete_family_and_selected_composite_reissue_under_rigid_frames`, `public_bounded_skew_lobes_rigid_copy_full_valid_and_replay_stable`, and `bounded_skew_lobe_rigid_copy_n_minus_one_is_failure_atomic`. Exact common-root GCD proofs group coincident axial cuts atomically, classify Boundary/Contact/Isolated strata, retain two corner contacts beside two open spans, and persist the 12-bit query plan plus all sheet events in family schema v2. Exact work is `896 + 32·popcount(Q) + 260M`; the corner fixture is 1,448 and has an atomic operation-stage frontier. Exact isolated points publish separately from stitched vertices, close the eight-fragment Section cycle with zero gaps, and drive Full-valid corner Subtract in both orders and World/Oblique frames. First-minus-second matches the Parasolid oracle's one-shell 8F/14E/8V representation with two degree-five shared vertices; reverse order commits 7F/13E/8V with a two-loop cylinder face and one endpoint-free ring. Both route through the existing bounded-skew theorem entry, use no degenerate edge, and have deterministic local X_T/Fast self-import evidence; the two new payloads await licensed R5 replay. Evidence: `two_exact_corner_clusters_retain_two_spans_and_two_isolated_points`, `exact_corner_clusters_are_persisted_and_accounted_without_zero_length_members`, `root_cluster_work_has_exact_n_and_atomic_n_minus_one_boundary`, `finite_window_contact_corner_sections_completely_read_only_in_both_orders`, `finite_window_contact_corner_boolean_commits_full_valid_in_both_orders_and_frames`, and `corner_contact_has_stable_xt_fast_self_import_twice_in_both_orders_and_frames`. Open: through-contacts, seam walks, unsafe cases, and other unpublished strata remain typed gaps.
...
Composed subtraction reuses bounded-arc results for separated and crossing follow-up cuts with Full proof, classification, preserved holes, independent volume, deterministic X_T, lineage, and budget rollback. Evidence: `composed_arc_results_support_separated_and_crossing_followup_cuts`, `composed_arc_reuse_budget_denial_is_failure_atomic`. Exit: licensed replay of final queued outputs, then reassess the pocket → boss → hole workflow. Exact tangencies, tilted/skew cuts, and NURBS are deferred.

Begin with narrow end-to-end Boolean slices as M4 cases are certified; **rung 4 remains the active queue head in ORCHESTRATION.md.**
