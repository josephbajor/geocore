...
| M5 Analytic booleans | IMPLEMENTED SLICE | Public block/block CSG is Full-checked, atomic, deterministic, and licensed-host validated. Exact axial rings drive block/cylinder intersection, cylinder-minus-block bands, and one-ring cap-overlap union; whole-boundary selection copies zero-cut union/subtraction. Multi-ring connected union, mixed/cavity subtraction, and curved external evidence remain. |
...
union/subtraction, assembles the one-ring axial cap-overlap union as a
Full-certified capped cylindrical boss, proves zero-cut disjoint intersections
empty, and refuses unsupported curved truth/topology classes before allocation.
The results export deterministically and self-import Fast-valid. Multi-ring and
other connected curved union, block-minus-cylinder mixed/cavity subtraction,
