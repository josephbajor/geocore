...
Parallel-cylinder Section and regularized CSG retain their certified overlap/contact/common-support/internal-tangency contracts and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses plus all four axial-bound topologies, then atomically classifies each sheet as Outside, Whole, or non-wrapping Open. Public Section maps every non-wrapping Open endpoint proof to a topology-owned cap-ring root, retains its guarded residual endpoint separately, publishes bounded procedural fragments, and stitches four spans plus four certified cap rulings into two closed components across world/oblique frames, replay, and swap. Cap-disk arrangements consume the real two-chord topology; the periodic Boolean adapter recognizes procedural endpoint identity only behind certified embedding evidence and still refuses the nonlinear gap. Kgraph reissues all 256 stored/exact UV and derivative cells plus two physical-root corridors; kops binds them to caller-ordered endpoint proofs at exact 260 work; Section preserves that sealed proof through operand/reversal orientation and exposes indexed cells and root corridors without substituting guarded endpoints. Exact graph and Section N/N-1 work plus malformed-evidence tests deny partial publication; evidence: `physical_root_corridors_reprove_root_uv_and_reject_wrong_guard_or_chart`, `malformed_final_open_endpoint_refuses_the_face_pair_without_a_prefix`, and `bounded_skew_spans_close_with_cap_rulings_and_topology_owned_roots`. Contact/corner roots, seam-spanning walks, unsafe cases, persistence, periodic embedding, Boolean composition, and the pinched containing-minus-contained difference remain typed gaps. Next: certify both periodic side embeddings from the Section proof, then implement regularized skew Cylinder/Cylinder CSG.
