...
source/unit-contract failure, or a distinct tolerance-only discovery returns
`Indeterminate`. Rounded frame-local polynomial roots survive only as
discovery evidence. Distinct algebraic roots remain separate through
isolation, while final world contacts retain linear-resolution consolidation.
This does not migrate the remaining conic/primitive quartics, generic
higher-degree roots, auxiliary identity handling, or stable extreme-finite
normalization in `Line::new` and `Frame::new`.
