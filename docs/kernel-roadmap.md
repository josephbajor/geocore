...
- The seven-row mixed-store affected-root baseline holds a four-body dependent
  ...
  ratchets, installed-index equality, and repeat parity. Elapsed time remains
  advisory. Add production-assembly and broader heterogeneous
  production-edit-footprint ladders, then
...
- The Q2 mixed-store cohort matrix separates total bodies from affected roots
  ...
  Broader heterogeneous production edit footprints, production assemblies,
  full graph validation, committed-index cloning, body-order refresh, and
  remaining global ordinary-commit costs are still explicit performance
  boundaries.
