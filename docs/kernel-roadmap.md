...
Parallel-cylinder Section certifies strict positive overlaps and exterior radial separation; at exact axial contact, strict radial secancy retains two proof-joined bounded cap arcs with dual source-ring root identity while tangent/internal/coincident/skew remain typed graph gaps. Public regularized CSG Full-commits strict overlaps, coincident caps, strict separation, exact zero-gap contact, exact external radial tangency, exact common-cylinder support, and the manifold subset of exact unequal-radius internal radial tangency through positive axial overlap. Common support uses one certified four-endpoint total preorder and open-cell sweep for crossing, nesting, shared-end, and equal intervals; all three operations atomically return zero, one, or two canonical 3F/2E/0V bands with exact endpoint Merge and ordered cross-body Face Split where proved. Internal tangency retains directed containment and the same preorder: Intersect returns the contained-radius overlap band or whole contained copy, Unite returns the whole containing copy only when the containing window covers the contained window, and contained-minus-containing returns regularized empty or one/two contained-radius bands. New cut rings retain ordered dual-source DerivedFrom dependencies, unequal supports never Merge, and two-band sides retain ordered Face Split. Deterministic outcomes and operation reports cover rebuilt one/two-band, source-copy, proven-empty, and refusal modes across world/translated-Pythagorean direction/order/repeat matrices; every created result is Full-valid. Representative analytic-property, X_T/Fast self-import, and rollback/replay evidence retains exact 64/63 relation, 420/419 per-band, 840/839 two-band, 26/25 source-copy, and 3953/3952 property work. Evidence: `exact_common_support_executes_the_complete_twenty_row_interval_table`, `exact_internal_tangency_retains_containment_boundaries_and_endpoint_preorder`, `exact_internal_tangency_executes_the_regularized_axial_semantic_table`, `exact_internal_tangency_is_deterministic_across_frames_orders_and_axis_directions`, `internal_tangency_realization_work_has_exact_atomic_frontiers`, `rebuilt_internal_tangent_band_properties_have_exact_frontiers_and_cylinder_oracles`, and `rebuilt_internal_tangent_band_xt_is_stable_and_fast_self_imports_twice`. One-ULP boundary neighbors and unsafe arithmetic fail closed. Internal-tangent Unite requiring a tangent shoulder/shared tangent vertex and containing-minus-contained's pinched non-manifold annulus remain allocation-free typed topology refusals. Next: certify tangent-shoulder/shared-tangent-vertex Unite; keep containing-minus-contained typed non-manifold and skew SSI later.
