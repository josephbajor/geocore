...
Composed subtraction reuses Full-valid Plane/Cylinder results for separated cuts, strict nested hole replacement, and cuts inside existing voids. Evidence: `composed_cuts_replace_nested_holes_and_preserve_unaffected_material` and `composed_cuts_nested_replacement_budget_denial_is_failure_atomic`. Open: crossing/touching source trims, tilted oblique through cuts, broader composition, and licensed replay of both composed-plate payloads.
