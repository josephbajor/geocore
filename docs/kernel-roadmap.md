...
|---|---|---|---|
| 1 | Close M2.5 topology contracts | Production seam/pole/apex interchange fixtures; operation-specific combination/propagation rules beyond landed MEF inheritance and KEF ordered maximum over the tolerance provenance, budgets, and checked facade batch; production-assembly, broader footprint-scaling, and global ordinary-commit performance baselines beyond the landed seven-row mixed-store affected-root cohort and first fixed-64-body `primitive_mix` affected-solid matrix; and discharge the remaining checker-v2 `Full` proof gaps with adaptive incidence, curved-loop/multi-loop containment, and shell proofs. Full-active-interval face-domain containment, private/checked mutation, transaction lineage, affected-root selection, per-body incremental indexing, failure-atomic facade tolerance batching, the opt-in Full-assurance write gate, and the first affected-production-solid footprint ladder are landed. | A B-rep that intersections and features can modify without inventing representation rules mid-boolean. |
...
- The seven-row mixed-store affected-root baseline holds a four-body dependent
  ...
  affected roots at 64 total bodies. The first affected-production-solid
  footprint ladder adds four fixed-64-body `primitive_mix` rows at 1/4/16/64
  affected roots. One ordinary checked transaction grows the first selected
  Face per solid to `2e-8` under an exact `N × 1e-8` operation budget, pinning
  N modified Face mutations and ordered tolerance events,
  affected/refreshed/checked/mutation equality, stable order/output digests,
  and committed-index equality. Elapsed time remains advisory. Add
  production-assembly and broader footprint-scaling ladders, then measure and
  optimize full graph validation, committed-index cloning, deterministic
  body-order/rank refresh, and global ordinary-commit cost if required. Retain
  full reconstruction as the debug/audit oracle.
...
- The Q2 mixed-store cohort matrix separates total bodies from affected roots
  ...
  production-solid-footprint matrix holds `primitive_mix` at 64 bodies while
  sweeping 1/4/16/64 affected roots and pins exact ordinary-transaction,
  tolerance-journal, scope/order/result, and committed-index evidence.
  Broader footprint scaling, production assemblies, and remaining global
  ordinary-commit costs are still explicit performance boundaries.
