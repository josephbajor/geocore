...
Certified transverse annulus traces, exact nested line-cycle planar cells, and disk cells now share finite source-arc planning/materialization; a count-independent chord-portal theorem Full-certifies cap-crossing Unite and block-minus-cylinder at 9F/18E/12V, while Intersect and cylinder-minus-block commit at 4F/6E/4V across both rigid frames/orders with literal-derived volume, deterministic X_T, and Fast self-import. Public `body_properties` consumes one Full-valid exact Plane/Cylinder B-rep and returns outward volume, centroid, surface-area, and unit-density centroidal-inertia enclosures without tessellation. Area finalizes one oriented `-v du` domain integral per face; inertia integrates six anchor-relative homogeneous second moments before a centroidal shift. Primitive, oblique covariance, cavity subtraction, all cap-crossing analytic tensors, common-origin Boolean valuation, refusal, determinism, and exact N/N-1 evidence are green. The sixteen-payload bundle is stale pending one licensed-host replay. Next are certified distance and clash.
