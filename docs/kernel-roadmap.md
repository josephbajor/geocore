...
|---|---|---|---|
| 1 | Close M2.5 topology contracts | Production seam/pole/apex interchange fixtures; operation-specific combination/propagation rules beyond landed MEF inheritance and KEF ordered maximum over the tolerance provenance, budgets, and checked facade batch; affected solid-footprint and production-assembly performance baselines beyond the landed seven-row mixed-store affected-root cohort matrix; and discharge the remaining checker-v2 `Full` proof gaps with adaptive incidence, curved-loop/multi-loop containment, and shell proofs. Full-active-interval face-domain containment, private/checked mutation, transaction lineage, affected-root selection, per-body incremental indexing, failure-atomic facade tolerance batching, and the opt-in Full-assurance write gate are landed. | A B-rep that intersections and features can modify without inventing representation rules mid-boolean. |
...
- The first mixed-store affected-root baseline is landed: seven rows hold a
  four-body dependent cohort across 4/16/64/256 total bodies and independently
  sweep 1/4/16/64 affected roots at 64 total bodies. Exact affected, refreshed,
  checked, order-digest, and output-digest evidence is load-bearing; elapsed
  time remains advisory. Add affected solid-footprint and production-assembly
  ladders, then optimize full graph validation, committed-index cloning, and
  deterministic body-order/rank refresh if measurements require it. Retain
...
- The Q2 mixed-store cohort matrix now separates total bodies from affected
  roots with exact scope counters through 256 total and 64 affected bodies.
  Affected solid-footprint scaling and the remaining global ordinary-commit
  costs are still explicit performance boundaries.
