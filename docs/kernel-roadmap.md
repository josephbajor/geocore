...
Parallel-cylinder Section certifies strict positive overlaps and exterior radial separation; at exact axial contact, strict radial secancy retains two proof-joined bounded cap arcs with dual source-ring root identity while tangent/internal/coincident/skew remain typed graph gaps. Public regularized CSG Full-commits strict overlaps, coincident caps, and strict separation across exact world/oblique axis-direction/order/repeat matrices. Exact zero-gap cap contact now returns mutation-free ProvenEmpty Intersect and an ordered whole-minuend Subtract copy with independent analytic properties, Full reports, complete lineage, deterministic X_T/Fast self-import, rollback, exact 64/63 relation work, and 26/25 copy refusal. Next: Full-commit zero-gap Unite through proof-owned shared-cap arrangement, interface removal, and shell closure.
