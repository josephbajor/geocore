...
tolerance. Exact root representation collision, an auxiliary identity, finite
arithmetic outside the reviewed expansion envelope, partial torus windows,
unit/orthonormal invariant failure caught by defensive solver checks, or a
distinct tolerance-only discovery returns `Indeterminate`. Rounded frame-local
...
higher-degree roots, or auxiliary identity handling.
