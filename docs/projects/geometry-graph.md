...
Exact-parallel strict-secant rulings and exact-nonparallel Whole/Open skew graph descriptors remain operation-local until bounded Open certification. On topology-certified whole source bands, public Section maps every non-wrapping Open endpoint proof to its topology-owned cap ring, sealing endpoint identity, exact Section-oriented physical-root enclosure, and topology-owned point separately from the residual guard. Four bounded procedural spans stitch with four cap rulings into two closed components across world/oblique frames, replay, and swap. Kgraph reissues every guarded nonlinear pcurve cell and both strict root-to-guard corridors; kops root-binds them at exact 260 work; Section preserves them through reversal; the periodic theorem consumes every stored/source cell and corridor to certify both side embeddings and deterministic Boolean plans. Kgraph certifies each bounded span as a normalized nonperiodic `[0,1]` composite with exact source dependencies, conservative boxes, reissued proof, sealed tolerance, and stored/exact-source directed chart-integral enclosures accumulated in the same 256-cell proof pass. An O(1) exact-geometry `DisjointRange` relationship uses full root enclosures, accepts distinct but geometrically equal live source handles, and retains exact 260 work. A sealed finite-window family derives complete sheet occupancy, deterministic member ordinals, exact endpoint tags, and lifted root intervals from exact strict-positive admission plus all four axial-bound outcomes. It retains formula/source geometry and permutation, reissues corridors from each residual, rejects replay, and adds no work beyond `896 + 260M` (1,936 for four members). Section bit-exactly binds authored slabs and projective roots through reversal to unique members; persistent materialization requires membership. Analytic-shell source-matches paired Cylinder uses independently of face IDs, inserts ordered carrier/pcurve dependencies, and journals aggregate edge-tolerance growth with exact rollback. Ktopo resolves both live Cylinder/pcurve bindings and O(1)-certifies all four whole-fin incidences with fail-closed tamper, tolerance, and stale-dependency rejection. The kernel plan-to-component adapter consumes all eight persisted fragments and certifies four physical composites once at exact 1,040 pre-certifier work; world/oblique, both-order, Unite/Intersect/Subtract component inputs Fast-preflight with exact N/N-1. The scaffold remains Full-indeterminate. Evidence: `exact_topology_mints_complete_deterministic_family_without_added_work`, `persistence_input_preserves_graph_root_order_across_section_reversal`, `disjoint_ranges_certify_distinct_handles_replay_swap_and_tamper_rejection`, and `persistent_skew_whole_fin_incidence_certifies_both_sources_and_all_fins`. Public dispatch remains disabled; `RequireValid`, rigid copy, and X_T export remain gated. Next: land the specialized 4F theorem and enable proof-gated nonparallel public dispatch. Contact roots, seam-spanning walks, and coincident families stay typed gaps; unsupported raw descriptors still refuse before mutation.
