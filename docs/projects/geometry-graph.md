...
Exact nonparallel Open spans persist as normalized `[0,1]` composites with paired Cylinder/pcurve dependencies, guarded cell/corridor replay, directed chart integrals, and sealed tolerance. The complete finite-window family owns occupancy, ordinals, endpoint tags, lifted roots, source order, and exact replay at `896 + 260M` work. Ktopo now consumes that authority to Full-certify each 4F/6E/4V bounded-skew lobe; public exact-axis dispatch commits the proven two-lobe Subtract across world/oblique frames with exact N/N-1 rollback. Evidence: `exact_topology_mints_complete_deterministic_family_without_added_work`, `public_bounded_skew_subtract_require_valid_commits_two_four_face_lobes`, and `bounded_skew_public_budgets_accept_n_and_refuse_n_minus_one_atomically`. Ktopo projects the same theorem into fixed-cell weighted-property enclosures; evidence: `public_bounded_skew_lobes_have_certified_rigid_invariant_properties`. X_T lowers each persistent edge to paired degree-1 SP-curves over its exact proof partition; evidence: `persistent_skew_edges_emit_certificate_partitioned_degree_one_sp_curves`. Licensed-host replay is queued under R5. Rigid copy now reissues the complete family, residual/corridor/member certificates, pcurves, and endpoints without cloning graph identity; evidence: `complete_family_and_selected_composite_reissue_under_rigid_frames` and `public_bounded_skew_lobes_rigid_copy_full_valid_and_replay_stable`. Open: contact/corner roots next, then seam walks and coincident families.
