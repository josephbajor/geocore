...
Exact nonparallel Open spans persist as normalized `[0,1]` composites with paired Cylinder/pcurve dependencies, guarded cell/corridor replay, directed integrals, and sealed tolerance. Family schema v2 owns occupancy, ordinals, clustered endpoint roots, every Contact/Isolated sheet event, source order, and the stable 12-bit root-query plan; work is `896 + 32·popcount(Q) + 260M`. Rigid reissue recomputes the plan and event shape before member replay. Evidence: `exact_topology_mints_complete_deterministic_family_without_added_work`, `exact_corner_clusters_are_persisted_and_accounted_without_zero_length_members`, `complete_family_and_selected_composite_reissue_under_rigid_frames`. Ktopo still consumes this authority for Full bounded-skew lobes, certified properties, and deterministic X_T. Open: graph-owned isolated-point proof transport, then seam and through-contact families.
