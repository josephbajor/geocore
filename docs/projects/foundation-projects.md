...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Parallel-cylinder regularized CSG now certifies strict overlaps, coincident caps, strict separation, exact zero-gap cap contact, exact external radial tangency, exact common-cylinder support, and the manifold subset of exact unequal-radius internal radial tangency through positive axial overlap. Internal Intersect returns the contained-radius overlap band or whole contained copy; contained-minus-containing returns regularized empty or one/two contained-radius bands; Unite returns a containing copy for zero contained-only tails, an atomic Full-valid 5F/4E/1V tangent shoulder for one tail, or a canonical 7F/6E/2V three-band/two-shoulder chain for two tails. Each shoulder's bounded full-period contact rings share one exact vertex and one two-fin planar loop; the chain vertices are distinct. Complete ordered lineage, deterministic rigid-frame/order/axis results, independent properties, X_T/Fast self-import, rollback, and exact 2052/2051 chain-realization, 1860/1859 chain-proof, and 11809/11808 chain-property frontiers land. Exact cyclic second-harmonic topology plus division-free dual-frame algebra certifies strict skew misses and all four finite axial-bound root topologies. Root-free windows atomically publish zero, one, or two complete operation-local sheets with exact-source/stored radicand, axial-window, common-chart, separation, and paired-residual proof. Public Section completes read-only as empty or publishes each contained sheet as a canonically oriented closed branch with source-ordered nonlinear pcurves, endpoint-free Whole fragments/components/rings, and typed nonlinear periodic-embedding evidence. Perpendicular/non-right oracles, mixed-order canonicalization, exact Bernstein fallback, boundary refusal, 128/127 classification, 512/511 sheet work, and 256/255 occupancy work land. Rooted windows remain one typed clipped-topology gap without partial publication; non-wrapping open spans are next, while contact roots, seam-wrapping spans, unsafe cases, and persistence remain gaps. Containing-minus-contained remains a pinched non-manifold refusal.
