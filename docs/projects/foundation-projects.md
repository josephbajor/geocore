...
`DerivedFrom` lineage, and checked-commits atomically. Verified-intersection
...
positive profile-normal component into exact planar caps and side
...
fixture is watertight with signed volume 24 within `1e-9`. Curved profiles,
reverse or zero-normal sweeps, revolve, and external X_T validation remain.
...
through five-sample direct-Plane/B-surface or direct-Plane/Offset(B-surface)
slice now retains finite positive noncanonical affine metadata on the
...
one exact adjacent cap-row pair,
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   crossing. The bounded noncanonical affine direct-Plane/B-surface or direct-
   Plane/Offset(B-surface) slice accepts two through five samples, retains
   finite positive chart
   metadata separately from the canonical sample-index basis, and preserves
   exact source/dependency identity. A record-5089 variant pins cumulative
   `139792442/4/10` Work/Items/Depth and exact N/N-1 rollback. No corpus chart
   ...
   the bounded direct-Plane/B-surface and direct-Plane/Offset(B-surface)
   affine slices remain typed.
   ...
   canonical bilinear planar direct-NURBS peer normal to the global X, Y, or Z
   axis. A 7-Work, 1-Item, Depth-1
   ...
   paired pcurves under one scope. X/Y one-span branches pin exact
   14343/14342 Work and 1024/1023 Items. The Z-normal 40-span, 41-control
   horizontal quarter-circle chordal branch pins 573447/573446 Work and
   40960/40959 Items. All orientations retain 10/9 Depth and 2/1 graph
   traversal;
   ...
   reverse-oriented, bit-exact shared edge on a regular longitude seam, removes
   only that edge, and restores the parent map without a polar seam-merger rule.
   ...
   five-anchor region. Both preserve the singular-pole alias with exact
   repeat/swap and outward residual evidence; exact 6/5 piece, 147/146 pair,
   and 588/587 arc ceilings remain pinned. A one-ULP shared-edge mutation and
   ...
   with a positive frame-normal component, with exact planar caps, one planar
   ...
   and the typed facade returns only opaque body and journal values. Curved
   loops, nested material islands, reverse or zero-normal sweeps, and revolve
   remain.
