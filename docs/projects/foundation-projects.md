...
collapsed, first single-wide decomposition, and two-wide all-empty-or-one-positive grid
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   pairs certify empty, or for exactly one positive region with eight
   certified-empty siblings. Closed sibling ownership excludes every
   artificial seam before parent correspondence is restored; exact 9/8
   piece-pair, 252/251 boundary-pair, and 1,008/1,007 arc ceilings preserve
   fail-closed admission. Multi-cell-positive two-wide, polar, and non-exact
