...
now uses the exact v9 corpus profile. V5 remains fixed at
...
NURBS interval residuals remain the proof evidence. The next broader dual-offset
chart fails typed without additional certificate Work.
...
exact three-cell path, exact four-cell path, or exact five-cell path grid arms to
six-or-more-positive, non-path,
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   remain typed and atomic. Half-null pairs and NURBS/Offset-NURBS omissions
   remain typed.
   ...
   forming a two-edge grid path with six certified-empty siblings, for exactly
   four positives forming a three-edge grid path with five certified-empty
   siblings, or for exactly five positives forming a four-edge grid path with
   four certified-empty siblings and sub-full-turn parents.
   ...
   preserve fail-closed admission. Six-or-more-positive two-wide, other
   three-cell layouts, four- or five-cell non-path, non-exact or multi-edge
   ...
   denial restore model and transaction-local budget state exactly. F6 splits and F4
