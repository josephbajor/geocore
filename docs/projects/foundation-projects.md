...
and item 2's representation/trim evidence gate is closed. The first unclosed
foundation obligation is selecting, testing, and adopting finite opt-in
`bounded_v1` tessellation presets from those measurements. Once that policy
step is closed, item 3 has three bounded, parallel-ready
...
2. **Finish hostile-input tessellation policy.** Exact per-face split/vertex/
   ...
   UV/model area, and all face stages. Selecting, testing, and adopting finite
   opt-in presets is the remaining item-2 work. In the body ladder, zero
