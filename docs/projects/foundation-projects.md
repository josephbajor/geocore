...
fields, direct Plane/NURBS in both orders, direct NURBS/NURBS, and direct one-
descriptor Offset(NURBS)/NURBS in both orders. It preserves transformed
carriers and traces, copied paired pcurves, chart metadata, tolerance, ordered
roots, and every admitted transitive dependency. Periodic, dual-offset, and
Offset(NURBS)/Plane transmitted charts remain unsupported, as do unsupported
or nonpositive/nonfinite effective-sphere proof families; altered, stale, or
overdeep source bindings fail atomically during graph binding or persistence.
Attributes remain follow-on work because no authorable storage contract exists;
non-rigid transforms also remain explicit follow-on work. Its first extrusion consumer is also complete: one validated
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   Y, or Z axis. The direct analytic Plane arm alone additionally admits
   exactly two offset descriptors, retains their outer-to-inner signed
   distances as certificate metadata, and proves that the intermediate and
   final cylinder radii remain positive before reducing the chain to its
   effective parallel sheet. A 7-Work, 1-Item, Depth-1
   ...
   Direct peers retain 2/1 graph Work for one descriptor and exact 3/2 Work
   admission plus 3/2 dependency-Depth admission for two descriptors; the
   admitted two-descriptor run consumes 3 graph Work and Depth 3. All
   orientations retain the existing 10/9 certificate Depth and unchanged
   certificate Work/Items budgets. The derived rational effective sheet remains
   discovery-only; original-source evidence owns completion. Singular or
   inconclusive fields, three-or-more-descriptor varying-normal roots,
   two-descriptor roots against planar NURBS or Offset(Plane), incompatible
   peers, and stale or altered sources fail closed and roll back atomically.
   Persistence validates the exact descriptor sequence, so altering the two
   distances while preserving their sum is rejected without allocation.
   ...
   varying-normal Offset(NURBS) families, varying-normal roots of depth three
   or more, and two-descriptor roots outside the direct analytic Plane arm,
   ...
   siblings certified empty, or any exact four-positive grid path with the
   other two siblings certified empty. The reviewed real L orientations are the
   ...
   `[0,1]`/`[0,2]`/`[1,0]`/`[1,1]` path. One-ULP shared-edge mutation,
   duplicate-edge ambiguity, and a four-cell cycle fail closed. The cap layouts preserve the
   singular-pole alias; every layout pins exact repeat/swap and outward
   residual evidence. Exact 6/5 piece,
   ...
   same-row, same-column, full-row, mixed-axis three-cell-path, and exact four-
   positive path families remain indeterminate. A first
