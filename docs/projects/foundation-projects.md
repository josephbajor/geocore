...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Parallel-cylinder regularized CSG retains certified overlap, contact, common-support, and manifold internal-tangency families with Full-check, X_T, rollback, property, and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses and all four finite axial-bound topologies. Graph SSI yields Whole/Open sheets; public Section keeps physical cap roots separate from guards and closes four procedural spans with four cap rulings into two components. Kgraph reissues fixed pcurve cells/root corridors; kops root-binds them; Section preserves them through reversal. The periodic theorem certifies both side embeddings across world/oblique frames, replay, and swap at exact 67,811 work. A read-only Boolean bridge now conserves all eight fragments once per operand across two periodic, two disk-cap, and two whole-cap arrangements, classifies every cell, and precharges exact 56/55 work. Physical-root carrier trims, persistent composite skew pcurves, planning/realization, contact/corner roots, seam walks, unsafe cases, and Boolean composition remain gaps. Containing-minus-contained remains pinched. Next: certify physical-root trims and planning-only coverage, then add persistent skew realization.
