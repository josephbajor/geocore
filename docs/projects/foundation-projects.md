...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Parallel-cylinder regularized CSG retains certified overlap, contact, common-support, and manifold internal-tangency families with Full-check, X_T, rollback, property, and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses and all four finite axial-bound topologies. Graph SSI yields Whole/Open sheets; public Section seals each physical cap root as endpoint identity, exact Section-oriented carrier enclosure, and topology-owned point strictly outside its guard, then closes four procedural spans with four cap rulings. Kgraph reissues fixed pcurve cells/root corridors; kops root-binds them; Section preserves them through reversal. The periodic theorem certifies both side embeddings across world/oblique frames, replay, and swap at exact 67,811 work. The Boolean bridge conserves all eight fragments across two periodic, two disk-cap, and two whole-cap arrangements and classifies every cell at exact 56/55 work. All three truth-selected plans retain eight Section edges and 12/14 coalesced physical edges with opposed distinct-face uses at exact 3,086/3,088/3,728/3,730 N/N-1 post-selection and composite-precharge work. Kgraph persists bounded Open spans as normalized `[0,1]` composites with exact dependencies, reissued proof, sealed tolerance, and stored/exact-source directed chart-integral enclosures. It certifies O(1) exact-geometry `DisjointRange` relationships from full root enclosures at unchanged exact 260 work. A sealed finite-window family derives complete occupancy, member ordinals, exact slab endpoints, and lifted roots from strict-positive admission plus all four bound outcomes, rejecting geometry/permutation/residual/corridor replay at unchanged `896 + 260M` work (1,936 for four). Section bit-exactly binds each trim through reversal to its unique family member; persistent materialization requires that membership. Analytic-shell source-matches paired Cylinder uses independently of face IDs, inserts exact graph dependencies, and journals aggregate edge-tolerance growth; a complete circular-cap source-lineage witness restores oblique Plane/Cylinder ruling-family proof without weakening strict secancy or residual checks. Ktopo resolves both exact Cylinder/pcurve bindings and O(1)-certifies all four whole-fin incidences with tamper, tolerance, and stale-dependency rejection. The 2F/2E/2V scaffold remains Fast-valid, Full-indeterminate, and `RequireValid` rolls back. The kernel plan-to-component adapter consumes all eight persisted fragments and certifies four physical composites once at exact 1,040 pre-certifier work; world/oblique, both-order, Unite/Intersect/Subtract component inputs Fast-preflight. The real non-swapped Subtract input has four ordered disjoint Upper spans and two 4F/6E/4V lobe targets. Evidence: `exact_topology_mints_complete_deterministic_family_without_added_work`, `persistence_input_preserves_graph_root_order_across_section_reversal`, `disjoint_ranges_certify_distinct_handles_replay_swap_and_tamper_rejection`, and `persistent_skew_whole_fin_incidence_certifies_both_sources_and_all_fins`. Public dispatch remains disabled; `RequireValid`, rigid copy, and X_T export remain gated. Contact/corner roots, seam walks, unsafe cases, and pinched containment remain gaps. Next: land the specialized 4F theorem and enable proof-gated nonparallel public dispatch.
