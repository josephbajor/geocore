...
path, or exact four-cell path grid arms to five-or-more-positive, non-path,
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   paths restores one parent region; three- and four-cell paths recheck every
   ...
   preserve fail-closed admission. Five-or-more-positive two-wide, three- or
   four-cell non-path, non-exact or multi-edge shared seams, full-turn aliases,
   ...
   pcurve face split/merge plus bridge-edge removal/ring join, preserve exact
   rollback identity, and return the committed facade journal. Ring joins
   ...
   retains the lower KEMR same-loop/nonempty-split proof. Broader edit families
