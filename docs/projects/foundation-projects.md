...
`DerivedFrom` lineage, and checked-commits atomically. Direct Plane/Plane line
and direct Plane/Sphere latitude or oblique circle descriptors now copy their
ordered sources, transform the carrier, copy the aligned Plane/latitude pcurves
into the copied surface frames or regenerate the oblique spherical pcurve, and
reissue the whole-range certificate in either source order. Facade preflight
admits only those direct `PlaneLine` and `PlaneSphereCircle` proof families;
offset-backed, NURBS, and
transmitted certificates are rejected before scope creation. Attributes and
non-rigid transforms remain explicit follow-on work. Its first extrusion consumer is also complete: one validated
polygonal profile with holes extrudes along any finite translation with a
...
Item 3's explicitly bounded magnitude-thirteen/configurable-
search component is
...
B-surface, or direct-B-surface/B-surface slice now retains finite positive
...
pins `7170/2/10`. Direct B/B and direct Offset(B)/B structural costs are
`14336/2/10`, `28672/3/10`, `43008/4/10`, and `57344/5/10` for two through
five samples. The new offset slice covers both source orders and polynomial or
rational bases while retaining the live root, signed distance, direct original
basis, ordered peer, and unchanged two-source whole-range proof. Nested,
multi-offset, and dual-offset noncanonical forms remain typed unsupported; corpus
...
(reviewed at `[0,2]`/`[1,2]`), and the exact full cap row,
...
six-cell grid, exact seven-cell 2×3-block-plus-tail and opposite-corner-empty layouts, exact connected
eight-cell edge- and corner-empty layouts, and exact exhaustive nine-cell arms to other
seven-, eight-, or nine-positive layouts,
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   Offset(B-surface)/direct B-surface, or direct-B-surface/B-surface slice accepts
   ...
   direct Offset(B)/B charts pin `14336/2/10`, `28672/3/10`, `43008/4/10`, and
   `57344/5/10`; the latter covers both operand orders and polynomial/rational
   bases while retaining the live root, signed distance, original direct basis,
   ordered direct peer, affine metadata, canonical sample-index carrier/pcurves,
   and unchanged original-source proof. Six-sample, nested/multi-offset, and
   dual-offset noncanonical variants remain typed and atomic. A record-5089
   ...
   direct-B-surface/B-surface affine slices remain typed; nested, multi-offset,
   and dual-offset noncanonical forms remain outside the landed direct-offset slice.
   ...
   four pin exact 5/4.
   A first varying-normal arm admits exactly one offset descriptor whose
   original basis is the exact rational quarter-cylinder extrusion, against a
   canonical bilinear planar direct-NURBS, direct analytic Plane, or one safe
   Offset(Plane) descriptor over a direct Plane basis normal to the global X,
   Y, or Z axis. A 7-Work, 1-Item, Depth-1
   ...
   Direct peers retain 2/1 graph Work. All orientations retain 10/9 certificate Depth;
   singular or inconclusive fields, nested varying-normal roots, incompatible
   peers, and stale or altered sources fail closed and roll back atomically.
   Planar or unaligned charts, disjoint or
   ...
   varying-normal Offset(NURBS) families, nested varying-normal roots, nested
   or intersecting/coincident Offset(NURBS)/Offset(NURBS), and
   broader NURBS/NURBS remain explicit unsupported boundaries. Compatible
   one- through four-descriptor constant-normal Offset(NURBS) pairs
   additionally own a graph-only complete-miss proof from strict outward
   original-control separation. The complete 4×4 matrix pins exact `A+B+2`
   Work and `max(A+1,B+1)` dependency depth, including maximum 10/9 Work and
   5/4 Depth admission, zero certificate usage, and no persistence allocation.
   Five-or-more-descriptor roots and coincident/intersecting effective sheets
   remain unsupported. Further fields and
   ...
   three-child cap row with
   all three lower-row siblings certified empty. The single-child path excludes every
   ...
   longitude seams, excludes the latitude seam, and retains one canonical
   11-anchor region. The cap layouts preserve the singular-pole alias; every
   layout pins exact repeat/swap and outward residual evidence. Exact 6/5 piece,
   147/146 pair, and 588/587 arc ceilings remain pinned. A one-ULP shared-edge
   mutation and broader or non-adjacent layouts outside the admitted exact
   same-row, same-column, and cap-row families remain indeterminate. A first
   ...
   siblings certified empty and sub-full-turn parents, or for an exact
   ...
   preserve fail-closed admission. Other seven-, eight-, or nine-positive two-wide,
...
The Q2 topology ladder now has 28 rows and adds a seven-row mixed-store cohort
matrix. It holds four affected minimal bodies across 4/16/64/256 total bodies
and separately sweeps 1/4/16/64 affected roots at 64 total bodies, pinning
affected/refreshed/checked equality, affected order, and complete result
digests. Those deterministic counters establish scoped index/checker work;
full graph validation, committed-index cloning, body-order refresh, and
affected solid-footprint scaling remain explicit performance boundaries. The
complete benchmark manifest now contains 163 cases.
