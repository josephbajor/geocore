...
| F3 | Four slices implemented: centralized class inspection, shared periodic/range and first-wins candidate emission through line/circle and plane/sphere, and canonical unordered-pair normalization across both current curve/curve and surface/surface matrices. Reversed calls share one specialized arm and recover first-operand ordering through completion-preserving result swapping; SSI uses an explicit internal rank so cone/cylinder keeps its natural solver convention. Curve/surface driver migration and the certified subdivision fallback remain. |
