...
canonical sample families complete the existing set. The two-sample line and
witnessed three-sample quadratic now each admit independent exact ordered
chains of one through four Offset(NURBS) descriptors per root: each full 4×4
matrix certifies in both trace orders. The witnessed cubic and the five- and
seven-sample families remain exactly one descriptor per root. All five require
two distinct ordered roots whose terminal sources are
...
or stale chains atomically. The graph trace representation currently caps the
admitted two-sample and quadratic families at four descriptors per root, so a
live depth-five source paired to its maximum-depth trace fails atomically at
graph insertion; broader depth is graph representation/binding work, not a
facade-copy preflight case. Rigid copy transforms both distinct terminal bases
...
recertification at every 4×4 chain pair; and protects both copied roots and
their complete basis closures from removal transitively. The facade verifies
the live roots and proof family before creating an operation scope. Graph-valid
shared-basis or periodic charts remain copy-unsupported, as do nested cubic,
...
The Q2 topology ladder now has 32 rows. Its prior 28 rows remain unchanged,
...
1/4/16/64 affected roots at 64 total bodies. Four new fixed-64-body
`primitive_mix` rows independently sweep 1/4/16/64 affected production-solid
footprints. One ordinary checked transaction grows the first face of every
selected solid to `2e-8` under an exact `N × 1e-8` operation-owned budget;
each row pins exactly N modified Face mutations and ordered tolerance events,
affected/refreshed/checked/mutation equality, stable affected order and full
result digests, and committed-index equality. These deterministic counters
establish both scoped root count and the first production-solid footprint
ladder; full graph validation, committed-index cloning, body-order refresh,
global ordinary-commit cost, broader footprint scaling, and production
assembly remain explicit performance boundaries. The complete benchmark
manifest now contains 167 cases.
