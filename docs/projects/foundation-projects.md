...
| F5 | K1-K3, typed K4 interchange, K5 adoption, and facade body tessellation are implemented: the `kernel` facade owns lifecycle, opaque IDs, classified sources, one-scope outcomes, safe checker subjects, opaque journals, child-accounted procedural evaluation, atomic typed X_T import/export, graph-owned bounded curve/curve intersection with facade-owned proof results, and immutable conforming body meshes with exact closed-solid or true-sheet incidence plus facade-safe face/edge identities. The standalone `kernel-lifecycle` client depends directly only on `kernel` and proves construction, semantic inspection, Full checking, body tessellation, curve intersection, surface evaluation, and byte-stable X_T export/import/re-export. Semantic edit/journal iteration remains. |
...
equal-limit records 1828 and 2008 are also certified for the endpoint-only,
single-periodic-axis form; production reconstruction now uses the exact v3
corpus profile and stops later at the preserved terminated `T/F` boundary.
The remaining parallel legs continue the certified non-octant sphere fallback
beyond its pole-clear, sub-π hit, certified-disjoint, and exact-boundary-lock
collapsed arms to wider/polar decomposition and non-exact tangent cases; and
extend the contextual exact-plane-field/NURBS graph arm beyond safe finite
Offset(Plane) chains to further non-plane fields. These legs do not
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   `81267732`. Production owners now adopt corpus-backed v3: record 1828 and
   every later admitted chart consume exact `115485725/20/10`
   ...
   remain typed and atomic. The exact
   ...
   graph arm now marches direct or safe finite Offset(Plane) fields against a
   genuinely curved direct NURBS in one owner scope, retains paired degree-1
   traces, certifies both lifts over the whole range at depth 10, and persists
   the original root identity in a non-transmitted verified NURBS descriptor
   atomically. Its one-segment curved fixture pins exact 7170/7169 certificate
   Work, while one Offset(Plane) pins exact 2/1 graph visits; planar encodings,
   Offset(NURBS), sphere/NURBS, and
   NURBS/NURBS remain explicit unsupported boundaries. Further fields and
   ...
   fails at 11. Decomposition/union and non-exact collapsed general-window
   contacts are the next sphere boundaries.
