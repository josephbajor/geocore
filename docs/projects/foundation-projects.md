...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Cycle-wide proof-local periodic lifting now certifies seam-crossing bounded loops. Exact planar-shell admission is now independent of its optional typed convex certificate: complete non-convex ten-support star/cylinder Intersect reaches general mixed planning and Full-commits at 17F/45E/30V with literal-derived volume, deterministic X_T, and Fast self-import, while pure planar BSP and convex shortcuts retain the certificate gate. Next is licensed-host replay of the queued deterministic fifteen-payload Boolean bundle; broader NURBS families, revolve, and curved profiles remain milestone backlog.

**External-evidence lane.** Onshape records pin exact historical bytes: the 15-file base and six-file
historical Boolean records are stale; the deterministic fifteen-file offline Boolean bundle is queued, not host-certified. Fingerprints: `docs/oracle-certification.json` and
