...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Parallel-cylinder regularized CSG now certifies strict overlaps, coincident caps, strict separation, and exact zero-gap cap contact. Exact-contact strict radial secancy retains two proof-joined bounded Section arcs with dual source-ring roots while the global graph remains honestly Indeterminate. Contact Intersect is mutation-free ProvenEmpty and ordered Subtract Full-commits one whole minuend copy; Unite Full-commits strict internal disk containment as 5F/4E/0V, exact coincident support as 3F/2E/0V, and equal/unequal-radius strict secancy as 6F/6E/2V across exact world/oblique direction/order/repeat matrices. The results retain independent analytic properties, Full reports, complete DerivedFrom/Merge/Split lineage, deterministic X_T/Fast self-import, rollback, and exact N/N-1 work. Next: regularize exact external-tangent contact Unite as an atomic two-component result without inventing a point-connected manifold shell.
