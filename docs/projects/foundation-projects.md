...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Parallel-cylinder regularized CSG retains certified overlap, contact, common-support, and manifold internal-tangency families with Full-check, X_T, rollback, property, and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses and all four finite axial-bound topologies. Graph SSI merges those topologies into Outside, Whole, or non-wrapping Open sheet states; public Section maps exact Open endpoints to topology-owned cap rings, keeps physical roots separate from residual guards, publishes bounded procedural fragments, and closes four spans with four cap rulings into two components across world/oblique frames, replay, and swap. Kgraph reissues fixed indexed pcurve cells and physical-root continuations; kops root-binds them at exact 260 work; Section preserves the sealed proof through operand/reversal orientation and exposes every cell and corridor. Cap-disk arrangements consume the topology-owned rulings, while the periodic Boolean adapter recognizes procedural endpoint identity only behind certified embedding evidence. Graph and Section N/N-1 work and malformed-evidence refusal are atomic. Contact/corner roots, seam-spanning walks, unsafe cases, persistence, nonlinear periodic embedding, and Boolean composition remain gaps. Containing-minus-contained remains a pinched non-manifold refusal. Next: certify both side embeddings from the Section proof, then compose regularized skew Cylinder/Cylinder CSG.
