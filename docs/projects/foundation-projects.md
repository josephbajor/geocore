...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Parallel-cylinder regularized CSG now certifies strict overlaps, coincident caps, strict separation, exact zero-gap cap contact, exact external radial tangency, exact common-cylinder support, and the manifold subset of exact unequal-radius internal radial tangency through positive axial overlap. Internal Intersect returns the contained-radius overlap band or whole contained copy, covered-window Unite returns the whole containing copy, and contained-minus-containing returns regularized empty or one/two contained-radius bands. Each rebuilt component is an atomic Full-valid 3F/2E/0V cylinder; complete lineage uses DerivedFrom, ordered Face Split, and ordered dual-source cut-ring dependencies without Merge. Deterministic outcomes cover rebuilt one/two-band, source-copy, proven-empty, and refusal modes across world/translated-Pythagorean direction/order/repeat matrices; every created result is Full-valid. Representative analytic-property, X_T/Fast self-import, rollback/replay, and exact 64/63 relation, 420/419 per-band, 840/839 two-band, 26/25 source-copy, and 3953/3952 property evidence close the slice. Evidence: `exact_internal_tangency_executes_the_regularized_axial_semantic_table`, `exact_internal_tangency_is_deterministic_across_frames_orders_and_axis_directions`, `internal_tangency_realization_work_has_exact_atomic_frontiers`, and `rebuilt_internal_tangent_band_xt_is_stable_and_fast_self_imports_twice`. Unite requiring a tangent shoulder/shared tangent vertex and containing-minus-contained's pinched non-manifold annulus remain allocation-free typed topology refusals. Next: certify tangent-shoulder/shared-tangent-vertex Unite; keep containing-minus-contained typed non-manifold and skew SSI later.
