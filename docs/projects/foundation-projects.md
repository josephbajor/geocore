...
total-size behavior. A distinct production-clean ladder holds an unchanged
`primitive_mix` at 4/16/64/256 total bodies and times exactly one ordinary
`commit_checked(&[])` scope with zero edits, affected, refreshed, checked, or
mutated entities. Every row commits, pins equal before/after store and index
snapshots plus stable store/index/output ratchets, and requires repeat equality.
This is the first production-solid global ordinary-commit baseline over the
combined current graph-validation, committed-index-clone, and body-order path;
it is not phase isolation. Phase counters and optimization, broader
