...
open two-sample degree-1, witnessed three-sample quadratic, or witnessed four-
sample cubic dual Offset(NURBS) charts in either ordered-root arrangement. All
three dual families require two
...
`[0,0,0,0,3,3,3,3]` over `[0,3]`. Both higher-order families carry exact
position and paired-UV interpolation witnesses. Rigid copy transforms both
bases and each higher-order exact-position witness set, rebuilds the quadratic
or cubic carrier controls from the transformed positions by the public
interpolation formula, retains ordered roots, exact UV witnesses, paired
pcurves, chart metadata, and tolerance, and reruns the public original-source
recertifier. The facade verifies the live roots and proof family before creating
an operation scope. Same-total multi-distance metadata, nested Offset(NURBS) roots, shared
bases, Offset(Plane) peer roots, periodic source or carrier charts, altered
higher-order witnesses, five-sample, seven-sample, or other dual families,
