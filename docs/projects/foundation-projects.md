...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   direct or one-level nested constant-normal Offset(NURBS) pairs additionally
   own a graph-only complete-miss proof from strict outward original-control
   separation: direct/direct pins 4/depth 2, direct/nested 5/depth 3, and
   nested/nested 6/depth 3, each with exact N/N-1 evidence, zero certificate
   usage, and no persistence allocation. Further fields and
