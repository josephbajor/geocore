...
now uses the exact v14 corpus profile. V5 remains fixed at
...
pinning `INTERSECTION` 1984's attempted 285,283,414 Work. V8 certifies record
...
exact cumulative `436131945/22/10`, preserving historical v1-v13 parity. It
then stops before two-sample dual-offset record 6044, chart 6043, whose isolated
`4352000` Work would request cumulative `440483945`.
...
collapsed, first single-wide decomposition, and two-wide all-empty, one-positive,
...
six-cell grid, and exact seven-cell 2×3-block-plus-tail arms to other
seven-positive or eight-/nine-positive layouts,
disconnected layouts without exact separation,
polar, or non-exact tangent
cases; and
extend the contextual verified graph arms beyond exact direct/safe-Offset(Plane),
direct/safe-Offset(Sphere), compatible direct-NURBS/NURBS, and the capped
three-descriptor constant-normal Offset(NURBS)/NURBS unit-chart arm to
broader offset or
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   5089 at exact `272430166/22/10` by recovering only paired-null interior
   Plane samples through exact frame inversion before the unchanged whole-range
   proof. V8 admits `INTERSECTION` 1984 at exact `315245660/22/10` by snapping
   ...
   `4277250/2/10`, reaches exact `436131945/22/10` with historical v1-v13
   parity, and stops before dual-offset record 6044's `440483945`-Work request.
   Half-null pairs,
   NURBS/Offset-NURBS omissions, and noncanonical five-sample forms remain typed.
   ...
   genuinely curved direct NURBS unit charts and a capped three-descriptor
   ...
   3/2 node-visit and dependency-depth admission, while three pin exact 4/3.
   Planar or unaligned charts, disjoint or
   boundary-only offset/direct ranges, unequal weights, collapsed or non-finite
   sphere offsets, four-or-more-descriptor chains, varying-normal Offset(NURBS), nested
   ...
   one- through three-descriptor constant-normal Offset(NURBS) pairs
   additionally own a graph-only complete-miss proof from strict outward
   original-control separation. The complete 3×3 matrix pins exact `A+B+2`
   Work and `max(A+1,B+1)` dependency depth, including maximum 8/7 Work and
   4/3 Depth admission, zero certificate usage, and no persistence allocation.
   Four-or-more-descriptor roots and coincident/intersecting effective sheets
   ...
   shared-seam union, or for the exact connected seven-cell
   2×3-block-plus-tail union with every other sibling certified empty and
   sub-full-turn parents.
   ...
   Connected four- through seven-cell non-path unions instead prove every
   ...
   preserve fail-closed admission. Other seven-positive and eight-/nine-positive two-wide,
   disconnected five-cell layouts without exact sibling separation, non-exact
   or otherwise ambiguous multi-edge
   shared seams, full-turn aliases, polar, and non-exact collapsed contacts are
