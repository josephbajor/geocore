...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Parallel-cylinder regularized CSG now certifies strict overlaps, coincident caps, strict separation, exact zero-gap cap contact, exact external radial tangency, exact common-cylinder support, and the manifold subset of exact unequal-radius internal radial tangency through positive axial overlap. Internal Intersect returns the contained-radius overlap band or whole contained copy; contained-minus-containing returns regularized empty or one/two contained-radius bands; Unite returns a containing copy for zero contained-only tails and an atomic Full-valid 5F/4E/1V tangent shoulder for one tail. The shoulder's bounded full-period contact rings share one exact vertex and one two-fin planar loop; complete ordered lineage, deterministic rigid-frame/order/axis results, independent properties, X_T/Fast self-import, rollback, and exact 1092/1091 realization, 1113/1112 proof, and 7881/7880 property frontiers land. Evidence: `exact_internal_tangency_executes_the_regularized_axial_semantic_table`, `exact_internal_tangency_is_deterministic_across_frames_orders_and_axis_directions`, `internal_tangency_realization_work_has_exact_atomic_frontiers`, `tangent_shoulder_properties_have_exact_frontier_and_independent_union_oracle`, and `rebuilt_internal_tangent_results_have_stable_xt_and_fast_self_import_twice`. Unite with two contained-only tails still needs the 7F/6E/2V three-band/two-shoulder theorem; containing-minus-contained remains a pinched non-manifold refusal. Next: certify the two-shoulder chain; keep containing-minus-contained typed non-manifold and skew SSI later.
