...
two-isolated-positive, exact shared-edge adjacent-positive, or exact three-cell
path grid arms to four-or-more-positive, non-path, polar, or non-exact tangent
cases; and
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   certified-empty siblings, or for exactly three positives forming a two-edge
   grid path with six certified-empty siblings and sub-full-turn parents.
   ...
   paths restores one parent region; the three-cell path rechecks its second
   seam after the first splice. Exact 9/8 piece-pair, 252/251 boundary-pair, and
   1,008/1,007 arc ceilings preserve fail-closed admission. Four-or-more-positive
   two-wide, three-cell non-path, non-exact or multi-edge shared seams, full-turn
   aliases, polar, and non-exact collapsed contacts are the next sphere boundaries.
