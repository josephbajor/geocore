...
| F2 | Stage 1, two Stage 2 pilots, two bounded Stage 3 pilots, and the first Stage 4 migration are implemented: scoped NURBS proof/march, Full face-domain checking, face tessellation, and NURBS/NURBS Newton conditioning/progress use shared policy/report vocabulary with compatibility, scale, and swap tests. Gradient/minimizer/legacy-slack guards, contextual projection, body tessellation, and broader migrations remain. |
...
| F5 | K1, the K2 block/check/query pilot, and K3 identity/read/surface-evaluation slices are implemented: the `kernel` facade owns lifecycle, opaque IDs, classified sources, deterministic views, one-scope typed outcomes, safe checker subjects, opaque journals, and child-accounted procedural evaluation without lower-layer leakage. Graph-aware intersection, K4, and K5 remain. |
...
| F7 | Q0-Q2 plus first Q3-Q6 slices are implemented: reproducible benchmarks cover topology, analytic body tessellation, NURBS isolation, and truthful X_T I/O; the isolated fuzz workspace adds a bounded `xt_read` contract, stable error inventory, atomicity checks, and licensed seeds. Q3/Q4/Q5 expansion, the pinned-nightly fuzz smoke, more Q6 targets/corpora, and Q7-Q8 remain. |
