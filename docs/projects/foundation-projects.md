...
| F2 | Stage 1, two Stage 2 pilots, the first conditioning-only Stage 3 pilot, and the first Stage 4 migration are implemented: scoped NURBS proof/march, Full face-domain checking, face tessellation, and NURBS/NURBS Newton conditioning now use shared policy/report vocabulary with compatibility and scale tests. Parameter-progress/gradient guards, contextual projection, body tessellation, and broader migrations remain. |
...
| F5 | K1, the first K2 contextual block/check slice, and the K3 geometry-identity/read-view slice are implemented: the `kernel` facade owns lifecycle, opaque IDs, classified source-preserving errors, deterministic views, one-scope typed outcomes, safe checker subjects, and opaque journals without lower-layer leakage. A tessellation/query K2 slice, operation-scoped K3 calls, K4, and K5 remain. |
...
| F7 | Q0-Q2 plus first Q3, Q4, and Q5 slices are implemented: the isolated runner covers topology commits/indexes, ten analytic body-tessellation cases, six NURBS-isolation cases, and eight truthful X_T parse/read/write/round-trip cases with semantic evidence and bounded smokes. Q3/Q4 expansion, additional X_T tiers, and Q6-Q8 remain. |
