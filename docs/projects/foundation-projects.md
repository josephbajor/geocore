...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Parallel-cylinder regularized CSG now certifies strict overlaps, coincident caps, strict separation, exact zero-gap cap contact, and exact external radial tangency with positive axial overlap. Exact-contact strict radial secancy retains two proof-joined bounded Section arcs with dual source-ring roots while the global graph remains honestly Indeterminate. Contact Intersect is mutation-free ProvenEmpty and ordered Subtract Full-commits one whole minuend copy; Unite Full-commits strict internal disk containment as 5F/4E/0V, exact coincident support as 3F/2E/0V, equal/unequal-radius strict secancy as 6F/6E/2V, and exact external tangency as two atomically copied 3F/2E/0V bodies. Positive-overlap tangency uses the same source-copy contract across nested, partial, and shared-end intervals. The results retain properties, Full reports, complete DerivedFrom/Merge/Split lineage, deterministic X_T/Fast self-import, rollback, and exact N/N-1 work across world/translated-Pythagorean direction/order/repeat matrices; one-ULP inward, internal-tangent, unrepresentable-sum, and rounded all-nonzero oblique cases stay refused. Analytic-shell batches now preflight and emit cross-body Face Split plus endpoint-free circle Edge Merge lineage with Full-valid atomic commit and rollback replay. Next: exact common-cylinder-support interval CSG through a generic four-endpoint axial cell sweep using the completed batch-lineage contract; internal tangency and skew SSI remain typed gaps.
