...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Parallel-cylinder regularized CSG now certifies strict overlaps, coincident caps, strict separation, exact zero-gap cap contact, exact external radial tangency, and exact common-cylinder support through positive axial overlap. Common support applies one certified four-endpoint open-cell sweep to all three operations and atomically returns zero, one, or two Full-valid 3F/2E/0V bands. Crossing, nesting, shared-end, and equal intervals retain analytic properties, deterministic X_T/Fast self-import, rollback/replay, exact N/N-1 work, and complete DerivedFrom/Merge/Face-Split lineage across world/translated-Pythagorean direction/order/repeat matrices. Evidence: `exact_common_support_executes_the_complete_twenty_row_interval_table`, `exact_common_support_crossing_is_deterministic_across_rigid_frames_and_orders`, and `exact_common_support_relation_and_realization_work_have_atomic_frontiers`. One-ULP radius drift and rounded all-nonzero oblique cases stay refused. Next: exact internal radial tangency over certified axial intervals; skew SSI remains a typed gap.
