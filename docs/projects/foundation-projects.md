...
with depth at most 64. Facade preflight admits exactly those direct/safe-offset
`PlaneLine` and `PlaneSphereCircle` proof families; NURBS, transmitted,
nonpositive/nonfinite effective-sphere, and deeper proof families are rejected
before scope creation. Attributes and
...
non-cap row retaining eight vertices,
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   certified empty. The single-child path excludes every
   ...
   canonical eight-vertex region. The cap layouts preserve the singular-pole
   alias; every layout pins exact repeat/swap and outward residual evidence. Exact 6/5 piece,
   ...
   same-row, same-column, and full-row families remain indeterminate. A first
