...
descriptors now copy their ordered sources and pcurves, transform the carrier,
and reissue the whole-range certificate. Plane/Sphere, NURBS, and transmitted
certificate reissuance, attributes, and non-rigid transforms remain explicit
follow-on work. Its first extrusion consumer is also complete: one validated
...
direct-Plane/Offset(B-surface), or direct-B-surface/B-surface slice now retains
finite positive noncanonical affine metadata on the
...
pins `7170/2/10`. Direct B/B structural costs are `14336/2/10`,
`28672/3/10`, `43008/4/10`, and `57344/5/10`; corpus
...
exact adjacent pairs in either latitude row and the exact full cap row,
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   Offset(Plane)/B-surface, direct-Plane/Offset(B-surface), or direct-B-surface/
   B-surface slice accepts two through five samples, retains
   ...
   `7170/2/10`, `14339/3/10`, `21508/4/10`, and `28677/5/10`. Direct B/B
   charts retain both ordered original NURBS sources and pin
   `14336/2/10`, `28672/3/10`, `43008/4/10`, and `57344/5/10`; six-sample and
   Offset(B)/B variants remain typed and atomic. A record-5089
   ...
   Plane/Offset(B-surface), and direct-B-surface/B-surface affine slices remain typed.
   ...
   with four certified-empty siblings, or the exact three-child cap row with
   ...
   reverse-oriented, bit-exact shared edge on a regular longitude seam in either
   latitude row, removes only that edge, rejects surviving latitude or unused-
   longitude seams, and restores the parent map without a polar seam-merger rule.
   ...
   six-anchor region without a pole alias. A one-turn-shifted full-row fixture cancels both strict
   ...
   147/146 pair, and 588/587 arc ceilings remain pinned. A one-ULP shared-edge mutation and
   broader or non-adjacent multi-occupied layouts remain indeterminate. A first wide arm decomposes exactly one pole-clear wide operand
