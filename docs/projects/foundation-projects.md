...
descriptor Offset(NURBS)/NURBS in both orders, and exactly one-descriptor
Offset(NURBS)/direct-Plane charts in both orders. It preserves transformed
carriers and traces, copied paired pcurves, chart metadata, tolerance, ordered
roots, the exact one-distance descriptor sequence, and the direct NURBS basis.
The facade verifies the live roots and proof family before creating an
operation scope. Same-total multi-distance metadata, nested Offset(NURBS)
roots, Offset(Plane) peer roots, periodic and dual-offset charts, unsupported
or nonpositive/nonfinite effective-sphere proof families, and altered, stale,
or overdeep bindings fail closed. The lower topology transaction proves
rollback-clean entity/geometry counts and future-identity reuse on rejection.
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   and 588/587 arc ceilings remain pinned. Four-cell cycles and broader or non-
   adjacent layouts outside the admitted exact same-row, same-column, full-row,
   mixed-axis three-cell-path, four-positive path, exactly-five-positive, and
   all-six-positive families remain indeterminate.
   A first
