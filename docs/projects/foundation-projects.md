...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Exact nonparallel Cylinder/Cylinder Section now seals its four bounded procedural spans as one complete finite-window family. The family-driven 4F theorem proves lobe cap/radial traversal, adjacent retained members, periodic replay, and omitted-member separation; public exact-axis dispatch atomically commits two Full-valid 4F/6E/4V bounded-cylinder-minus-transverse-cylinder lobes across world/oblique frames. Exact 9,235 post-selection, 2,914 shell-proof, and eight-vertex N/N-1 frontiers roll back with live sources and stable future IDs. Evidence: `public_bounded_skew_subtract_require_valid_commits_two_four_face_lobes`, `bounded_skew_public_dispatch_is_proof_gated_across_operation_order_matrix`, `bounded_skew_public_budgets_accept_n_and_refuse_n_minus_one_atomically`. Weighted properties now reuse the theorem and fixed-cell enclosures for volume/centroid/area/inertia under rigid frames at exact 1,463,426 analytic-work N/N-1; evidence: `public_bounded_skew_lobes_have_certified_rigid_invariant_properties`. X_T now lowers persistent edges to curve-less tolerant transport with paired degree-1 SP-curves over the exact proof partition; parsed invariants and atomic self-import pass. Evidence: `persistent_skew_edges_emit_certificate_partitioned_degree_one_sp_curves`, `bounded_skew_lobes_have_stable_xt_fast_self_import_twice_and_rigid_replay`; host replay is queued under R5. Proof-safe rigid copy reissues the family and every live Cylinder/pcurve/member dependency; source/copy Full replay, repeat determinism, lineage, and exact N/N-1 rollback pass. Evidence: `complete_family_and_selected_composite_reissue_under_rigid_frames`, `public_bounded_skew_lobes_rigid_copy_full_valid_and_replay_stable`, `bounded_skew_lobe_rigid_copy_n_minus_one_is_failure_atomic`. Open: contact/corner roots next, then seam walks, unsafe cases, and pinched containment.
