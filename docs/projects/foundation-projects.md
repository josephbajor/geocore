...
canonical sample families complete the existing set at the current exactly-one-
descriptor-per-root scope. All five require two
...
five-sample, or seven-sample carrier directly; for either witnessed higher-
order family it transforms the exact-position witness set and rebuilds the
quadratic or cubic carrier controls from the transformed positions by the
public interpolation formula. It retains
...
an operation scope. Same-total multi-distance metadata, shared bases, and
periodic source or carrier charts remain unsupported at every admitted sample
count, including seven-sample. Nested Offset(NURBS) roots at every sample count
including two-sample, Offset(Plane) peer roots, altered higher-order witnesses,
other sample counts, unsupported or nonpositive/nonfinite effective-sphere
proofs, and altered,
