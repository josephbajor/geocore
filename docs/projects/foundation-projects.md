...
sample cubic, or canonical five-sample degree-1 dual Offset(NURBS) charts in
either ordered-root arrangement. All four dual families require two
distinct ordered roots over distinct direct nonperiodic NURBS basis handles and
exactly one descriptor per root. The line uses matching unweighted two-control
carrier and pcurves on knots `[0,0,1,1]` over `[0,1]` with no interpolation
witnesses; the quadratic uses matching unweighted degree-2 three-control
carrier and pcurves on knots `[0,0,0,2,2,2]` over `[0,2]`; and the cubic uses
...
`[0,0,1,2,3,4,4]` over `[0,4]` with neither interpolation witnesses nor a
carrier period. Rigid copy transforms both bases and the line or five-sample
carrier directly; for either witnessed higher-order family it transforms the
exact-position witness set and rebuilds the quadratic or cubic carrier controls
from the transformed positions by the public interpolation formula. It retains
ordered roots, exact UV witnesses when present, paired pcurves, chart metadata,
and tolerance, and reruns the corresponding public original-source
recertifier. The facade verifies the live roots and proof family before creating
an operation scope. Same-total multi-distance metadata, nested Offset(NURBS)
roots, shared bases, Offset(Plane) peer roots, periodic source or carrier
charts, altered higher-order witnesses, seven-sample or other dual families,
unsupported or nonpositive/nonfinite effective-sphere proofs, and altered,
stale, or overdeep bindings fail closed.
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   the two middle-column siblings certified empty, or exactly five positive
   children with the sole remaining sibling certified empty, or all six
   children positive with no empty sibling. The reviewed real L orientations are the
   ...
   disconnected route admits only outer-column vertical pairs
   ...
   and duplicate-edge ambiguity rejection. Other disconnected four-positive
   layouts, plus broader or non-adjacent layouts outside the admitted exact
   same-row, same-column, full-row, mixed-axis three-cell-path, connected
   four-positive path/T/cycle, disconnected outer-column vertical-pair,
   exactly-five-positive, and all-six-positive families, remain indeterminate.
   A first
