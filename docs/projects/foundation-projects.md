...
| F3 | Three slices implemented: centralized class inspection, shared periodic/range and first-wins candidate emission through line/circle and plane/sphere, and canonical curve-pair normalization across the current line/circle/ellipse/NURBS matrix. Reversed calls share one specialized arm and recover first-operand ordering through completion-preserving result swapping. Broader curve/surface and surface/surface driver migration plus the certified subdivision fallback remain. |
