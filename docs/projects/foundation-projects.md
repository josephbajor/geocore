...
| F2 | Stage 1, Stage 1b composition, the bounded NURBS/NURBS Stage 3 scale gate, two Stage 2 pilots, and contextual face/body-tessellation, projection, checker, and generic curve/curve entries are implemented. `OperationContext` owns family-default < session < request budget precedence for graph evaluation, Full checking, tessellation, projection, exact curve-pair isolation, and bounded cell-local seed attempts, including canonical root stops and accounting-mode validation. Whole-body tessellation owns one scope across graph evaluation, projection fallback, refinement/storage, per-patch work, and retained output. X_T reconstruction and generic curve/curve dispatch now account owned iterative work through one scope; exact compatibility, N/N+1 limits, numerical-policy propagation, rollback/completion evidence are pinned. Body/standalone-face tessellation and both standalone projection wrappers are closed to new production callers by the CI retirement ratchet. NURBS/NURBS scale guards are normalized while residuals retain sole acceptance authority; failed curve-pair polishing and its nested fallback minimizers have typed stops with stable bounded diagnostics, and curve/curve structured incomplete evidence reaches the facade. Segment conditioning, overlap/input/dedup slack, other intersection-family minimizer/evidence migrations, contextual caller adoption/ratcheting, corpus-backed bounded tessellation presets, and broader migrations remain. |
| F3 | Thirteen slices implemented: centralized class inspection; shared periodic/scalar fitting and first-wins candidate emission through line/circle and plane/sphere; shared curve/surface finite-range validation and two-axis window fitting across analytic line/planar-conic plane paths, the NURBS/plane bridge, and the general NURBS marcher; canonical unordered-pair normalization across both current curve/curve and surface/surface matrices; typed one-arm-per-pair curve/surface routing across the analytic and provisional NURBS bridges; contextual/shared-scope generic curve/curve dispatch; root control-hull exclusion; deterministic exact NURBS curve-pair subdivision with conservative retained cells plus structured work, candidate, depth, and numeric stops; cell-local safeguarded polishing with verified tolerance-contact witnesses plus an exact seed-attempt budget; and unique exact transverse-root certificates for polynomial and positive-weight rational retained cells in arbitrary exact affine planes using robust coplanarity, Poincaré–Miranda, and interval P-matrix univalence. Strictly separated exact covers become complete curve/curve misses; complete isolation plus certified and verified representatives can complete transverse solves, while partial certificates, seed-limit stops, and the inclusive tolerance boundary retain ordered structured obligations. Failed Newton polishing, fallback selection, and nested fallback-minimizer termination have stable bounded diagnostics, with parameter-resolution preserved as always-on report evidence. Reversed symmetric calls share one specialized arm and recover first-operand ordering through completion-preserving result swapping; SSI uses an explicit internal rank so cone/cylinder keeps its natural solver convention. Curve/surface reports unsupported valid pairs through the structured capability boundary instead of `InvalidGeometry`. Genuinely spatial root certification, certified overlap extent/completeness, and broader shared per-pair solver drivers remain. |
