...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Parallel-cylinder regularized CSG retains certified overlap, contact, common-support, and manifold internal-tangency families with Full-check, X_T, rollback, property, and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses and all four finite axial-bound topologies. Graph SSI merges those topologies into Outside, Whole, or non-wrapping Open sheets; public Section keeps physical cap-ring roots separate from residual guards and closes four procedural spans with four cap rulings into two components. Kgraph reissues fixed pcurve cells and root continuations; kops root-binds them at exact 260 work; Section preserves them through reversal. The periodic theorem now consumes every cell/corridor to certify both side embeddings, global simplicity, transverse joins, orientation, and zero winding; the Boolean adapter arranges both sides across world/oblique frames, replay, and swap. Exact 67,811 Section work and malformed-evidence refusal are atomic. Contact/corner roots, seam-spanning walks, unsafe cases, persistence, and Boolean composition remain gaps. Containing-minus-contained remains a pinched non-manifold refusal. Next: compose the certified side/cap arrangements into regularized skew Cylinder/Cylinder CSG.
