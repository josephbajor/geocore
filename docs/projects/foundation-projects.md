...
polygonal profile with holes extrudes along the positive profile axis into
exact planar caps and side quads with shared edges and per-fin pcurves. The
builder commits atomically, the typed facade returns opaque body/journal
values, Full checking is `Valid`, and the holed fixture is watertight with
signed volume 24 within `1e-9`. Curved profiles, general sweep directions,
revolve, and external X_T validation remain. Item 3's explicitly bounded magnitude-thirteen/configurable-
search component is
...
through five-sample direct-Plane/Offset(B-surface) slice now retains finite
positive noncanonical affine metadata on the canonical shared sample-index
basis. An exact record-5089 variant reuses record 778's metadata and pins
`139792442/4/10` cumulative Work/Items/Depth with N/N-1 rollback; corpus
...
collapsed, exact one-pole cap and polar-by-one-wide 2×3 decompositions,
...
3. **Resume algorithm/API expansion behind the completed gates.** The first facade
   ...
   crossing. The bounded noncanonical affine direct-Plane/Offset(B-surface)
   slice accepts two through five samples, retains finite positive chart
   ...
   the bounded direct-Plane/Offset(B-surface) affine slice remain typed.
   ...
   canonical bilinear planar direct-NURBS peer normal to either the global X or
   Y axis. A 7-Work, 1-Item, Depth-1
   ...
   discovery. Outward-scaled original controls own complete misses, while a
   positive branch retains the live root, original basis, direct peer, and
   paired pcurves under one scope. Its combined proof pins exact
   14343/14342 Work, 1024/1023 Items, 10/9 Depth, and 2/1 graph traversal;
   ...
   pole-clear wide peer. It accepts all-empty or exactly one occupied child
   with five certified-empty siblings, so every artificial seam is excluded
   before the six-cell parent map is restored without a polar seam-merger rule.
   The reviewed `[1,1]` cap/middle-longitude fixture retains one canonical
   three-anchor region and singular-pole alias with exact repeat/swap; exact
   6/5 piece, 147/146 pair, and 588/587 arc ceilings are pinned. A straddling
   multi-occupied fixture remains indeterminate. A first wide arm decomposes exactly one pole-clear wide operand
   ...
   extrudes that same polygon-with-holes profile along the positive frame axis,
   with exact planar caps, one planar quad per boundary segment, shared
   perimeter/vertical edges, and line pcurves on every use. A builder-owned
   translated-prism proof makes Full checking `Valid`; the holed fixture is
   watertight with signed volume 24 within `1e-9`, failed heights are allocation-clean,
   and the typed facade returns only opaque body and journal values. Curved
   loops, nested material islands, general sweeps, and revolve remain.
