...
3. **Resume algorithm/API expansion behind the completed gates — OPEN (current head).** Block/block
   ...
   Parallel-cylinder regularized CSG retains certified overlap, contact, common-support, and manifold internal-tangency families with Full-check, X_T, rollback, property, and exact N/N-1 evidence. Exact nonparallel Cylinder/Cylinder SSI certifies strict misses and all four finite axial-bound root topologies. Graph SSI now merges those topologies into Outside, Whole, or non-wrapping Open sheet states; every Open edge retains caller-ordered axial-root evidence and an independent paired residual certificate under exact 256/255 per-span work. Root-free Whole sheets still publish through public Section, while an Open graph span remains one Section gap until a bounded procedural fragment/component contract lands. Contact/corner roots, seam-spanning walks, unsafe cases, and persistence remain gaps. Containing-minus-contained remains a pinched non-manifold refusal. Next: publish graph-certified Open spans through Section.
