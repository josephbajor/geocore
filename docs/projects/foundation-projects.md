...
| F4 | Phase 1, representative Phase 2 slices, three Phase 3 pilots, and two source-identity vertical migrations are implemented: graph evaluation and intersection certificates own stable classifications; SSI and NURBS curve/curve retain ordered structured incomplete evidence through limits, numeric/method stops, canonicalization, swapping, dispatch, and facade adaptation. Ellipse/ellipse retains every `ProjectionError` through `IntersectionError`, `GeometryIntersectionError`, and `KernelError`. Graph-surface preserves its aggregate adapter code and exact certificate source, while typed rigid copy carries `IntersectionCertificateError` through `ktopo::BodyCopyError`, facade `BodyCopyError`, and `KernelError::BodyCopy` alongside the scoped report. Legacy compatibility wrappers and other solver-local collapses plus broader result-family migrations remain. |
...
| F7 | Q0-Q2b, Q8, and the first Q3-Q6 slices are implemented: CI now enforces Python/oracle freshness, compiles and smoke-runs the excluded benchmark package including graph construction/traversal, contextual body/face tessellation, and curve-pair isolation/solve, and runs both pinned fuzz targets within fixed limits. Q2 now has 43 rows: the prior 39 remain unchanged, including their output digests, the seven-row mixed-store affected-root cohort matrix with exact scope/order/result digests through 256 total and 64 affected bodies, and the existing seven affected-solid `primitive_mix` rows. Those face-only rows retain their crossed production grid, exact budget/event/counter/digest/index evidence, and byte-identical manifest contracts. A distinct four-row fixed-total-64 block-cohort ladder selects only body ordinals divisible by five, at 1/4/8/13 affected blocks (all 13 eligible blocks), because the cylinder and cone ring solids have no vertices. One ordinary checked operation scope grows each selected block's deterministic first Face, then first Edge, then first Vertex to `2e-8` under a deterministic `3N`-event budget charging `1e-8` per entity. It pins N modified Face, N modified Edge, and N modified Vertex mutations; exact ordered `3N` tolerance events; affected/refreshed/checked = N; affected/store/output digest ratchets; installed-index equality; and repeat parity. The distinct production-clean v2 ladder holds an unchanged `primitive_mix` at 4/16/64/256 total bodies and times exactly one ordinary `commit_checked(&[])` scope with zero edits, affected/refreshed roots, selected checker obligations, or mutations. Every row commits and pins equal before/after store and index snapshots plus repeat equality. Exact phase evidence is graph validation 1 with 61/228/805/3,204 primary nodes; candidate clone 1 with cloned footprint/body-order cardinality equal to bodies; candidate refresh and body-order refresh 0; affected selection 2 with zero mutation items; and Fast starts 0. Only the four phase-tagged output digests change. These are invocation/cardinality counters, not elapsed work, and they do not isolate every validation subphase; clone/refresh counters exclude validate-all full rebuilds. `checked_bodies` is selected obligations rather than actual Fast starts, and `last_commit` is the latest terminal decision recording evidence, so early Full errors may retain prior evidence. Phase optimization, full-rebuild phase instrumentation, broader heterogeneous production edit footprints, and production assembly remain open. Q2a drove the reverse-index replacement and its v2 ladder now pins zero full-order rebuilds across 21 rows, including four real verified-intersection diamonds whose dependency-first closure visits a shared basis exactly once. Q2b v2 has ten deterministic closure/path rows through 1,000 edges plus real verified-intersection diamond closure and missing-path cases; the timed closure visits the shared basis exactly once after traversal membership indexing. Q3 `body-tessellation.v3` has 32 rows and pins all 21 composed counters: twenty generalized legacy solids plus four tiers each for a locally verified genuinely curved NURBS block and historically host-certified plane and full-period cylinder sheets. Its 24 solids and eight sheets pin exact directed incidence, topological boundary, face-sense orientation, and the applicable signed-volume or faceted-area measure. Q3 face v2 crosses three representations, three trim topologies, and two tolerances with lift, orientation, boundary, area, mesh, and report evidence. Both matrices pass finite `bounded_v1` profiles and pin root Work N/N-1. Q4 implicit isolation v3 has eight cases including the surface roundoff adversary and multi-span Work N-1; curve-pair isolation v4 has nine span-accounted cases; solve v18 has twenty-eight cases including coordinate, unit-form, and primitive magnitude-two through magnitude-twelve algebraic `1/3` certificates, common-refinement and inverse-history success, altered-history rejection, and exact per-path overlap Work/Items denial. The benchmark manifest now contains 178 total cases. Q3-Q5 expansion, exact coefficient forms beyond twelve, more Q6 targets/corpora, and Q7 remain. |
...
The Q2 topology ladder now has 43 rows. Its prior 39 rows remain unchanged,
...
1/4/16/64 affected roots at 64 total bodies. The existing seven affected-solid
`primitive_mix` rows remain unchanged and form a crossed production grid:
1/4/16/64 affected roots
...
digest, and index-equality evidence. A distinct four-row fixed-total-64
block-cohort ladder selects only `primitive_mix` block roots whose body ordinal
is divisible by five, at 1/4/8/13 affected blocks (all 13 eligible blocks),
because the cylinder and cone ring solids have no vertices. One ordinary
checked operation scope grows each selected block's deterministic first Face,
then first Edge, then first Vertex to `2e-8` under a deterministic `3N`-event
budget charging `1e-8` per entity. It pins N modified mutations of each entity
type, exact ordered events, affected/refreshed/checked = N, affected/store/output
digest ratchets, installed-index equality, and repeat parity. These
deterministic counters establish scoped root count and production-solid
total-size behavior. The distinct production-clean v2 ladder holds an unchanged
`primitive_mix` at 4/16/64/256 total bodies and times exactly one ordinary
`commit_checked(&[])` scope with zero edits, affected/refreshed roots, selected
checker obligations, or mutations. Every row commits and pins equal
before/after store and index snapshots plus repeat equality. Exact boundary
evidence records graph validation 1 with 61/228/805/3,204 primary node starts;
candidate clone 1 with cloned footprint/body-order cardinalities equal to total
bodies; candidate refresh and body-order refresh 0; affected selection 2 with
zero mutation items; and Fast starts 0. The prior 39 Q2 paths and output digests
remain unchanged; only the four tagged production-clean output digests change.
These counters measure invocation boundaries and cardinalities, not elapsed
work, and do not isolate every validation subphase. Candidate clone/refresh
counters exclude validate-all full rebuilds. `checked_bodies` counts selected
obligations rather than actual Fast starts, and `last_commit` reports the latest
terminal decision that recorded evidence, so early Full errors may retain prior
evidence. Phase optimization, full-rebuild phase instrumentation, broader
heterogeneous production edit footprints, and production assembly remain
explicit performance boundaries. The complete benchmark manifest remains at
178 cases.
...
Graph intersection certificates now own their complete stable classification
inventory. `HarmonicRootClassification` and `SingularSphereChart` have distinct
unsupported codes and capabilities; all other certificate variants also have
owner-defined class/code/capability semantics. The graph-surface adapter keeps
its published aggregate code and adapter-owned class meaning while exposing the
exact leaf certificate through `source()`.

Rigid-copy recertification now has a typed lower and facade chain.
`ktopo::BodyCopyError::{Kernel, Certificate}` retains every certificate failure
at the six verified/transmitted reissuance sites.
`Transaction::copy_body_rigid_with_source` exposes it without breaking the
historical `copy_body_rigid -> kcore::Result` contract. The kernel operation
uses the typed entry, wraps it in facade-safe `BodyCopyError`, and returns
`KernelError::BodyCopy` with unchanged class, code, capability, limit, and
nested sources. The failure remains paired with the scoped report, and the
transaction rolls back atomically. The legacy transaction wrapper and other
prose-only `InvalidGeometry` conversions remain migration debt.
