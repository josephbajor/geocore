...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof evidence for exact Plane/Cylinder solids without equating zero lower distance with interference. Exact unequal-radius internal tangency lets Unite return a containing copy for zero contained-only tails, an atomic Full-valid 5F/4E/1V shoulder for one tail, or a canonical 7F/6E/2V three-band/two-shoulder chain for two tails; Intersect and contained-minus-containing retain their contained-radius source-copy/band semantics. Each shoulder's bounded full-period contact rings share one exact vertex and one two-fin planar loop, with complete lineage, deterministic rigid-frame/order/axis results, independent properties, stable X_T/Fast self-import, rollback, loose-tolerance invariance, and exact 2052/2051 chain-realization, 1860/1859 chain-proof, and 11809/11808 chain-property frontiers. Exact dual-frame skew Cylinder/Cylinder SSI now certifies strict full-cycle misses through the public read-only Section path with source-face provenance, 128/127 atomic work, bounded same-scope aggregation, deterministic one-sided-envelope reversal, non-right-angle distance-oracle and swap/rigid/ULP evidence, and structured two-sheet/contact/numeric gaps. Evidence: `tangent_chain_properties_have_exact_frontier_and_independent_union_oracle`, `rebuilt_internal_tangent_results_have_stable_xt_and_fast_self_import_twice`, `non_right_angle_skew_miss_matches_axis_distance_oracle_and_is_swap_stable`, `one_sided_exact_envelope_refusal_retries_reversed_parameterization`, `default_graph_budget_admits_multiple_skew_pairs_in_one_owner_scope`, and `exact_skew_discriminant_miss_is_complete_read_only_and_swap_stable`. Containing-minus-contained remains a pinched non-manifold refusal. Next: construct finite-window two-sheet skew carriers and paired pcurves; keep the pinched difference typed non-manifold.
