...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof evidence for exact Plane/Cylinder solids without equating zero lower distance with interference. Exact unequal-radius internal tangency now lets Unite return a containing copy for zero contained-only tails or an atomic Full-valid 5F/4E/1V shoulder for one tail; Intersect and contained-minus-containing retain their contained-radius source-copy/band semantics. The shoulder's bounded full-period contact rings share one exact vertex and one two-fin planar loop, with complete lineage, deterministic rigid-frame/order/axis results, independent properties, stable X_T/Fast self-import, rollback, loose-tolerance invariance, and exact 1092/1091 realization, 1113/1112 proof, and 7881/7880 property frontiers. Evidence: `exact_internal_tangency_executes_the_regularized_axial_semantic_table`, `internal_tangency_realization_work_has_exact_atomic_frontiers`, `tangent_shoulder_properties_have_exact_frontier_and_independent_union_oracle`, and `rebuilt_internal_tangent_results_have_stable_xt_and_fast_self_import_twice`. Unite with two tails still needs the 7F/6E/2V three-band/two-shoulder theorem; containing-minus-contained remains a pinched non-manifold refusal. Next: certify the two-shoulder chain; keep containing-minus-contained typed non-manifold and skew SSI later.
