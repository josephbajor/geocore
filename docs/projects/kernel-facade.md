...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof for exact Plane/Cylinder solids; parallel-cylinder regularized CSG retains certified internal-tangency families and exact realization/proof/property frontiers. Exact nonparallel Cylinder/Cylinder Section publishes root-free Whole sheets and bounded non-wrapping Open spans. Physical cap-ring roots remain separate from guards; four spans join four cap rulings into two closed components. Kgraph exposes 256 indexed pcurve cells plus two root corridors; kops root-binds them at exact 260 work; Section preserves them through reversal. Both periodic side embeddings now certify global simplicity, transverse joins, orientation, and zero winding from every stored/source cell and corridor; the Boolean adapter arranges both sides across world/oblique frames, replay, and swap at exact 67,811 Section work. Evidence: `bounded_procedural_embeddings_are_certified_and_arrangeable`, `attached_certificate_with_mismatched_carrier_fails_closed`, and `bounded_skew_section_work_accepts_n_and_refuses_n_minus_one_atomically`. Contact/corner roots, seam walks, unsafe arithmetic, persistence, Boolean composition, and pinched containing-minus-contained remain typed gaps. Next: compose the certified side/cap arrangements into regularized skew Cylinder/Cylinder CSG.
