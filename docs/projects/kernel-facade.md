...
- Public exact-axis Cylinder/Cylinder dispatch now routes the complete nonparallel Section/Boolean proof path. The family-driven theorem atomically Full-commits bounded-cylinder-minus-transverse-cylinder Subtract as two 4F/6E/4V lobes across world/oblique frames; all other operation/order meanings retain candidate Full gaps and roll back. Exact 9,235 post-selection, 2,914 shell-proof, and eight-vertex N/N-1 limits preserve sources, store signatures, and future IDs. Evidence: `public_bounded_skew_subtract_require_valid_commits_two_four_face_lobes`, `bounded_skew_public_dispatch_is_proof_gated_across_operation_order_matrix`, `bounded_skew_public_budgets_accept_n_and_refuse_n_minus_one_atomically`. Weighted properties now certify volume/centroid/area/inertia under rigid frames at exact 1,463,426 analytic-work N/N-1; evidence: `public_bounded_skew_lobes_have_certified_rigid_invariant_properties`. X_T now emits paired degree-1 SP-curves over the persistent proof partition with parsed invariants and atomic self-import; evidence: `persistent_skew_edges_emit_certificate_partitioned_degree_one_sp_curves`, `bounded_skew_lobes_have_stable_xt_fast_self_import_twice_and_rigid_replay`. Licensed-host replay is queued under R5. Public rigid copy now reissues every bounded-skew family dependency and preserves source/copy Full validity, deterministic replay, complete lineage, and exact N/N-1 failure atomicity; evidence: `public_bounded_skew_lobes_rigid_copy_full_valid_and_replay_stable`, `bounded_skew_lobe_rigid_copy_n_minus_one_is_failure_atomic`. Open: contact/corner roots next, then seam families and pinched containment.
