...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof evidence for exact Plane/Cylinder solids without equating zero lower distance with interference. Parallel-cylinder CSG Full-commits strict overlaps, coincident caps, strict separations, exact zero-gap contact, and exact external radial tangency through positive axial overlap. Contact Intersect is ProvenEmpty and ordered Subtract copies the minuend; Unite commits strict containment as 5F/4E/0V, coincident support as 3F/2E/0V, strict secancy as 6F/6E/2V, and exact external tangency as two atomically copied 3F/2E/0V bodies. Positive-overlap tangency preserves that source-copy contract for nested, partial, and shared-end intervals. The slices retain properties, Full reports, complete lineage, deterministic X_T/Fast self-import, rollback, and exact work across world/translated-Pythagorean direction/order/repeat matrices; one-ULP inward, internal-tangent, unrepresentable-sum, and rounded oblique cases stay refused. Analytic-shell batches now preflight cross-body Face Split and endpoint-free circle Edge Merge lineage before allocation. Next: exact common-cylinder-support interval CSG through a generic four-endpoint axial cell sweep using that lineage contract; internal tangency and skew SSI remain typed gaps.
