...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof for exact Plane/Cylinder solids; parallel-cylinder regularized CSG retains certified internal-tangency families and exact realization/proof/property frontiers. Exact nonparallel Cylinder/Cylinder Section publishes Whole/Open sheets, seals physical-root identity/enclosure/point separately from residual guards, and closes four procedural spans with four cap rulings. Kgraph exposes 256 indexed pcurve cells plus two root corridors; kops root-binds them; Section preserves them through reversal. Both periodic sides certify simplicity, joins, orientation, and zero winding across world/oblique frames, replay, and swap at exact 67,811 work. The Boolean bridge classifies all cells at exact 56/55 work, then all three truth-selected plans retain every Section fragment and 12/14 physical edges with opposed distinct-face uses at exact 3,086/3,088/3,728/3,730 N/N-1 post-selection and composite-precharge work. Kgraph persists each bounded span as a normalized `[0,1]` composite with exact dependencies, reissued proof, sealed tolerance, and stored/exact-source directed chart-integral enclosures; its O(1) exact-geometry `DisjointRange` proof uses full root enclosures at unchanged exact 260 work. Its sealed finite-window family derives complete occupancy, every member ordinal, endpoint tag, and lifted root interval from exact strict-positive/bound topology, rejecting replay at unchanged `896 + 260M` work (1,936 for four). Section bit-exactly binds authored slabs and projective roots through reversal to unique members, and persistent materialization requires membership. Analytic-shell source-matches paired Cylinder uses independently of face IDs, persists exact graph dependencies, and journals aggregate edge-tolerance growth. A complete circular-cap source-lineage witness restores oblique Plane/Cylinder ruling-family proof without relaxing strict secancy or residual checks. Ktopo resolves both live Cylinder/pcurve dependencies and O(1)-certifies all four whole-fin incidences with fail-closed tamper, tolerance, and stale-dependency rejection. The kernel plan-to-component adapter consumes all eight persisted fragments and certifies four physical composites once at exact 1,040 pre-certifier work; world/oblique, both-order, Unite/Intersect/Subtract component inputs Fast-preflight. The scaffold remains Full-indeterminate. The real non-swapped Subtract input exposes four ordered disjoint Upper spans and two 4F/6E/4V lobe targets. Evidence: `exact_topology_mints_complete_deterministic_family_without_added_work`, `persistence_input_preserves_graph_root_order_across_section_reversal`, `disjoint_ranges_certify_distinct_handles_replay_swap_and_tamper_rejection`, and `persistent_skew_whole_fin_incidence_certifies_both_sources_and_all_fins`. Public dispatch remains disabled; `RequireValid`, rigid copy, and X_T export remain gated. Contact/corner roots, seam walks, unsafe arithmetic, and pinched containment remain typed gaps. Next: land the specialized 4F theorem and enable proof-gated nonparallel public dispatch.
