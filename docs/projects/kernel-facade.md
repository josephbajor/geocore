...
- Public `body_properties` returns outward volume/centroid/area/inertia for Full-valid exact Plane/Cylinder solids. Public `body_distance` and `body_clash` retain ordered proof evidence without equating zero lower distance with interference. Admitted same-signed strict positive-overlap Cylinder/Cylinder Section owns two unique physical overlap ends. Partial-overlap Intersect Full-commits at 4F/6E/4V, and both ordered Subtract meanings Full-commit at 5F/7E/4V across world/oblique frames and repeats with independent properties, deterministic X_T/Fast self-import, retained sources, rollback, and exact realization/property/shell N/N-1 refusal. Next: partial-overlap Unite at 6F/8E/4V through a structural two-host axial-chain shell proof.
