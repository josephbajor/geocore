...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof evidence for exact Plane/Cylinder solids without equating zero lower distance with interference. Exact unequal-radius internal tangency lets Unite return a containing copy for zero contained-only tails, an atomic Full-valid 5F/4E/1V shoulder for one tail, or a canonical 7F/6E/2V three-band/two-shoulder chain for two tails; Intersect and contained-minus-containing retain their contained-radius source-copy/band semantics. Each shoulder's bounded full-period contact rings share one exact vertex and one two-fin planar loop, with complete lineage, deterministic rigid-frame/order/axis results, independent properties, stable X_T/Fast self-import, rollback, loose-tolerance invariance, and exact 2052/2051 chain-realization, 1860/1859 chain-proof, and 11809/11808 chain-property frontiers. Exact dual-frame skew Cylinder/Cylinder SSI certifies strict full-cycle misses and, only on topology-certified whole source bands, strictly contained two-sheet intersections through public read-only Section. Contained sheets retain source-face provenance, source-ordered nonlinear pcurves, canonical normal-cross traversal, two Whole fragments/components/rings, deterministic replay/swap/rigid evidence, and typed nonlinear periodic-embedding evidence without mutating either body; conservative windows not aligned with trim topology, clipping, contact roots, and persistence remain typed gaps. Evidence: `tangent_chain_properties_have_exact_frontier_and_independent_union_oracle`, `rebuilt_internal_tangent_results_have_stable_xt_and_fast_self_import_twice`, `non_right_angle_skew_miss_matches_axis_distance_oracle_and_is_swap_stable`, `one_sided_exact_envelope_refusal_retries_reversed_parameterization`, `default_graph_budget_admits_multiple_skew_pairs_in_one_owner_scope`, `exact_skew_discriminant_miss_is_complete_read_only_and_swap_stable`, and `contained_skew_two_sheet_section_is_complete_read_only_and_transform_stable`. Containing-minus-contained remains a pinched non-manifold refusal. Next: certify clipped finite-window skew topology; keep the pinched difference typed non-manifold.
