...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof for exact Plane/Cylinder solids; parallel-cylinder regularized CSG retains certified internal-tangency families and exact realization/proof/property frontiers. Exact nonparallel Cylinder/Cylinder Section publishes root-free Whole sheets and bounded non-wrapping Open spans. Each bounded procedural end exposes exact cap-ring/root provenance separately from its guarded residual point; four spans join four topology-clipped cap rulings into two closed components across world/oblique frames, replay, and swap. Cap disks arrange the resulting topology-owned chords; the periodic Boolean adapter recognizes procedural endpoint identity only behind certified embedding evidence. Kgraph now exposes fixed-work indexed pcurve cells and strict root-to-guard corridors with stored/exact UV and derivatives; kops binding and Section consumption remain. Evidence: `indexed_pcurve_cells_cover_guarded_range_and_enclose_independent_derivative_oracle`, `physical_root_corridors_reprove_root_uv_and_reject_wrong_guard_or_chart`, `bounded_skew_spans_close_with_cap_rulings_and_topology_owned_roots`, `section_adapter_arranges_bounded_skew_cap_chords_exactly_and_fail_closed`, and `uncertified_procedural_embeddings_still_fail_closed`. Contact/corner roots, seam walks, unsafe arithmetic, persistence, certificate transport/Section embedding, Boolean composition, and pinched containing-minus-contained remain typed gaps. Next: bind and consume fixed-cell evidence through both side arrangements, then implement regularized skew Cylinder/Cylinder CSG.
