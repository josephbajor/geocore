...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof evidence for exact Plane/Cylinder solids without equating zero lower distance with interference. Parallel-cylinder CSG Full-commits strict overlaps, coincident caps, strict separations, and exact zero-gap contact. Contact Intersect is ProvenEmpty and ordered Subtract copies the minuend; Unite commits strict containment as 5F/4E/0V, coincident support as 3F/2E/0V, strict secancy as 6F/6E/2V, and exact external tangency as two atomically copied 3F/2E/0V bodies. The slices retain properties, Full reports, complete lineage, deterministic X_T/Fast self-import, rollback, and exact work across world/Pythagorean-tilted direction/order/repeat matrices; one-ULP and rounded oblique near cases stay refused. Next: certify exact external radial tangency with positive axial overlap for the same source-copy semantics; internal tangency remains a typed gap.
