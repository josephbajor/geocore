...
- Public `body_properties` returns outward volume/centroid/area/inertia for Full-valid exact Plane/Cylinder solids. Public `body_distance` and `body_clash` retain ordered proof evidence without equating zero lower distance with interference. The same-signed nested-height Cylinder/Cylinder Section and 4F/6E/4V public Intersect lens prism are Full-green across world/oblique frames and swap with analytic properties, deterministic X_T/Fast self-import, and exact N/N-1 refusal; tangent/miss/coincident/skew remain typed graph gaps. Next: Full-check axial-inner-minus-outer Subtract.
...
- F5 exposes body section graphs, verified Plane/Cylinder circle and finite exact-family transverse ruling
  ...
  with operation-shared source-root endpoint identity and topology provenance. A same-signed strict-secant nested-height Cylinder/Cylinder pair yields two rulings plus two cap arcs in one topology-owned closed component across world/oblique frames, replay, and swap; public Intersect Full-commits its lens prism while Unite/Subtract remain unsupported. Proof-keyed disk-cap chords and one certified cap arc feed exact disk arrangements with dual classification and period-lifted source-arc lineage. Certified maximal transverse annulus traces retain exact source-loop roots, canonical/lifted UV, integer chart shifts, and count-independent universal-cover noncrossing proof, while semantic Plane supports certify rounded-frame source incidence. Exact planar-circle and periodic-cylinder face-region
