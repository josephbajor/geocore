...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof evidence for exact Plane/Cylinder solids without equating zero lower distance with interference. Parallel-cylinder CSG Full-commits strict overlaps, coincident caps, strict separation, exact contact/tangency, and exact common support. The common-support facade applies one four-endpoint sweep to all three operations and returns zero, one, or two atomic 3F/2E/0V bands with properties, Full reports, complete DerivedFrom/Merge/Face-Split lineage, deterministic X_T/Fast self-import, rollback/replay, and exact work across world/translated-Pythagorean direction/order/repeat matrices. Evidence: `exact_common_support_executes_the_complete_twenty_row_interval_table` and `exact_common_support_relation_and_realization_work_have_atomic_frontiers`. One-ULP radius drift stays refused and one-ULP endpoint drift never becomes Merge. Next: exact internal radial tangency over certified axial intervals; skew SSI remains a typed gap.
