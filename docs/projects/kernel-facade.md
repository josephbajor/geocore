...
- Public `body_properties` returns outward volume/centroid/area/inertia for Full-valid exact Plane/Cylinder solids. Public `body_distance` and `body_clash` retain ordered proof evidence without equating zero lower distance with interference. Admitted same-signed strict positive-overlap Cylinder/Cylinder Section owns two unique physical overlap ends. Partial-overlap Intersect, Unite, and both ordered Subtract meanings Full-commit at 4F/6E/4V, 6F/8E/4V, and 5F/7E/4V across world/oblique repeats with independent properties, deterministic X_T/Fast self-import, retained sources, rollback, and exact realization/property/shell N/N-1 refusal. Next: normalize antiparallel source boundary order onto one certified common axial coordinate; equal-height/shared-end caps await coincident planar-cell ownership/dedup.
