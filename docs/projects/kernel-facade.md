...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof for exact Plane/Cylinder solids; parallel-cylinder regularized CSG retains certified internal-tangency families and exact realization/proof/property frontiers. Exact nonparallel Cylinder/Cylinder Section publishes Whole/Open sheets, keeps physical cap roots separate from guards, and closes four procedural spans with four cap rulings. Kgraph exposes 256 indexed pcurve cells plus two root corridors; kops root-binds them; Section preserves them through reversal. Both periodic sides certify simplicity, joins, orientation, and zero winding across world/oblique frames, replay, and swap at exact 67,811 work. The read-only Boolean bridge conserves all eight fragments once per operand across two periodic, two disk-cap, and two whole-cap arrangements, classifies every cell, and has an exact 56/55 frontier. Evidence: `bounded_skew_preparation_is_complete_and_rigid_replay_stable`, `bounded_skew_boundary_work_accepts_exact_n_and_refuses_n_minus_one_atomically`, and `wrong_part_refuses_before_boundary_work`. Physical-root carrier trims, persistent skew pcurves, planning/realization, contact/corner roots, seam walks, unsafe arithmetic, and pinched containment remain typed gaps. Next: certify physical-root trims and planning-only complete-fragment coverage, then add persistent skew realization.
