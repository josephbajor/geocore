...
- Public `body_properties` returns outward volume/centroid/area/inertia for Full-valid exact Plane/Cylinder solids. Public `body_distance` and `body_clash` retain ordered proof evidence without equating zero lower distance with interference. Admitted same-signed strict positive-overlap Cylinder/Cylinder Section owns two unique physical overlap ends; partial-overlap Intersect now Full-commits at 4F/6E/4V across world/oblique frames, swaps, and repeats with independent properties, deterministic X_T/Fast self-import, retained sources, rollback, and exact N/N-1 refusal. Nested Intersect/Unite/Subtract remain Full-green. Next: both ordered partial-overlap Subtract meanings at 5F/7E/4V through a structural cap-reaching notch proof.
