...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof for exact Plane/Cylinder solids; parallel-cylinder regularized CSG retains its certified internal-tangency result families and exact realization/proof/property frontiers. Exact nonparallel Cylinder/Cylinder SSI now publishes root-free Whole sheets and non-wrapping Open graph spans. Open edges preserve source-ordered nonlinear pcurves, independent subrange residual proof, exact caller-bound/root endpoint evidence, replay/swap stability, seam refusal, and atomic 256/255 work. Public Section still adapts only Whole skew sheets; Open spans remain one gap until bounded procedural fragments/components can map endpoint proofs to source cap rings. Evidence: `nonwrapping_axial_clip_publishes_one_open_upper_span_replay_and_swap_stably`, `open_span_certificate_work_has_exact_n_and_atomic_n_minus_one_boundary`, and `narrow_skew_height_publishes_one_complete_lower_sheet_read_only`. Contact/corner roots, seam-spanning walks, unsafe arithmetic, persistence, and pinched containing-minus-contained remain typed gaps. Next: publish graph-certified Open spans through Section.
