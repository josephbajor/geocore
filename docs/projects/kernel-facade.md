...
- Public exact-axis Cylinder/Cylinder dispatch Full-commits the existing two 4F/6E/4V Subtract lobes with certified properties, deterministic X_T, proof-safe rigid copy, and atomic budgets. Exact common-root clustering now retains two isolated corner contacts beside two open spans; the 32-work query stage is N/N-1 atomic, while kops deliberately returns typed Indeterminate until zero-dimensional graph publication exists. Facade Section/Boolean baselines are read-only and failure-atomic across world/oblique frames and both orders. Evidence: `root_cluster_work_has_exact_n_and_atomic_n_minus_one_boundary`, `finite_window_contact_corner_retains_downstream_gaps_read_only_in_both_orders`, `finite_window_contact_corner_boolean_refuses_atomically_in_both_orders`. Open: publish isolated contacts separately from stitch vertices, close the eight-fragment cycle, then enable Boolean; through-contacts, seams, and pinched containment remain gaps.
