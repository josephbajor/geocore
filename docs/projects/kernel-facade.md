...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof evidence for exact Plane/Cylinder solids without equating zero lower distance with interference. Exact unequal-radius internal tangency lets Unite return a containing copy for zero contained-only tails, an atomic Full-valid 5F/4E/1V shoulder for one tail, or a canonical 7F/6E/2V three-band/two-shoulder chain for two tails; Intersect and contained-minus-containing retain their contained-radius source-copy/band semantics. Each shoulder's bounded full-period contact rings share one exact vertex and one two-fin planar loop, with complete lineage, deterministic rigid-frame/order/axis results, independent properties, stable X_T/Fast self-import, rollback, loose-tolerance invariance, and exact 2052/2051 chain-realization, 1860/1859 chain-proof, and 11809/11808 chain-property frontiers. Exact dual-frame skew Cylinder/Cylinder SSI certifies strict full-cycle misses and exact root topology for all four finite axial bounds. Root-free windows atomically complete as empty or publish one/two whole sheets through read-only Section. Each contained sheet retains source-face provenance, source-ordered nonlinear pcurves, canonical normal-cross traversal, one Whole fragment/component/ring, deterministic replay/swap/rigid evidence, and typed nonlinear periodic-embedding evidence without mutating either body. A rooted window remains one typed clipped-topology gap with no partial publication; contact roots, seam-wrapping spans, unsafe arithmetic, and persistence remain gaps. Evidence: `tangent_chain_properties_have_exact_frontier_and_independent_union_oracle`, `rebuilt_internal_tangent_results_have_stable_xt_and_fast_self_import_twice`, `exact_skew_discriminant_miss_is_complete_read_only_and_swap_stable`, `root_free_one_sheet_window_is_complete_replay_and_swap_stable`, `axial_clip_work_has_exact_n_and_atomic_n_minus_one_boundary`, and `narrow_skew_height_publishes_one_complete_lower_sheet_read_only`. Containing-minus-contained remains a pinched non-manifold refusal. Next: certify non-wrapping open skew spans; keep the pinched difference typed non-manifold.
