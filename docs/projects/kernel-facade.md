...
- Public `body_properties`, `body_distance`, and `body_clash` retain Full proof evidence for exact Plane/Cylinder solids without equating zero lower distance with interference. Parallel-cylinder CSG Full-commits strict overlaps, coincident caps, and strict separations. Exact zero-gap contact returns regularized ProvenEmpty Intersect and an ordered whole-minuend Subtract copy; Unite Full-commits strict internal disk containment as 5F/4E/0V, exact coincident support as 3F/2E/0V, and equal/unequal-radius strict secancy as 6F/6E/2V across exact world/oblique direction/order/repeat matrices, with analytic properties, Full reports, complete DerivedFrom/Merge/Split lineage, deterministic X_T/Fast self-import, rollback, and exact work frontiers. Next: regularize exact external-tangent contact Unite as an atomic two-component result without inventing a point-connected manifold shell.
