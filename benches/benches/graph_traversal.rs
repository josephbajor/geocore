//! Q2b geometry-graph traversal ladder.
...
use kernel_benchmarks::graph_traversal::{CASES, GraphTraversalFixture, verify};
...
fn graph_traversal(criterion: &mut Criterion) {
    for case in CASES {
        ...
        group.bench_function(case.path.rsplit_once('/').unwrap().1, move |bencher| {
            let fixture = prepared.get_or_init(|| GraphTraversalFixture::new(case));
