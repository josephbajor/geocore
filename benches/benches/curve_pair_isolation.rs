//! Q4 contextual NURBS curve-pair isolation benchmark ladder.
...
use kernel_benchmarks::curve_pair_isolation::{CASES, fixture, verify};
...
fn curve_pair_isolation(criterion: &mut Criterion) {
    for case in CASES {
        ...
        group.bench_function(case.path.rsplit_once('/').unwrap().1, move |bencher| {
            ...
            let expected = expected.get_or_init(|| {
                ...
                assert_eq!(first, repeated, "Q4 curve-pair repeatability drift");
                first
            });
...
criterion_group! {
    ...
    targets = curve_pair_isolation
}
criterion_main!(q4_curve_pair);
