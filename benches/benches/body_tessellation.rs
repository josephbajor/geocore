...
use kernel_benchmarks::body_tessellation::{
    BodyTessellationRun, CASES, compatibility_session, fixture, tessellation_options, verify,
};
    ...
    for case in CASES {
        ...
        group.bench_function(case.path.rsplit_once('/').unwrap().1, move |bencher| {
            ...
            let context = OperationContext::new(&session, Tolerances::default())
                .expect("reviewed Q3 tolerances satisfy compatibility-v1 precision");
            ...
            let expected = expected.get_or_init(|| {
                ...
                assert_eq!(first.mesh, repeated.mesh, "Q3 preflight mesh drift");
                assert_eq!(first.report, repeated.report, "Q3 preflight report drift");
            ...
            bencher.iter_custom(|iterations| {
                ...
                for _ in 0..iterations {
                    ...
                    let outcome = fixture.tessellate_outcome(options, &context);
                    measured += started.elapsed();
                    let run = BodyTessellationRun::from_outcome(
                        outcome.expect("reviewed Q3 policy must be valid"),
                    );
