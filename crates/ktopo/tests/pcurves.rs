...
fn inspect_invalid_edit<R>(
    store: &mut Store,
    inspect: impl FnOnce(&mut AssemblyStore<'_>) -> R,
) -> R {
    let mut transaction = store.transaction().unwrap();
    let result = {
        let mut assembly = transaction.assembly();
        inspect(&mut assembly)
    };
...
fn make_first_edge_curveless_tolerant(store: &mut Store, body: ktopo::entity::BodyId) -> EdgeId {
    edit_body(store, body, |store| {
        ...
        edge.tolerance =
            Some(EntityTolerance::operation(LINEAR_RESOLUTION, "pcurve-test").unwrap());
        edge_id
    })
}
...
fn checker_requires_the_face_domain_to_match_periodic_charts() {
    ...
    let faults = inspect_invalid_edit(&mut store, |store| {
        store.get_mut(first_fin).unwrap().pcurve =
