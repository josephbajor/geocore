...
pub(super) fn certify_shell_surgery(
    ...
    let proposals = discover_circular_product_surgeries(store, shell_id)?;
...
fn discover_circular_product_surgeries(
    store: &Store,
    shell_id: ShellId,
) -> Result<Vec<ShellSurgeryEvidence>> {
...
mod tests {
    ...
    fn accepted_evidence(store: &Store, shell: ShellId) -> ShellSurgeryEvidence {
        discover_circular_product_surgeries(store, shell)
            .unwrap()
            .into_iter()
            .find(|evidence| {
