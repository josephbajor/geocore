...
#[test]
fn two_host_axial_chain_is_full_valid_across_frames_and_permutations() {
        ...
        for permuted in [false, true] {
            ...
            let output = transaction
                .assemble_analytic_shell(
                    &two_host_axial_chain_union_input(frame, permuted),
                    TOLERANCE,
                )
                .unwrap();
...
fn two_host_axial_chain_accepts_asymmetric_outer_tails() {
    ...
    let mut transaction = store.transaction().unwrap();
    let output = transaction
        .assemble_analytic_shell(&two_host_axial_chain_union_input(frame, false), TOLERANCE)
        .unwrap();
    let circle_center = |key| {
        let edge = transaction.store().get(edge_for_key(&output, key)).unwrap();
        let CurveGeom::Circle(circle) = transaction.store().get(edge.curve.unwrap()).unwrap()
        else {
            panic!("whole edge must retain its circle carrier")
        };
        circle.frame().origin()
    };
    let first_tail = (circle_center(8) - frame.origin()).dot(frame.z());
    let second_tail = (circle_center(9) - frame.origin()).dot(frame.z()) - 1.0;
    assert_eq!(first_tail.to_bits(), (-2.0_f64).to_bits());
...
#[test]
fn two_host_axial_chain_accepts_unequal_strict_secant_radii() {
    for frame in [Frame::world(), oblique_frame()] {
        let mut store = Store::new();
        let mut transaction = store.transaction().unwrap();
        let output = transaction
            .assemble_analytic_shell(
                &unequal_radius_two_host_axial_chain_union_input(frame, false),
                TOLERANCE,
            )
            .unwrap();
        assert_eq!(
...
fn two_host_axial_chain_tampers_fail_closed() {
    ...
    let upper_first_arc = edge_for_key(&output, 1);
    let mut wrong_endpoints = transaction.store().clone();
    wrong_endpoints
        .get_mut(upper_first_arc)
        .unwrap()
        .vertices
        .swap(0, 1);
    assert_eq!(
    ...
    let upper_second_arc = edge_for_key(&output, 3);
    let replacement_curve = transaction.store().get(upper_first_arc).unwrap().curve;
