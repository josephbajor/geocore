...
const TERMINAL_CASES: [TerminalCase; 2] = [TerminalCase::Notch, TerminalCase::Boss];

impl TerminalCase {
    fn input(self, frame: Frame, permuted: bool) -> crate::analytic_shell::AnalyticShellInput {
        match self {
            Self::Notch => cap_reaching_cylinder_notch_input(frame, permuted),
            Self::Boss => cap_reaching_cylinder_boss_input(frame, permuted),
        }
    }
}
...
#[test]
fn cap_reaching_feature_table_is_full_valid_across_frames_and_permutations() {
        ...
        for frame in [Frame::world(), oblique_frame()] {
            for permuted in [false, true] {
                ...
                let output = transaction
                    .assemble_analytic_shell(&case.input(frame, permuted), TOLERANCE)
                    .unwrap();
                assert_eq!(output.faces().len(), 5, "{case:?}");
                assert_eq!(output.edges().len(), 7, "{case:?}");
                assert_eq!(output.vertices().len(), 4, "{case:?}");
                assert_eq!(
...
fn cap_reaching_orientation_and_endpoint_tampers_fail_closed() {
    for case in TERMINAL_CASES {
        ...
        let mut transaction = store.transaction().unwrap();
        let output = transaction
            .assemble_analytic_shell(&case.input(Frame::world(), false), TOLERANCE)
            .unwrap();

        let feature_face = face_for_key(&output, 1);
        let mut wrong_sense = transaction.store().clone();
        let flipped_sense = wrong_sense.get(feature_face).unwrap().sense.flipped();
        wrong_sense.get_mut(feature_face).unwrap().sense = flipped_sense;
        assert_eq!(
        ...
        let feature_fin = wrong_incidence
            ...
            .unwrap();
        let flipped_sense = wrong_incidence.get(feature_fin).unwrap().sense.flipped();
        wrong_incidence.get_mut(feature_fin).unwrap().sense = flipped_sense;
        assert_eq!(
...
fn cap_reaching_work_accepts_exact_n_rejects_n_minus_one_and_skips_inapplicable() {
    for case in TERMINAL_CASES {
        ...
        let required = proof_work(transaction.store(), output.shell(), 2)
            .unwrap()
            .unwrap();
        assert_eq!(required, 13_600, "{case:?}");
