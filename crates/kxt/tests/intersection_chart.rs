...
fn noncanonical_plane_bsurface_intersection_file(rational: bool, swapped: bool) -> XtFile {
    let mut file = plane_bsurface_intersection_file(rational, swapped);
    let intersection = file
        .nodes
        .iter()
        .find_map(|(&index, node)| (node.code == code::INTERSECTION).then_some(index))
    ...
    set_field(
        ...
        chart,
        "base_parameter",
        Value::Double(0.003_586_209_316_397_325),
    );
    set_field(
        &mut file,
        chart,
        "base_scale",
        Value::Double(0.999_999_996_408_403),
    );
    file
}
...
fn make_affine_noncanonical_finite_open_chart(file: &mut XtFile, retained_count: usize) {
    ...
    let intersection = file
        ...
        .unwrap();
    let chart = ptr(file, intersection, "chart");
    let endpoints = match file.field(&file.nodes[&chart], "hvec").unwrap() {
        Value::Arr(values) if values.len() == 2 => [
            values[0].as_vector().unwrap(),
            values[1].as_vector().unwrap(),
        ],
        value => panic!("unexpected chart positions: {value:?}"),
    };
    let data = ptr(file, intersection, "intersection_data");
    let endpoint_uv = match file.field(&file.nodes[&data], "values").unwrap() {
        Value::Arr(values) if values.len() == 8 => [
            values[..4]
                .iter()
        ...
        value => panic!("unexpected chart data: {value:?}"),
    };
    let denominator = (retained_count - 1) as f64;
    let positions = (0..retained_count)
        .map(|sample| {
            let t = sample as f64 / denominator;
            Value::Vector(Some(std::array::from_fn(|axis| {
                endpoints[0][axis] * (1.0 - t) + endpoints[1][axis] * t
            })))
        })
        .collect();
    let values = (0..retained_count)
        .flat_map(|sample| {
            let t = sample as f64 / denominator;
            let endpoint_uv = &endpoint_uv;
            (0..4).map(move |component| {
                Value::Double(endpoint_uv[0][component] * (1.0 - t) + endpoint_uv[1][component] * t)
            })
        })
        .collect();
    set_field(
        ...
        chart,
        "chart_count",
        Value::Int(retained_count as i64),
    );
    set_field(file, chart, "hvec", Value::Arr(positions));
    set_field(file, data, "values", Value::Arr(values));
    let trim = file
        .nodes
        .iter()
        .find_map(|(&index, node)| {
        ...
        .unwrap();
    let end_parameter = (retained_count - 1) as f64;
    if file
        .field(&file.nodes[&trim], "parm_1")
        .and_then(Value::as_f64)
        .unwrap()
        == 0.0
    {
        set_field(file, trim, "parm_2", Value::Double(end_parameter));
    } else {
        set_field(file, trim, "parm_1", Value::Double(end_parameter));
    }
    set_field(
        file,
        chart,
...
fn noncanonical_offset_bsurface_intersection_file(
    rational: bool,
    nested: bool,
    swapped: bool,
    retained_count: usize,
) -> XtFile {
    let mut file = offset_bsurface_intersection_file(rational, nested, swapped);
    make_affine_noncanonical_finite_open_chart(&mut file, retained_count);
    file
}

fn noncanonical_bsurface_bsurface_intersection_file(
    rational_a: bool,
    rational_b: bool,
    swapped: bool,
    retained_count: usize,
) -> XtFile {
    let mut file = bsurface_bsurface_intersection_file(rational_a, rational_b, swapped);
    make_affine_noncanonical_finite_open_chart(&mut file, retained_count);
    file
}

fn noncanonical_offset_nurbs_bsurface_intersection_file(
    rational: bool,
    swapped: bool,
    retained_count: usize,
) -> XtFile {
    let mut file = offset_nurbs_bsurface_intersection_file(rational, swapped);
    make_affine_noncanonical_finite_open_chart(&mut file, retained_count);
    file
}
...
#[test]
fn noncanonical_bsurface_bsurface_retains_affine_metadata_ordered_sources_and_dependencies() {
    ...
    for rational_a in [false, true] {
        for rational_b in [false, true] {
            for swapped in [false, true] {
                let file = noncanonical_bsurface_bsurface_intersection_file(
                    rational_a, rational_b, swapped, 2,
                );
                let mut store = Store::new();
                let reconstruction = reconstruct(&file, &mut store).unwrap();
                assert_eq!(reconstruction.bodies.len(), 1);
                ...
                for index in 0..2 {
                    let live = store.get(sources[index]).unwrap().as_nurbs().unwrap();
                    assert_eq!(live.is_rational(), expected_rational[index]);
                    assert_eq!(certificate.traces()[index].as_nurbs(), Some(live));
                    assert!(
                        store
                            .geometry()
                            .dependents(GeometryRef::Surface(sources[index]))
...
fn offset_nurbs_charts_bind_live_root_basis_and_signed_distance_in_both_orders() {
    for (rational, swapped) in [(false, false), (false, true), (true, false), (true, true)] {
...
#[test]
fn noncanonical_offset_nurbs_bsurface_retains_affine_metadata_root_basis_and_ordered_peer() {
    const BASE_PARAMETER: f64 = 0.003_586_209_316_397_325;
    const BASE_SCALE: f64 = 0.999_999_996_408_403;
    let expected_knots = [0.0, 0.0, 1.0, 1.0];
    ...
    for rational in [false, true] {
        for swapped in [false, true] {
            let file = noncanonical_offset_nurbs_bsurface_intersection_file(rational, swapped, 2);
            let mut store = Store::new();
            let reconstruction = reconstruct(&file, &mut store).unwrap();
            assert_eq!(reconstruction.bodies.len(), 1);
            let (curve, descriptor) = store
            ...
            let sources = intersection.source_surfaces();
            let offset_index = usize::from(swapped);
            let direct_index = 1 - offset_index;
            let expected_classes = if swapped {
                [SurfaceClass::Nurbs, SurfaceClass::Offset]
            } else {
                [SurfaceClass::Offset, SurfaceClass::Nurbs]
            ...
            let live_offset = store.get(root).unwrap().as_offset().copied().unwrap();
            let offset_trace = certificate.traces()[offset_index]
                .as_offset_nurbs()
                .unwrap();
            assert_eq!(
                offset_trace.signed_distance(),
                live_offset.signed_distance()
            );
            assert_eq!(offset_trace.signed_distance().abs(), 0.125);
            assert_eq!(
                store.get(live_offset.basis()).unwrap().as_nurbs(),
                Some(offset_trace.basis())
            );
            let live_peer = store
                .get(sources[direct_index])
                .unwrap()
                .as_nurbs()
                .unwrap();
            assert_eq!(
                certificate.traces()[direct_index].as_nurbs(),
                Some(live_peer)
            );
            assert_eq!(live_peer.is_rational(), rational);
            assert_eq!(
                store
                    .geometry()
                    .direct_dependencies(GeometryRef::Curve(curve))
...
#[test]
fn noncanonical_offset_nurbs_bsurface_two_through_five_cover_both_orders_and_basis_forms() {
    for retained_count in 2..=5 {
        for rational in [false, true] {
            for swapped in [false, true] {
                let file = noncanonical_offset_nurbs_bsurface_intersection_file(
                    rational,
                    swapped,
                    retained_count,
                );
                ...
                let intersection = store
                    .geometry()
                    .curves()
                    .find_map(|(_, curve)| curve.as_transmitted_nurbs_intersection())
                    .unwrap();
                let certificate = intersection.certificate();
                assert_eq!(
                    (
                    ...
                    (0.003_586_209_316_397_325, 0.999_999_996_408_403)
                );
                let mut expected_knots = vec![0.0, 0.0];
                expected_knots.extend((1..retained_count - 1).map(|sample| sample as f64));
                expected_knots.extend([(retained_count - 1) as f64; 2]);
                assert_eq!(
                    certificate.carrier().knots().as_slice(),
                    expected_knots.as_slice()
                );
                for pcurve in certificate.pcurves() {
                    assert_eq!(pcurve.knots().as_slice(), expected_knots.as_slice());
                    assert_eq!(pcurve.param_range(), certificate.carrier_range());
                }
                ...
                let offset_trace = certificate.traces()[offset_index]
                    .as_offset_nurbs()
                    .unwrap();
                assert_eq!(root.signed_distance(), offset_trace.signed_distance());
                assert_eq!(
                    store.get(root.basis()).unwrap().as_nurbs(),
                    Some(offset_trace.basis())
                );
                let direct = store
                    .get(sources[direct_index])
                    ...
                    .as_nurbs()
                    .unwrap();
                assert_eq!(direct.is_rational(), rational);
                assert_eq!(certificate.traces()[direct_index].as_nurbs(), Some(direct));
                store.geometry().validate().unwrap();
            }
        }
    }
...
#[test]
fn noncanonical_plane_bsurface_retains_affine_metadata_sources_and_dependencies() {
    const BASE_PARAMETER: f64 = 0.003_586_209_316_397_325;
    const BASE_SCALE: f64 = 0.999_999_996_408_403;
    let expected_knots = [0.0, 0.0, 1.0, 1.0];

    for (rational, swapped) in [(false, false), (false, true), (true, false), (true, true)] {
        let file = noncanonical_plane_bsurface_intersection_file(rational, swapped);
        let mut store = Store::new();
        let reconstruction = reconstruct(&file, &mut store).unwrap();
        assert_eq!(reconstruction.bodies.len(), 1);
        let (curve, descriptor) = store
        ...
        let sources = intersection.source_surfaces();
        let expected_classes = if swapped {
            [SurfaceClass::Nurbs, SurfaceClass::Plane]
        } else {
            [SurfaceClass::Plane, SurfaceClass::Nurbs]
        };
        assert_eq!(
            sources.map(|source| store.get(source).unwrap().class()),
            expected_classes
        );
        let nurbs_index = usize::from(!swapped);
        assert_eq!(
            store.get(sources[nurbs_index]).unwrap().as_nurbs(),
            certificate.traces()[nurbs_index].as_nurbs()
        );
        assert_eq!(
            store
                .geometry()
...
#[test]
fn noncanonical_offset_plane_bsurface_retains_affine_metadata_roots_and_dependencies() {
    const BASE_PARAMETER: f64 = 0.003_586_209_316_397_325;
    const BASE_SCALE: f64 = 0.999_999_996_408_403;
    let expected_knots = [0.0, 0.0, 1.0, 1.0];
    ...
    for rational in [false, true] {
        for nested in [false, true] {
            for swapped in [false, true] {
                let file =
                    noncanonical_offset_bsurface_intersection_file(rational, nested, swapped, 2);
                let mut store = Store::new();
                let reconstruction = reconstruct(&file, &mut store).unwrap();
                assert_eq!(reconstruction.bodies.len(), 1);
                let (curve, descriptor) = store
                ...
                let nurbs_index = usize::from(!swapped);
                let root = sources[offset_index];
                let certified_plane = certificate.traces()[offset_index].as_plane().unwrap();
                let mut evaluator =
                    store.eval_context(kgraph::EvalLimits::default(), Tolerances::default());
                assert_eq!(
                    evaluator.surface_exact_plane(root).unwrap(),
                ...
                let live_nurbs = store.get(sources[nurbs_index]).unwrap().as_nurbs().unwrap();
                assert_eq!(live_nurbs.is_rational(), rational);
                assert_eq!(
                    certificate.traces()[nurbs_index].as_nurbs(),
                    Some(live_nurbs)
                );
                ...
                assert!(
                    store
                        ...
                        .unwrap()
                        .contains(&GeometryRef::Curve(curve)),
                    "the transmitted chart must retain its actual offset root"
                );
                assert!(
                    store
                        .geometry()
                        ...
                        .unwrap()
                        .iter()
                        .any(|dependent| matches!(dependent, GeometryRef::Surface(_))),
                    "the offset chain must retain its terminal Plane basis"
                );
                assert_eq!(
                    store
                        .geometry()
...
fn plane_bsurface_proof_has_exact_work_items_and_depth_boundaries() {
    for file in [
        ...
        noncanonical_plane_bsurface_intersection_file(true, true),
        offset_bsurface_intersection_file(true, false, false),
        offset_bsurface_intersection_file(true, true, true),
        noncanonical_offset_bsurface_intersection_file(true, false, false, 2),
        noncanonical_offset_bsurface_intersection_file(true, true, true, 2),
...
#[test]
fn noncanonical_offset_plane_bsurface_two_through_five_samples_have_exact_boundaries() {
    for (retained_count, exact_work) in [(2, 7_170), (3, 14_339), (4, 21_508), (5, 28_677)] {
        let file = noncanonical_offset_bsurface_intersection_file(
            true,
            retained_count % 2 == 1,
            retained_count % 2 == 0,
            retained_count,
...
fn two_nurbs_trace_proofs_have_exact_summed_work_items_and_depth_boundaries() {
    for file in [
        ...
        noncanonical_bsurface_bsurface_intersection_file(true, false, true, 2),
        offset_nurbs_bsurface_intersection_file(true, false),
        offset_nurbs_bsurface_intersection_file(true, true),
        noncanonical_offset_nurbs_bsurface_intersection_file(true, false, 2),
        noncanonical_offset_nurbs_bsurface_intersection_file(true, true, 2),
    ] {
        for (stage, resource, mode, exact) in [
            (
                INTERSECTION_CHART_CERTIFICATE_WORK,
...
#[test]
fn noncanonical_offset_nurbs_bsurface_two_through_five_samples_have_exact_boundaries() {
    for (retained_count, exact_work) in [(2, 14_336), (3, 28_672), (4, 43_008), (5, 57_344)] {
        let file = noncanonical_offset_nurbs_bsurface_intersection_file(
            retained_count != 5,
            retained_count % 2 == 1,
...
#[test]
fn noncanonical_bsurface_bsurface_two_through_five_samples_have_exact_boundaries() {
    for (retained_count, exact_work) in [(2, 14_336), (3, 28_672), (4, 43_008), (5, 57_344)] {
        let file = noncanonical_bsurface_bsurface_intersection_file(
            retained_count != 5,
            retained_count >= 4,
...
fn bsurface_bsurface_periodic_closed_altered_and_noncanonical_cases_fail_with_reuse() {
    ...
    let out_of_family = noncanonical_bsurface_bsurface_intersection_file(true, true, false, 6);
    assert!(matches!(
        reconstruct(&out_of_family, &mut store),
        Err(XtError::Unsupported {
            capability: XtCapability::IntersectionChartConvention,
...
#[test]
fn noncanonical_bsurface_bsurface_malformed_unsafe_and_out_of_family_fail_atomically() {
    let fixture = || noncanonical_bsurface_bsurface_intersection_file(true, false, true, 5);
    let valid = fixture();
    let sources = intersection_bsurface_sources(&valid);
    let mut store = Store::new();

    let mut malformed = fixture();
    let nurbs = ptr(&malformed, sources[0], "nurbs");
    set_field(&mut malformed, nurbs, "n_u_vertices", Value::Int(3));
    assert!(matches!(
        reconstruct(&malformed, &mut store),
        Err(XtError::BadField { index, .. }) if index == sources[0]
    ));
    assert_eq!(store_counts(&store), (0, 0, 0));

    let mut unsafe_source = fixture();
    let nurbs = ptr(&unsafe_source, sources[1], "nurbs");
    let poles = ptr(&unsafe_source, nurbs, "bspline_vertices");
    let mut values = match unsafe_source
        .field(&unsafe_source.nodes[&poles], "vertices")
        .unwrap()
        .clone()
    {
        Value::Arr(values) => values,
    ...
    values[2] = Value::Double(values[2].as_f64().unwrap() + 0.05);
    set_field(&mut unsafe_source, poles, "vertices", Value::Arr(values));
    assert!(matches!(
        reconstruct(&unsafe_source, &mut store),
        Err(XtError::IntersectionCertificate { .. })
    ));
    assert_eq!(store_counts(&store), (0, 0, 0));

    let mut unsupported = fixture();
    let nurbs = ptr(&unsupported, sources[0], "nurbs");
    set_field(&mut unsupported, nurbs, "u_periodic", Value::Logical(true));
    let error = reconstruct(&unsupported, &mut store).unwrap_err();
    assert_eq!(
        error.capability(),
        Some(XtCapability::PeriodicNurbsSurfaces)
    );
...
#[test]
fn noncanonical_offset_nurbs_bsurface_stale_nested_and_out_of_family_fail_atomically() {
    let fixture = || noncanonical_offset_nurbs_bsurface_intersection_file(true, true, 5);
    let valid = fixture();
    let mut store = Store::new();

    let mut stale_distance = fixture();
    let root = intersection_offset_source(&stale_distance);
    set_field(&mut stale_distance, root, "offset", Value::Double(0.2));
    assert!(matches!(
        reconstruct(&stale_distance, &mut store),
        Err(XtError::IntersectionCertificate { .. })
    ));
    assert_eq!(store_counts(&store), (0, 0, 0));

    let mut stale_basis = fixture();
    let root = intersection_offset_source(&stale_basis);
    let basis = ptr(&stale_basis, root, "surface");
    let nurbs = ptr(&stale_basis, basis, "nurbs");
    let poles = ptr(&stale_basis, nurbs, "bspline_vertices");
    let mut values = match stale_basis
        .field(&stale_basis.nodes[&poles], "vertices")
        .unwrap()
        .clone()
    {
        Value::Arr(values) => values,
    ...
    values[2] = Value::Double(values[2].as_f64().unwrap() + 0.05);
    set_field(&mut stale_basis, poles, "vertices", Value::Arr(values));
    assert!(matches!(
        reconstruct(&stale_basis, &mut store),
        Err(XtError::IntersectionCertificate { .. })
    ));
    assert_eq!(store_counts(&store), (0, 0, 0));

    let mut malformed_peer = fixture();
    let peer = intersection_bsurface_source(&malformed_peer);
    let nurbs = ptr(&malformed_peer, peer, "nurbs");
    set_field(&mut malformed_peer, nurbs, "n_u_vertices", Value::Int(3));
    assert!(matches!(
        reconstruct(&malformed_peer, &mut store),
        Err(XtError::BadField { index, .. }) if index == peer
    ));
    assert_eq!(store_counts(&store), (0, 0, 0));

    let mut unsafe_root = fixture();
    let root = intersection_offset_source(&unsafe_root);
    set_field(&mut unsafe_root, root, "check", Value::Char('I'));
    assert!(matches!(
        reconstruct(&unsafe_root, &mut store),
    ...
    let intersection = nested
        ...
        .find_map(|(&index, node)| (node.code == code::INTERSECTION).then_some(index))
        .unwrap();
    let root = intersection_offset_source(&nested);
    let outer = nested.nodes.keys().next_back().copied().unwrap() + 1;
    nested.nodes.insert(outer, nested.nodes[&root].clone());
    set_field(&mut nested, outer, "node_id", Value::Int(i64::from(outer)));
    set_field(&mut nested, outer, "surface", Value::Ptr(root));
    let mut sources = match nested
        .field(&nested.nodes[&intersection], "surface")
        .unwrap()
        .clone()
    {
    ...
    let root_index = sources
        .iter()
        .position(|value| value.as_ptr() == Some(root))
        .unwrap();
    sources[root_index] = Value::Ptr(outer);
    set_field(&mut nested, intersection, "surface", Value::Arr(sources));
    assert!(matches!(
        reconstruct(&nested, &mut store),
        Err(XtError::Unsupported {
            capability: XtCapability::IntersectionSurfaceFamily,
    ...
    let out_of_family = noncanonical_offset_nurbs_bsurface_intersection_file(false, false, 6);
    assert!(matches!(
        reconstruct(&out_of_family, &mut store),
        Err(XtError::Unsupported {
            capability: XtCapability::IntersectionChartConvention,
...
#[test]
fn noncanonical_offset_plane_bsurface_unsafe_nonplane_and_out_of_family_fail_atomically() {
    let valid = noncanonical_offset_bsurface_intersection_file(true, true, false, 5);
    let mut store = Store::new();

    let mut unsafe_offset = noncanonical_offset_bsurface_intersection_file(true, false, false, 2);
    let unsafe_root = intersection_offset_source(&unsafe_offset);
    set_field(&mut unsafe_offset, unsafe_root, "check", Value::Char('I'));
    assert!(matches!(
        reconstruct(&unsafe_offset, &mut store),
        Err(XtError::BadField { index, .. }) if index == unsafe_root
    ));
    assert_eq!(store_counts(&store), (0, 0, 0));

    let mut nonplane_chain = noncanonical_offset_bsurface_intersection_file(true, false, false, 2);
    let nonplane_root = intersection_offset_source(&nonplane_chain);
    let nonplane_bsurface = intersection_bsurface_source(&nonplane_chain);
    let inner = nonplane_chain.nodes.keys().next_back().copied().unwrap() + 1;
    nonplane_chain
        .nodes
        .insert(inner, nonplane_chain.nodes[&nonplane_root].clone());
    set_field(
        &mut nonplane_chain,
        inner,
        "node_id",
    ...
    assert!(matches!(
        reconstruct(&nonplane_chain, &mut store),
        Err(XtError::Unsupported {
            capability: XtCapability::IntersectionSurfaceFamily,
            what: "INTERSECTION offset source does not resolve to an exact plane field",
        })
    ...
    let out_of_family = noncanonical_offset_bsurface_intersection_file(true, false, false, 6);
    assert!(matches!(
        reconstruct(&out_of_family, &mut store),
        Err(XtError::Unsupported {
            capability: XtCapability::IntersectionChartConvention,
