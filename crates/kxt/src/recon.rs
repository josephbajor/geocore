...
impl IntersectionImportBudgetProfile {
    ...
    /// Corpus-backed defaults through the next canonical finite-open
    /// two-sample Offset(B-surface)/Offset(B-surface) line chart.
    ///
    /// Historical v1-v14 profiles retain their exact policy contracts.
    pub fn v15_defaults() -> BudgetPlan {
        BudgetPlan::new([
            LimitSpec::new(
                INTERSECTION_CHART_CERTIFICATE_WORK,
                ResourceKind::Work,
        ...
        .expect("built-in X_T two-sample dual-offset line profile is valid")
    }
...
pub fn reconstruction_budget_profile() -> BudgetPlan {
    ...
    let intersection = IntersectionImportBudgetProfile::v15_defaults();
    BudgetPlan::new(
        graph
            .limits()
            .iter()
...
impl Recon<'_, '_, '_, '_, '_, '_, '_> {
    ...
    fn intersection_curve(&mut self, curve_idx: u32, node: &Node) -> Result<CurveId> {
        ...
        let safe_offset_plane_bsurface = direct_planes.iter().flatten().next().is_none()
            && nurbs_effective_offset_planes.iter().flatten().count() == 1
            && offset_source_count == 1
            && nurbs_source_count == 1
            && !offset_nurbs_sources.contains(&true);
        let direct_bsurface_bsurface = offset_source_count == 0 && nurbs_source_count == 2;
