use super::result::{
    ContactKind, SurfaceIntersectionCurve, SurfaceSurfaceCurve, SurfaceSurfaceIntersections,
};
use kcore::error::{Error, Result};
use kcore::tolerance::Tolerances;
use kgeom::curve::Curve;
use kgeom::nurbs::NurbsSurface;
use kgeom::param::ParamRange;
use kgeom::surface::{Dir, Plane, Surface};
use kgeom::vec::Point3;

const MIN_GRID_STEPS: usize = 24;
const MAX_GRID_STEPS: usize = 96;
const MAX_BISECTION_STEPS: usize = 80;

#[derive(Debug, Clone, Copy)]
struct Sample {
    uv: [f64; 2],
    distance: f64,
}

#[derive(Debug, Clone, Copy)]
struct MarchPoint {
    uv: [f64; 2],
    plane_uv: [f64; 2],
    point: Point3,
}

#[derive(Debug, Clone, Copy)]
struct CellHit {
    edge: usize,
    point: MarchPoint,
}

#[derive(Debug, Clone, Copy)]
struct Segment {
    a: MarchPoint,
    b: MarchPoint,
}

/// Intersect a finite plane window with a clamped NURBS surface over a finite
/// parameter window.
///
/// This is the first SSI marching/subdivision bridge: it samples the signed
/// distance field over the NURBS parameter rectangle, traces zero-contour
/// segments with marching squares, and returns the joined branches as degree-1
/// NURBS polylines. Exact adaptive start-point discovery and Newton-polished
/// marching can refine this path while preserving the same result contract.
pub fn intersect_bounded_plane_nurbs_surface(
    plane: &Plane,
    plane_range: [ParamRange; 2],
    surface: &NurbsSurface,
    surface_range: [ParamRange; 2],
    tolerances: Tolerances,
) -> Result<SurfaceSurfaceIntersections> {
    validate_ranges(surface, plane_range, surface_range, tolerances)?;
    if control_net_lies_in_plane(surface, plane, tolerances) {
        return Err(Error::InvalidGeometry {
            reason: "coincident plane/nurbs-surface intersection is a surface overlap",
        });
    }

    let parameter_tol = surface_parameter_tolerance(surface_range, tolerances);
    if surface_range[0].width() <= parameter_tol || surface_range[1].width() <= parameter_tol {
        return Ok(SurfaceSurfaceIntersections::default());
    }

    let (u_steps, v_steps) = marching_steps(surface);
    let samples = sample_grid(surface, plane, surface_range, u_steps, v_steps)?;
    let mut segments = Vec::new();
    for i in 0..u_steps {
        for j in 0..v_steps {
            collect_cell_segments(
                surface,
                plane,
                plane_range,
                surface_range,
                [
                    sample_at(&samples, v_steps, i, j),
                    sample_at(&samples, v_steps, i + 1, j),
                    sample_at(&samples, v_steps, i + 1, j + 1),
                    sample_at(&samples, v_steps, i, j + 1),
                ],
                parameter_tol,
                tolerances,
                &mut segments,
            );
        }
    }

    let polylines = join_segments(segments, parameter_tol, tolerances);
    let mut curves = Vec::new();
    for polyline in polylines {
        if let Some(curve) =
            branch_from_polyline(surface, plane, surface_range, polyline, tolerances)?
        {
            curves.push(curve);
        }
    }
    SurfaceSurfaceIntersections::canonicalized(Vec::new(), curves)
}

#[allow(clippy::too_many_arguments)]
fn collect_cell_segments(
    surface: &NurbsSurface,
    plane: &Plane,
    plane_range: [ParamRange; 2],
    surface_range: [ParamRange; 2],
    corners: [Sample; 4],
    parameter_tol: f64,
    tolerances: Tolerances,
    segments: &mut Vec<Segment>,
) {
    if corners
        .iter()
        .all(|corner| corner.distance.abs() <= tolerances.linear())
    {
        return;
    }

    let edge_corners = [(0, 1), (1, 2), (2, 3), (3, 0)];
    let mut hits = Vec::new();
    for (edge, (a, b)) in edge_corners.into_iter().enumerate() {
        for point in edge_roots(
            surface,
            plane,
            plane_range,
            surface_range,
            corners[a],
            corners[b],
            parameter_tol,
            tolerances,
        ) {
            push_cell_hit(
                &mut hits,
                CellHit { edge, point },
                parameter_tol,
                tolerances,
            );
        }
    }

    hits.sort_by(|a, b| {
        a.edge
            .cmp(&b.edge)
            .then(a.point.uv[0].total_cmp(&b.point.uv[0]))
            .then(a.point.uv[1].total_cmp(&b.point.uv[1]))
    });

    match hits.len() {
        0 | 1 => {}
        2 => push_segment(
            segments,
            Segment {
                a: hits[0].point,
                b: hits[1].point,
            },
            parameter_tol,
            tolerances,
        ),
        _ => {
            for pair in hits.chunks_exact(2) {
                push_segment(
                    segments,
                    Segment {
                        a: pair[0].point,
                        b: pair[1].point,
                    },
                    parameter_tol,
                    tolerances,
                );
            }
        }
    }
}

#[allow(clippy::too_many_arguments)]
fn edge_roots(
    surface: &NurbsSurface,
    plane: &Plane,
    plane_range: [ParamRange; 2],
    surface_range: [ParamRange; 2],
    a: Sample,
    b: Sample,
    parameter_tol: f64,
    tolerances: Tolerances,
) -> Vec<MarchPoint> {
    let a_zero = a.distance.abs() <= tolerances.linear();
    let b_zero = b.distance.abs() <= tolerances.linear();
    if a_zero && b_zero {
        return [a.uv, b.uv]
            .into_iter()
            .filter_map(|uv| {
                march_point(
                    surface,
                    plane,
                    plane_range,
                    surface_range,
                    uv,
                    parameter_tol,
                    tolerances,
                )
            })
            .collect();
    }
    if a_zero {
        return march_point(
            surface,
            plane,
            plane_range,
            surface_range,
            a.uv,
            parameter_tol,
            tolerances,
        )
        .into_iter()
        .collect();
    }
    if b_zero {
        return march_point(
            surface,
            plane,
            plane_range,
            surface_range,
            b.uv,
            parameter_tol,
            tolerances,
        )
        .into_iter()
        .collect();
    }
    if same_sign(a.distance, b.distance) {
        return Vec::new();
    }

    let mut lo_uv = a.uv;
    let mut hi_uv = b.uv;
    let mut f_lo = a.distance;
    let mut root_uv = midpoint_uv(lo_uv, hi_uv);
    for _ in 0..MAX_BISECTION_STEPS {
        root_uv = midpoint_uv(lo_uv, hi_uv);
        let f_mid = signed_distance(surface.eval(root_uv), plane);
        if f_mid.abs() <= tolerances.linear() || uv_distance(lo_uv, hi_uv) <= parameter_tol {
            break;
        }
        if same_sign(f_lo, f_mid) {
            lo_uv = root_uv;
            f_lo = f_mid;
        } else {
            hi_uv = root_uv;
        }
    }

    march_point(
        surface,
        plane,
        plane_range,
        surface_range,
        root_uv,
        parameter_tol,
        tolerances,
    )
    .into_iter()
    .collect()
}

fn branch_from_polyline(
    surface: &NurbsSurface,
    plane: &Plane,
    surface_range: [ParamRange; 2],
    polyline: Vec<MarchPoint>,
    tolerances: Tolerances,
) -> Result<Option<SurfaceSurfaceCurve>> {
    let points = distinct_polyline_points(polyline, tolerances);
    if points.len() < 2 {
        return Ok(None);
    }
    let Some(nurbs) = polyline_nurbs(&points, tolerances)? else {
        return Ok(None);
    };
    let range = nurbs.param_range();
    let start = points[0];
    let end = points[points.len() - 1];
    Ok(Some(SurfaceSurfaceCurve {
        curve: SurfaceIntersectionCurve::Nurbs(nurbs),
        curve_range: range,
        uv_a_start: start.plane_uv,
        uv_a_end: end.plane_uv,
        uv_b_start: fit_uv(
            start.uv,
            surface_range,
            surface_parameter_tolerance(surface_range, tolerances),
        )
        .unwrap_or(start.uv),
        uv_b_end: fit_uv(
            end.uv,
            surface_range,
            surface_parameter_tolerance(surface_range, tolerances),
        )
        .unwrap_or(end.uv),
        kind: branch_kind(surface, plane, &points, tolerances),
    }))
}

fn polyline_nurbs(
    points: &[MarchPoint],
    tolerances: Tolerances,
) -> Result<Option<kgeom::nurbs::NurbsCurve>> {
    let mut controls = Vec::with_capacity(points.len());
    let mut cumulative = Vec::with_capacity(points.len());
    let mut length = 0.0;
    controls.push(points[0].point);
    cumulative.push(0.0);
    for point in &points[1..] {
        let step = controls[controls.len() - 1].dist(point.point);
        if step <= tolerances.linear() {
            continue;
        }
        length += step;
        controls.push(point.point);
        cumulative.push(length);
    }
    if controls.len() < 2 || length <= tolerances.linear() {
        return Ok(None);
    }

    let mut knots = vec![0.0, 0.0];
    for s in cumulative
        .iter()
        .skip(1)
        .take(cumulative.len().saturating_sub(2))
    {
        knots.push(*s / length);
    }
    knots.push(1.0);
    knots.push(1.0);
    Ok(Some(kgeom::nurbs::NurbsCurve::new(
        1, knots, controls, None,
    )?))
}

fn distinct_polyline_points(polyline: Vec<MarchPoint>, tolerances: Tolerances) -> Vec<MarchPoint> {
    let mut points = Vec::new();
    for point in polyline {
        if points.last().is_none_or(|last: &MarchPoint| {
            !points_match(last, &point, tolerances.angular(), tolerances)
        }) {
            points.push(point);
        }
    }
    points
}

fn join_segments(
    mut segments: Vec<Segment>,
    parameter_tol: f64,
    tolerances: Tolerances,
) -> Vec<Vec<MarchPoint>> {
    let mut polylines = Vec::new();
    while let Some(segment) = segments.pop() {
        let mut polyline = vec![segment.a, segment.b];
        let mut changed = true;
        while changed {
            changed = false;
            let mut index = 0;
            while index < segments.len() {
                if attach_segment(&mut polyline, segments[index], parameter_tol, tolerances) {
                    segments.swap_remove(index);
                    changed = true;
                } else {
                    index += 1;
                }
            }
        }
        polylines.push(polyline);
    }
    polylines
}

fn attach_segment(
    polyline: &mut Vec<MarchPoint>,
    segment: Segment,
    parameter_tol: f64,
    tolerances: Tolerances,
) -> bool {
    let front = polyline[0];
    let back = polyline[polyline.len() - 1];
    if points_match(&back, &segment.a, parameter_tol, tolerances) {
        polyline.push(segment.b);
        true
    } else if points_match(&back, &segment.b, parameter_tol, tolerances) {
        polyline.push(segment.a);
        true
    } else if points_match(&front, &segment.b, parameter_tol, tolerances) {
        polyline.insert(0, segment.a);
        true
    } else if points_match(&front, &segment.a, parameter_tol, tolerances) {
        polyline.insert(0, segment.b);
        true
    } else {
        false
    }
}

fn branch_kind(
    surface: &NurbsSurface,
    plane: &Plane,
    points: &[MarchPoint],
    tolerances: Tolerances,
) -> ContactKind {
    let mid = points[points.len() / 2].uv;
    let Some(surface_normal) = surface.normal(mid) else {
        return ContactKind::Singular;
    };
    if surface_normal.cross(plane.frame().z()).norm() <= tolerances.angular() {
        ContactKind::Tangent
    } else {
        ContactKind::Transverse
    }
}

fn march_point(
    surface: &NurbsSurface,
    plane: &Plane,
    plane_range: [ParamRange; 2],
    surface_range: [ParamRange; 2],
    uv: [f64; 2],
    parameter_tol: f64,
    tolerances: Tolerances,
) -> Option<MarchPoint> {
    let uv = fit_uv(uv, surface_range, parameter_tol)?;
    let point = surface.eval(uv);
    let local = plane.frame().to_local(point);
    if local.z.abs() > 4.0 * tolerances.linear() {
        return None;
    }
    let plane_uv = fit_uv([local.x, local.y], plane_range, tolerances.linear())?;
    Some(MarchPoint {
        uv,
        plane_uv,
        point,
    })
}

fn sample_grid(
    surface: &NurbsSurface,
    plane: &Plane,
    range: [ParamRange; 2],
    u_steps: usize,
    v_steps: usize,
) -> Result<Vec<Sample>> {
    let mut samples = Vec::with_capacity((u_steps + 1) * (v_steps + 1));
    for i in 0..=u_steps {
        for j in 0..=v_steps {
            let uv = [
                range[0].lerp(i as f64 / u_steps as f64),
                range[1].lerp(j as f64 / v_steps as f64),
            ];
            let distance = signed_distance(surface.eval(uv), plane);
            if !distance.is_finite() {
                return Err(Error::InvalidGeometry {
                    reason: "plane/nurbs-surface intersection sampled non-finite geometry",
                });
            }
            samples.push(Sample { uv, distance });
        }
    }
    Ok(samples)
}

fn sample_at(samples: &[Sample], v_steps: usize, i: usize, j: usize) -> Sample {
    samples[i * (v_steps + 1) + j]
}

fn signed_distance(point: Point3, plane: &Plane) -> f64 {
    plane.frame().to_local(point).z
}

fn control_net_lies_in_plane(
    surface: &NurbsSurface,
    plane: &Plane,
    tolerances: Tolerances,
) -> bool {
    surface
        .points()
        .iter()
        .all(|&point| signed_distance(point, plane).abs() <= tolerances.linear())
}

fn marching_steps(surface: &NurbsSurface) -> (usize, usize) {
    let (nu, nv) = surface.net_size();
    (
        ((nu + surface.degree_u()) * 8).clamp(MIN_GRID_STEPS, MAX_GRID_STEPS),
        ((nv + surface.degree_v()) * 8).clamp(MIN_GRID_STEPS, MAX_GRID_STEPS),
    )
}

fn midpoint_uv(a: [f64; 2], b: [f64; 2]) -> [f64; 2] {
    [(a[0] + b[0]) / 2.0, (a[1] + b[1]) / 2.0]
}

fn uv_distance(a: [f64; 2], b: [f64; 2]) -> f64 {
    let du = a[0] - b[0];
    let dv = a[1] - b[1];
    (du * du + dv * dv).sqrt()
}

fn same_sign(a: f64, b: f64) -> bool {
    (a < 0.0 && b < 0.0) || (a > 0.0 && b > 0.0)
}

fn fit_uv(candidate: [f64; 2], range: [ParamRange; 2], tolerance: f64) -> Option<[f64; 2]> {
    let mut uv = [0.0; 2];
    for axis in 0..2 {
        if candidate[axis] < range[axis].lo - tolerance
            || candidate[axis] > range[axis].hi + tolerance
        {
            return None;
        }
        uv[axis] = candidate[axis].clamp(range[axis].lo, range[axis].hi);
    }
    Some(uv)
}

fn points_match(
    a: &MarchPoint,
    b: &MarchPoint,
    parameter_tol: f64,
    tolerances: Tolerances,
) -> bool {
    uv_distance(a.uv, b.uv) <= parameter_tol || a.point.dist(b.point) <= tolerances.linear()
}

fn push_cell_hit(
    hits: &mut Vec<CellHit>,
    candidate: CellHit,
    parameter_tol: f64,
    tolerances: Tolerances,
) {
    if !hits
        .iter()
        .any(|hit| points_match(&hit.point, &candidate.point, parameter_tol, tolerances))
    {
        hits.push(candidate);
    }
}

fn push_segment(
    segments: &mut Vec<Segment>,
    candidate: Segment,
    parameter_tol: f64,
    tolerances: Tolerances,
) {
    if points_match(&candidate.a, &candidate.b, parameter_tol, tolerances) {
        return;
    }
    if !segments.iter().any(|segment| {
        (points_match(&segment.a, &candidate.a, parameter_tol, tolerances)
            && points_match(&segment.b, &candidate.b, parameter_tol, tolerances))
            || (points_match(&segment.a, &candidate.b, parameter_tol, tolerances)
                && points_match(&segment.b, &candidate.a, parameter_tol, tolerances))
    }) {
        segments.push(candidate);
    }
}

fn surface_parameter_tolerance(range: [ParamRange; 2], tolerances: Tolerances) -> f64 {
    (range[0].width().abs().max(range[1].width().abs()) * 1e-10)
        .max(tolerances.angular())
        .max(1e-12)
}

fn validate_ranges(
    surface: &NurbsSurface,
    plane_range: [ParamRange; 2],
    surface_range: [ParamRange; 2],
    tolerances: Tolerances,
) -> Result<()> {
    if plane_range
        .iter()
        .any(|range| !range.is_finite() || range.width() < 0.0)
    {
        return Err(Error::InvalidGeometry {
            reason: "plane/nurbs-surface intersection requires finite non-reversed plane ranges",
        });
    }
    if surface_range
        .iter()
        .any(|range| !range.is_finite() || range.width() < 0.0)
    {
        return Err(Error::InvalidGeometry {
            reason: "plane/nurbs-surface intersection requires finite non-reversed NURBS surface ranges",
        });
    }
    if !surface.knots(Dir::U).is_clamped() || !surface.knots(Dir::V).is_clamped() {
        return Err(Error::InvalidGeometry {
            reason: "plane/nurbs-surface intersection requires a clamped NURBS surface",
        });
    }
    let domain = surface.param_range();
    let parameter_tol = surface_parameter_tolerance(domain, tolerances);
    for axis in 0..2 {
        if surface_range[axis].lo < domain[axis].lo - parameter_tol
            || surface_range[axis].hi > domain[axis].hi + parameter_tol
        {
            return Err(Error::InvalidGeometry {
                reason: "plane/nurbs-surface intersection surface range must lie within the NURBS domain",
            });
        }
    }
    Ok(())
}
