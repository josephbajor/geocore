...
fn fit_periodic_parameter(candidate: f64, range: ParamRange, tolerance: f64) -> Option<f64> {
...
fn push_angle_root(roots: &mut Vec<f64>, t: f64) {
...
fn real_polynomial_roots(coeffs: &[f64]) -> Vec<f64> {
...
fn polynomial_derivative(poly: &[f64]) -> Vec<f64> {
    poly.iter()
        .enumerate()
        .skip(1)
        .map(|(i, c)| *c * i as f64)
