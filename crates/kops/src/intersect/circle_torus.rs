...
pub fn intersect_bounded_circle_torus(
    ...
    validate_ranges(circle_range, circle.radius(), torus_range, tolerances)?;

    let context = CircleTorusContext::new(circle, circle_range, torus, torus_range, tolerances);
    ...
    if coeffs.iter().all(|coeff| coeff.abs() <= tolerance) {
        return contained_circle_torus(&context);
    }
...
impl<'a> CircleTorusContext<'a> {
    fn new(
        ...
        Self {
            ...
            torus,
            torus_range,
            local_center: torus.frame().to_local(circle.frame().origin()),
            local_x: Vec3::new(
                circle_x.dot(torus.frame().x()),
                circle_x.dot(torus.frame().y()),
                circle_x.dot(torus.frame().z()),
            ),
            local_y: Vec3::new(
                circle_y.dot(torus.frame().x()),
                circle_y.dot(torus.frame().y()),
                circle_y.dot(torus.frame().z()),
            ),
            tolerances,
        }
    }
...
fn contained_circle_torus(context: &CircleTorusContext<'_>) -> Result<CurveSurfaceIntersections> {
...
fn implicit_coefficients(context: &CircleTorusContext<'_>) -> [f64; 5] {
    let radius = context.circle.radius();
    let c = context.local_center;
    let x = context.local_x;
    let y = context.local_y;
...
fn implicit_value(context: &CircleTorusContext<'_>, t_curve: f64) -> f64 {
...
fn longitude_matches_bound(context: &CircleTorusContext<'_>, local: Vec3, bound: f64) -> bool {
    let xy = (local.x * local.x + local.y * local.y).sqrt();
...
fn tube_angle_matches_bound(context: &CircleTorusContext<'_>, local: Vec3, bound: f64) -> bool {
    let xy = (local.x * local.x + local.y * local.y).sqrt();
...
fn implicit_tolerance(context: &CircleTorusContext<'_>) -> f64 {
    let scale =
