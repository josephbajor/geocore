...
pub fn intersect_bounded_ellipse_sphere(
    ...
    let context =
        EllipseSphereContext::new(ellipse, ellipse_range, sphere, sphere_range, tolerances);
    ...
    if coeffs.iter().all(|coeff| coeff.abs() <= tolerance) {
        return contained_ellipse_sphere(&context);
    }
...
impl<'a> EllipseSphereContext<'a> {
    fn new(
        ...
        Self {
            ...
            sphere,
            sphere_range,
            local_center: sphere.frame().to_local(ellipse.frame().origin()),
            local_x: Vec3::new(
                ellipse_x.dot(sphere.frame().x()),
                ellipse_x.dot(sphere.frame().y()),
                ellipse_x.dot(sphere.frame().z()),
            ),
            local_y: Vec3::new(
                ellipse_y.dot(sphere.frame().x()),
                ellipse_y.dot(sphere.frame().y()),
                ellipse_y.dot(sphere.frame().z()),
            ),
            tolerances,
        }
    }
...
fn contained_ellipse_sphere(
    context: &EllipseSphereContext<'_>,
) -> Result<CurveSurfaceIntersections> {
...
fn push_sphere_window_cuts(context: &EllipseSphereContext<'_>, cuts: &mut Vec<f64>) {
    ...
    for v_bound in [context.sphere_range[1].lo, context.sphere_range[1].hi] {
        let z_bound = context.sphere.radius() * math::sin(v_bound);
...
fn implicit_coefficients(context: &EllipseSphereContext<'_>) -> [f64; 5] {
    let a_vec = context.local_x * context.ellipse.major_radius();
    let b_vec = context.local_y * context.ellipse.minor_radius();
    let c0 = context.local_center.dot(context.local_center)
        - context.sphere.radius() * context.sphere.radius();
    let cos = 2.0 * context.local_center.dot(a_vec);
    let sin = 2.0 * context.local_center.dot(b_vec);
...
fn implicit_value(context: &EllipseSphereContext<'_>, t_curve: f64) -> f64 {
    let local = context.local_point(t_curve);
    local.dot(local) - context.sphere.radius() * context.sphere.radius()
}
...
fn implicit_tolerance(context: &EllipseSphereContext<'_>) -> f64 {
    let scale = (context.local_extent() + context.sphere.radius()).max(1.0);
