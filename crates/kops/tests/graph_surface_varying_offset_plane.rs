...
#[test]
fn malformed_peers_nested_offsets_and_offset_planes_are_unsupported() {
    let tolerances = Tolerances::with_linear(1.0e-3).unwrap();
    let mut graph = GeometryGraph::new();
    let basis = graph
        .insert_surface(rational_quarter_cylinder(1.0))
    ...
    let offset_plane = graph
        .insert_surface(OffsetSurfaceDescriptor::new(canonical, 0.01))
        .unwrap();
    for peer in [malformed, offset_plane] {
        assert!(matches!(
