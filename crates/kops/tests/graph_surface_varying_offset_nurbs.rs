...
fn direct_z_planar_nurbs(plane_z: f64) -> NurbsSurface {
    NurbsSurface::new(
        1,
        1,
        vec![0.0, 0.0, 1.0, 1.0],
        ...
        vec![
            ...
            Point3::new(0.0, 1.2, plane_z),
            Point3::new(1.2, 0.0, plane_z),
            Point3::new(1.2, 1.2, plane_z),
        ],
        None,
    )
    .unwrap()
...
fn varying_normal_original_control_miss_is_complete_and_allocates_nothing() {
    let tolerances = Tolerances::with_linear(1.0e-3).unwrap();
    for direct in [
        direct_x_planar_nurbs(1.2),
        direct_y_planar_nurbs(1.2),
        direct_z_planar_nurbs(1.2),
    ] {
        let mut graph = GeometryGraph::new();
...
fn singular_nested_and_incompatible_varying_normal_sources_fail_typed() {
    ...
    let direct_z = direct_z_planar_nurbs(0.5);
    let mut points = direct_z.points().to_vec();
    points[3].z += 0.01;
    let incompatible_z = NurbsSurface::new(
        direct_z.degree_u(),
        direct_z.degree_v(),
        direct_z.knots(kgeom::surface::Dir::U).as_slice().to_vec(),
        direct_z.knots(kgeom::surface::Dir::V).as_slice().to_vec(),
        points,
        direct_z.weights().map(<[f64]>::to_vec),
    )
    .unwrap();
    let incompatible_z = graph.insert_surface(incompatible_z).unwrap();
    for (a, b, a_range, b_range) in [
        (inner, incompatible_z, offset_window(), direct_window()),
        (incompatible_z, inner, direct_window(), offset_window()),
    ] {
        assert!(matches!(
            intersect_bounded_graph_surfaces(&graph, a, a_range, b, b_range, tolerances),
            Err(GraphSurfaceIntersectionError::Intersection(
...
#[test]
fn altered_or_stale_axis_oriented_peer_rolls_persistence_back_atomically() {
    let tolerances = Tolerances::with_linear(1.0e-3).unwrap();
    for orientation in 0..2 {
        for mutation in 0..2 {
            ...
            let root = graph
                .insert_surface(OffsetSurfaceDescriptor::new(basis, SIGNED_DISTANCE))
                .unwrap();
            let direct_surface = match orientation {
                0 => direct_y_planar_nurbs(0.88),
                1 => direct_z_planar_nurbs(0.5),
                _ => unreachable!(),
            };
            let direct = graph.insert_surface(direct_surface).unwrap();
            let local = intersect_bounded_graph_surfaces(
                &graph,
                root,
                offset_window(),
            ...
            match mutation {
                0 => {
                    let altered = match orientation {
                        0 => direct_y_planar_nurbs(0.89),
                        1 => direct_z_planar_nurbs(0.51),
                        _ => unreachable!(),
                    };
                    graph.replace_surface(direct, altered).unwrap();
                }
                1 => {
...
#[test]
fn varying_normal_z_plane_promotes_swaps_persists_and_pins_exact_limits() {
    let tolerances = Tolerances::with_linear(1.0e-3).unwrap();
    let basis_surface = rational_quarter_cylinder(1.0);
    let direct_surface = direct_z_planar_nurbs(0.5);
    let lower = intersect_bounded_offset_nurbs_nurbs_surfaces(
        &basis_surface,
        SIGNED_DISTANCE,
        offset_window(),
    ...
    let root = graph
        .insert_surface(OffsetSurfaceDescriptor::new(basis, SIGNED_DISTANCE))
        .unwrap();
    let direct = graph.insert_surface(direct_surface.clone()).unwrap();
    let session = SessionPolicy::v1();
    let context = OperationContext::new(&session, tolerances).unwrap();
    let outcome = intersect_bounded_graph_surfaces_with_context(
        &graph,
    ...
    let certificate = result.branch_graph.edges[0].certificate.as_nurbs().unwrap();
    assert_eq!(certificate.carrier().points().len(), 41);
    let offset_trace = certificate.traces()[0].as_offset_nurbs().unwrap();
    assert_eq!(offset_trace.basis(), &basis_surface);
    assert_eq!(offset_trace.signed_distance(), SIGNED_DISTANCE);
    assert_eq!(certificate.traces()[1].as_nurbs(), Some(&direct_surface));
    let cost = verified_offset_nurbs_nurbs_intersection_certificate_cost(
        certificate.carrier(),
        certificate.traces(),
    )
    ...
    let reverse_certificate = reverse.branch_graph.edges[0]
        ...
        .unwrap();
    assert_eq!(
        reverse_certificate.traces()[0].as_nurbs(),
        Some(&direct_surface)
    );
    assert_eq!(
        reverse_certificate.traces()[1].as_offset_nurbs(),
    ...
    for (stage, resource, mode, allowed, consumed) in [
        ...
        let GraphSurfaceIntersectionError::OperationPolicy(
            ...
            panic!("N-1 z-oriented varying-normal resource must stop at {stage:?}");
        };
        assert_eq!(crossing.stage, stage);
        assert_eq!(crossing.resource, resource);
        assert_eq!(crossing.allowed, allowed);
    ...
    let persistent = persist_verified_graph_surface_intersections(&mut graph, result).unwrap();
    assert_eq!(persistent.edges.len(), 1);
    for protected in [basis, root, direct] {
        assert!(matches!(
            graph.remove_surface(protected),
            Err(GeometryGraphError::HasDependents { .. })
        ));
