...
fn direct_planar_nurbs(plane_x: f64) -> NurbsSurface {
    NurbsSurface::new(
        1,
        1,
        vec![0.0, 0.0, 1.0, 1.0],
...
fn varying_normal_offset_promotes_in_both_orders_with_exact_limits_and_persists() {
    ...
    let direct = direct_planar_nurbs(0.66);
...
fn varying_normal_original_control_miss_is_complete_and_allocates_nothing() {
    let basis = rational_quarter_cylinder(1.0);
    let direct = direct_planar_nurbs(1.2);
    let tolerances = Tolerances::with_linear(1.0e-3).unwrap();
    let mut graph = GeometryGraph::new();
...
fn altered_or_nested_varying_normal_proof_sources_roll_back_atomically() {
    ...
    for mutation in 0..3 {
        ...
        let root = graph
            ...
            .unwrap();
        let direct = graph.insert_surface(direct_planar_nurbs(0.66)).unwrap();
        ...
        match mutation {
            ...
            1 => {
                graph
                    .replace_surface(direct, direct_planar_nurbs(0.67))
                    .unwrap();
            }
            2 => {
                let inner = graph
