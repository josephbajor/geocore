...
fn opposite_corner_empty_seven_cell_wide_fixture()
-> (Sphere, Sphere, [ParamRange; 2], [ParamRange; 2]) {
    (
        world_sphere(),
        y_tilted_sphere(Point3::new(0.0, 0.0, 0.0), 1.0, 0.4122861498111098),
        window(
            -2.05071789320265,
            2.7033606413045987,
            -1.1927387289259745,
...
fn nine_cell_non_path_wide_fixture() -> (Sphere, Sphere, [ParamRange; 2], [ParamRange; 2]) {
    (
        world_sphere(),
        y_tilted_sphere(Point3::new(0.0, 0.0, 0.0), 1.0, 0.941731645814849),
        window(
            -0.6905707622863242,
            2.7325627610063625,
            -1.0093591690898873,
...
fn assert_general_polar_by_wide_shared_seam_path(
    ...
    let used_longitude_seams = expected_occupied
        ...
        .filter(|(first, second)| first[0] == second[0] && first[1].abs_diff(second[1]) == 1)
        .map(|(first, second)| longitude_seams[first[1].max(second[1])])
        .collect::<Vec<_>>();
    assert!(!used_longitude_seams.is_empty());
    for unused_longitude_seam in longitude_seams[1..3].iter().copied().filter(|seam| {
        !used_longitude_seams
            .iter()
            .any(|used| used.to_bits() == seam.to_bits())
    }) {
        assert!(
            boundary
                .iter()
...
#[test]
fn general_polar_by_wide_two_vertical_cells_merge_exactly_and_swap() {
    let a = world_sphere();
    let b = y_tilted_sphere(Point3::new(0.0, 0.0, 0.0), 1.0, 0.4);
    let half_pi = core::f64::consts::FRAC_PI_2;
    let a_v_lo = 1.0 - half_pi;
    let a_window = window(-1.2, -0.8, a_v_lo, half_pi);
    let b_window = window(-4.4, -4.4 + core::f64::consts::PI, 0.0, 0.8);
    let latitude_seam = a_window[1].lo + 0.5 * a_window[1].width();
    let a_pieces = [
        window(-1.2, -0.8, a_v_lo, latitude_seam),
        window(-1.2, -0.8, latitude_seam, half_pi),
    ];
    let longitude_width = b_window[0].width() / 3.0;
    let longitude_seams = [
        b_window[0].lo,
    ...
    assert!(boundary.iter().all(|vertex| {
        longitude_seams[1..3]
            .iter()
            .all(|seam| vertex.uv_b[0].to_bits() != seam.to_bits())
            && a.eval(vertex.uv_a).dist(b.eval(vertex.uv_b)) <= hit.regions[0].max_residual
...
#[test]
fn general_polar_by_wide_two_adjacent_lower_cells_merge_exactly_and_swap() {
    let a = world_sphere();
    let b = y_tilted_sphere(Point3::new(0.0, 0.0, 0.0), 1.0, 0.4);
    let half_pi = core::f64::consts::FRAC_PI_2;
    let tau = 2.0 * core::f64::consts::PI;
    let a_window = window(-0.5, 0.5, -1.3, half_pi);
    let b_window = window(1.9 - core::f64::consts::PI + tau, 1.9 + tau, -1.3, -0.6);
    let latitude_seam = a_window[1].lo + 0.5 * a_window[1].width();
    let a_pieces = [
        window(-0.5, 0.5, -1.3, latitude_seam),
        window(-0.5, 0.5, latitude_seam, half_pi),
    ];
    let longitude_width = b_window[0].width() / 3.0;
    let longitude_seams = [
    ...
    assert!(boundary.iter().all(|vertex| {
        vertex.uv_a[1].to_bits() != latitude_seam.to_bits()
            && vertex.uv_b[0].to_bits() != longitude_seams[2].to_bits()
            && a.eval(vertex.uv_a).dist(b.eval(vertex.uv_b)) <= hit.regions[0].max_residual
    }));
    for (index, vertex) in boundary.iter().enumerate() {
        let next = boundary[(index + 1) % boundary.len()];
        assert!(
            vertex.uv_b[0].to_bits() != longitude_seams[1].to_bits()
                || next.uv_b[0].to_bits() != longitude_seams[1].to_bits(),
            "shared lower-row longitude seam edge survived the exact merge"
        );
    }
...
fn general_polar_by_wide_two_nonadjacent_lower_cells_fail_closed() {
    ...
    let half_pi = core::f64::consts::FRAC_PI_2;
    let a_window = window(-0.5, 0.5, -1.3, half_pi);
    let b_window = window(-0.2, 5.95, -1.3, -0.6);
    let latitude_seam = a_window[1].lo + 0.5 * a_window[1].width();
    let a_pieces = [
        window(-0.5, 0.5, -1.3, latitude_seam),
        window(-0.5, 0.5, latitude_seam, half_pi),
...
#[test]
fn general_polar_by_wide_left_four_cell_cycle_merges_exactly_and_swaps() {
    assert_general_polar_by_wide_shared_seam_path(
        0.8094306659089616,
        window(
            0.10072528893857235,
...
#[test]
fn general_polar_by_wide_right_four_cell_cycle_merges_exactly_and_swaps() {
    assert_general_polar_by_wide_shared_seam_path(
        0.3840978458329001,
        window(
            -0.7839294031122588,
...
#[test]
fn general_polar_by_wide_lower_stem_four_cell_t_merges_exactly_and_swaps() {
    assert_general_polar_by_wide_shared_seam_path(
        0.8390485119441324,
        window(
            0.7546277362780804,
...
#[test]
fn general_polar_by_wide_upper_stem_four_cell_t_merges_exactly_and_swaps() {
    assert_general_polar_by_wide_shared_seam_path(
        0.8675104737097041,
        window(
            -0.8336879368685368,
...
#[test]
fn general_polar_by_wide_disconnected_vertical_pairs_merge_exactly_and_swap() {
    let a = world_sphere();
    let b = y_tilted_sphere(Point3::new(0.0, 0.0, 0.0), 1.0, 0.4);
    let a_window = window(-0.8, 0.2, -1.3, core::f64::consts::FRAC_PI_2);
    let b_window = window(-0.5, 5.65, -1.5, 0.6);
    let latitude_seam = a_window[1].lo + 0.5 * a_window[1].width();
    let a_pieces = [
        window(-0.8, 0.2, a_window[1].lo, latitude_seam),
        window(-0.8, 0.2, latitude_seam, a_window[1].hi),
    ];
    let longitude_width = b_window[0].width() / 3.0;
    let longitude_seams = [
        b_window[0].lo,
    ...
    let mut occupied = Vec::new();
    let mut empty = Vec::new();
    for (polar_index, a_piece) in a_pieces.into_iter().enumerate() {
        for wide_index in 0..3 {
            let child = intersect_bounded_spheres(
                &a,
                a_piece,
                &b,
                window(
                    longitude_seams[wide_index],
                    longitude_seams[wide_index + 1],
                    b_window[1].lo,
                    b_window[1].hi,
                ),
                Tolerances::default(),
    ...
    assert_eq!(occupied, [[0, 0], [0, 2], [1, 0], [1, 2]]);
    ...
    for region in &hit.regions {
        ...
        for (index, vertex) in region.boundary.iter().enumerate() {
            ...
            assert!(
                vertex.uv_a[1].to_bits() != latitude_seam.to_bits()
                    || next.uv_a[1].to_bits() != latitude_seam.to_bits(),
                "the exact vertical-pair latitude seam survived"
            );
            assert!(
                longitude_seams[1..3]
                    .iter()
...
#[test]
fn general_polar_by_wide_six_cell_union_merges_exactly_and_swaps() {
    assert_general_polar_by_wide_shared_seam_path(
        0.14716980102990423,
        window(
            -0.6749164680039561,
...
#[test]
fn general_polar_by_wide_three_cell_cap_row_path_merges_exactly_and_swaps() {
    let a = world_sphere();
    let b = y_tilted_sphere(Point3::new(0.0, 0.0, 0.0), 1.0, 0.4);
    let half_pi = core::f64::consts::FRAC_PI_2;
    let a_window = window(-1.55, 1.55, -1.0, half_pi);
    let b_u_lo = half_pi + 2.0 * core::f64::consts::PI;
    let b_u_hi = b_u_lo + core::f64::consts::PI;
    let b_window = window(b_u_lo, b_u_hi, 1.13, 1.5);
    let latitude_seam = a_window[1].lo + 0.5 * a_window[1].width();
    let a_pieces = [
        window(-1.55, 1.55, -1.0, latitude_seam),
        window(-1.55, 1.55, latitude_seam, half_pi),
    ];
    let longitude_width = b_window[0].width() / 3.0;
    let longitude_seams = [
        b_window[0].lo,
    ...
    assert_eq!(occupied, [[1, 0], [1, 1], [1, 2]]);
    ...
    assert_eq!(map.second_range(), b_window);
    assert_eq!(
        map.map_first_to_second([-1.55, half_pi]).unwrap(),
        map.map_first_to_second([1.55, half_pi]).unwrap()
    );
    let boundary = &hit.regions[0].boundary;
    assert!(
    ...
    for seam in &longitude_seams[1..3] {
        for (index, vertex) in boundary.iter().enumerate() {
            ...
            assert!(
                vertex.uv_b[0].to_bits() != seam.to_bits()
                    || next.uv_b[0].to_bits() != seam.to_bits(),
                "shared longitude seam edge survived the exact cap-row merge"
            );
        }
    }
    assert!(boundary.iter().all(|vertex| {
...
#[test]
fn general_polar_by_wide_three_cell_non_cap_row_path_merges_exactly_and_swaps() {
    let a = world_sphere();
    let b = y_tilted_sphere(Point3::new(0.0, 0.0, 0.0), 1.0, 0.4);
    let half_pi = core::f64::consts::FRAC_PI_2;
    let turn = 2.0 * core::f64::consts::PI;
    let a_window = window(-1.55, 1.55, 0.3, half_pi);
    let b_u_lo = -half_pi + turn;
    let b_u_hi = half_pi + turn;
    let b_window = window(b_u_lo, b_u_hi, 0.75, 0.9);
    let latitude_seam = a_window[1].lo + 0.5 * a_window[1].width();
    let a_pieces = [
        window(-1.55, 1.55, 0.3, latitude_seam),
        window(-1.55, 1.55, latitude_seam, half_pi),
    ];
    let longitude_width = b_window[0].width() / 3.0;
    let longitude_seams = [
    ...
    assert_eq!(occupied, [[0, 0], [0, 1], [0, 2]]);
    ...
    for seam in &longitude_seams[1..3] {
        for (index, vertex) in boundary.iter().enumerate() {
            ...
            assert!(
                vertex.uv_b[0].to_bits() != seam.to_bits()
                    || next.uv_b[0].to_bits() != seam.to_bits(),
                "shared longitude seam edge survived the exact non-cap-row merge"
            );
        }
    }
    assert!(boundary.iter().all(|vertex| {
    ...
    let opposite_row_window = window(b_u_lo, b_u_hi, 0.75, 1.1);
    let mut occupied_rows = [false; 2];
    for (a_index, a_piece) in a_pieces.into_iter().enumerate() {
        for b_index in 0..3 {
            let child = intersect_bounded_spheres(
                &a,
                ...
                window(
                    longitude_seams[b_index],
                    longitude_seams[b_index + 1],
                    opposite_row_window[1].lo,
                    opposite_row_window[1].hi,
                ),
                Tolerances::default(),
            )
            .unwrap();
            if !child.is_proven_empty() {
                occupied_rows[a_index] = true;
            }
        }
    }
...
#[test]
fn general_both_wide_nine_positive_cells_certify_exhaustive_union_and_swap() {
    let (a, b, a_window, b_window) = nine_cell_non_path_wide_fixture();
    assert!(a_window[0].width() < core::f64::consts::TAU);
    assert!(b_window[0].width() < core::f64::consts::TAU);
    assert!(a_window[1].lo > -core::f64::consts::FRAC_PI_2);
    assert!(a_window[1].hi < core::f64::consts::FRAC_PI_2);
    assert!(b_window[1].lo > -core::f64::consts::FRAC_PI_2);
    assert!(b_window[1].hi < core::f64::consts::FRAC_PI_2);
    for a_index in 0..3 {
        for b_index in 0..3 {
            let child = intersect_bounded_spheres(
                &a,
    ...
    // Every decomposition seam is internal to the exhaustive 3x3 union. It
    // may meet the true outer boundary at isolated vertices, but no seam edge
    // may survive the exact cancellation proof.
    for (seam_on_first_operand, seam) in [
        (true, a_window[0].lo + a_window[0].width() / 3.0),
        (true, a_window[0].lo + 2.0 * a_window[0].width() / 3.0),
        (false, b_window[0].lo + b_window[0].width() / 3.0),
        ...
        assert!((0..hit.regions[0].boundary.len()).all(|start| {
            ...
            let parameter = |index: usize| {
                if seam_on_first_operand {
                    hit.regions[0].boundary[index].uv_a[0]
                } else {
                    hit.regions[0].boundary[index].uv_b[0]
                }
            };
...
#[test]
fn general_both_wide_opposite_corner_empty_seven_cells_certify_distinct_union_and_swap() {
    let (a, b, a_window, b_window) = opposite_corner_empty_seven_cell_wide_fixture();
    let empty_cells: [[usize; 2]; 2] = [[0, 2], [2, 0]];
    assert_eq!(empty_cells[0][0].abs_diff(empty_cells[1][0]), 2);
    assert_eq!(empty_cells[0][1].abs_diff(empty_cells[1][1]), 2);
    for a_index in 0..3 {
        for b_index in 0..3 {
            let child = intersect_bounded_spheres(
                &a,
            ...
            .unwrap();
            if empty_cells.contains(&[a_index, b_index]) {
                assert!(child.is_proven_empty());
            } else {
                assert_general_sphere_window_region(&child, &a, &b);
            }
