//! Bounded ellipse/NURBS curve intersections.
...
use kops::intersect::{ContactKind, ParamOrientation, intersect_bounded_ellipse_nurbs};
...
fn quarter_ellipse_nurbs() -> NurbsCurve {
    NurbsCurve::new(
        2,
...
fn reversed_quarter_ellipse_nurbs() -> NurbsCurve {
    NurbsCurve::new(
        2,
        vec![0.0, 0.0, 0.0, 1.0, 1.0, 1.0],
        vec![
...
#[test]
fn ellipse_nurbs_crossing_tangent_and_range_filtering() {
    let ellipse = world_ellipse();
    let curve = crossing_line_nurbs();
    let hit = intersect_bounded_ellipse_nurbs(
        &ellipse,
        ellipse.param_range(),
        &curve,
        curve.param_range(),
    ...
    assert!(hit.points[0].point.dist(Point3::new(2.0, 0.0, 0.0)) < 1e-8);
    ...
    assert!(hit.points[1].point.dist(Point3::new(-2.0, 0.0, 0.0)) < 1e-8);
    assert!((hit.points[1].t_a - core::f64::consts::PI).abs() < 1e-8);
    assert!((hit.points[1].t_b - 1.0 / 6.0).abs() < 1e-8);

    let ellipse_filtered = intersect_bounded_ellipse_nurbs(
        &ellipse,
    ...
    .unwrap();
    assert_eq!(ellipse_filtered.points.len(), 1);
    assert!(
        ellipse_filtered.points[0]
            .point
            .dist(Point3::new(2.0, 0.0, 0.0))
            < 1e-8
    );

    let curve_miss = intersect_bounded_ellipse_nurbs(
        &ellipse,
        ellipse.param_range(),
        &curve,
        ParamRange::new(0.0, 0.1),
        Tolerances::default(),
    )
    .unwrap();
...
#[test]
fn ellipse_nurbs_contained_overlap_clips_to_ellipse_range() {
    let ellipse = world_ellipse();
    let curve = quarter_ellipse_nurbs();
    let hit = intersect_bounded_ellipse_nurbs(
        &ellipse,
    ...
    let reversed = reversed_quarter_ellipse_nurbs();
    let hit = intersect_bounded_ellipse_nurbs(
        &ellipse,
        ParamRange::new(0.0, core::f64::consts::FRAC_PI_4),
        &reversed,
