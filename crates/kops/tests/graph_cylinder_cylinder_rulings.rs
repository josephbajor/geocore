...
fn non_cardinal_seam_mixed_folded_support_pair(frame: Frame) -> [Cylinder; 2] {
    let second_axis = frame.x() * 0.8 - frame.y() * 0.6;
    let second_radial = frame.x() * 0.6 + frame.y() * 0.8;
    let first = Cylinder::new(frame, 0.078125).unwrap();
    let second = Cylinder::new(
        Frame::new(
...
fn non_cardinal_seam_mixed_folded_support_windows() -> [[ParamRange; 2]; 2] {
    [
        cylinder_window(range(-0.2, 0.2)),
        cylinder_window(range(-0.2, 0.2)),
    ]
...
#[path = "graph_cylinder_cylinder_rulings/folded_support.rs"]
mod folded_support;
