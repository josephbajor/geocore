//! Root-free finite-window occupancy for exact skew Cylinder/Cylinder sheets.
...
use kops::intersect::{
    GraphSurfaceIntersectionError, IntersectionBranchEndpointEvent, IntersectionBranchTopology,
    IntersectionBranchVertexEvent, SKEW_CYLINDER_AXIAL_CLIP_EXACT_WORK,
    SKEW_CYLINDER_AXIAL_CLIP_WORK, SKEW_CYLINDER_CLIPPED_BRANCH_TOPOLOGY,
    SKEW_CYLINDER_CLIPPED_TOPOLOGY_INCOMPLETE, SKEW_CYLINDER_TWO_SHEET_BRANCH_CARRIER,
...
#[test]
fn rooted_axial_clip_remains_one_typed_gap_without_partial_publication() {
    let cylinders = perpendicular_pair();
    let windows = [
