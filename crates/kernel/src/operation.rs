...
mod tests {
    ...
    fn rigid_body_copy_facade_reissues_witnessed_quadratic_dual_offset() {
        ...
        for swap_order in [false, true] {
            ...
            assert_eq!(certificate.metadata(), transmitted_metadata());
            assert_eq!(certificate.carrier().points().len(), 3);
            assert_eq!(
                witnesses.positions(),
                source_witnesses
                    .positions()
    ...
    fn rigid_body_copy_facade_reissues_nested_witnessed_cubic_dual_offset() {
        ...
        for swap_order in [false, true] {
            ...
            assert_eq!(certificate.metadata(), transmitted_metadata());
            assert_eq!(certificate.carrier().points().len(), 4);
            assert_eq!(
                witnesses.positions(),
                source_witnesses
                    .positions()
            ...
            let mut altered_positions = witnesses.positions();
    ...
    #[test]
    fn rigid_body_copy_facade_reissues_canonical_five_sample_dual_offset() {
        let placement = Frame::new(
            Point3::new(2.0, -1.0, 3.0),
            Vec3::new(1.0, 2.0, 3.0).normalized().unwrap(),
            Vec3::new(2.0, -1.0, 0.0).normalized().unwrap(),
        ...
        for swap_order in [false, true] {
            ...
            let certificate = copied.certificate().clone();
            assert_eq!(certificate.metadata(), transmitted_metadata());
            assert_eq!(certificate.carrier().points().len(), 5);
