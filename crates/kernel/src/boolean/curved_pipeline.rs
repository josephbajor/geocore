...
//! topology adapter commits exactly the selected axial cylinder bands; any
//! other closed-boundary class remains an explicit typed refusal.
...
fn execute_stages(
    ...
    let proposals = prepare_band_proposals(&cylinder_source, &cuts, selected)?;
...
fn commit_proposals(
    edit: &mut PartEdit<'_>,
    proposals: Vec<CylindricalBandSolidInput>,
    scope: &mut OperationScope<'_, '_>,
) -> StageResult<CurvedBooleanPipelineOutcome> {
...
mod tests {
    ...
    #[test]
    fn zero_cut_contained_cylinder_is_one_selected_band() {
        let mut session = Kernel::new().create_session();
        let part = session.create_part();
        let (block, cylinder) = {
            let mut edit = session.edit_part(part.clone()).unwrap();
