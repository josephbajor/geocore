...
fn seam_support_fixture(frame: Frame) -> Fixture {
    let mut session = Kernel::new().create_session();
    let part = session.create_part();
    let (first, second) = {
        let mut edit = session.edit_part(part.clone()).unwrap();
...
fn double_folded_support_fixture(frame: Frame) -> Fixture {
    let mut session = Kernel::new().create_session();
    let part = session.create_part();
    let (first, second) = {
        let mut edit = session.edit_part(part.clone()).unwrap();
        let first = edit
            ...
            .body();
        let second_frame =
            Frame::new(frame.origin() - frame.x() * 1.25, frame.x(), frame.y()).unwrap();
        let second = edit
            .create_cylinder(CylinderRequest::new(second_frame, 0.03125, 2.5))
            .unwrap()
            .into_result()
            .unwrap()
            .body();
...
fn non_cardinal_mixed_folded_support_fixture(frame: Frame, radial: [f64; 2]) -> Fixture {
    let mut session = Kernel::new().create_session();
    let part = session.create_part();
    let (first, second) = {
        let mut edit = session.edit_part(part.clone()).unwrap();
        ...
        let first = edit
            ...
            .unwrap()
            .body();
        // Reflect both directions together so the stored shifted origin
        // preserves the exact contact identity in every quadrant.
        let second_axis =
            frame.x() * (0.8 * radial[0].signum()) - frame.y() * (0.6 * radial[1].signum());
        let second_radial = frame.x() * radial[0] + frame.y() * radial[1];
        let second_frame = Frame::new(
...
fn short_seam_root_folded_support_fixture(frame: Frame) -> Fixture {
    let mut session = Kernel::new().create_session();
    let part = session.create_part();
    let (first, second) = {
        let mut edit = session.edit_part(part.clone()).unwrap();
        ...
        let offset = second_radial * second_radius - frame.x() * first_radius + second_axis * 0.125
            - frame.z() * 0.125;
        let first = edit
            .create_cylinder(CylinderRequest::new(frame, first_radius, 1.0))
            .unwrap()
            .into_result()
            .unwrap()
            .body();
        let second_frame = Frame::new(frame.origin() - offset, second_axis, frame.z()).unwrap();
        let second = edit
            .create_cylinder(CylinderRequest::new(second_frame, second_radius, 1.0))
            .unwrap()
            .into_result()
            .unwrap()
            .body();
...
fn short_seam_root_across_folded_support_fixture(frame: Frame) -> Fixture {
    let mut session = Kernel::new().create_session();
    let part = session.create_part();
    let (first, second) = {
        let mut edit = session.edit_part(part.clone()).unwrap();
        ...
        let first = edit
            .create_cylinder(CylinderRequest::new(reversed_first, first_radius, 1.0))
            .unwrap()
            .into_result()
            .unwrap()
            .body();
...
fn long_seam_root_across_folded_support_fixture(frame: Frame) -> Fixture {
    let mut session = Kernel::new().create_session();
    let part = session.create_part();
    let (first, second) = {
        let mut edit = session.edit_part(part.clone()).unwrap();
        ...
        let offset = second_radial * second_radius - frame.x() * first_radius + second_axis * 0.125
            - frame.z() * 0.125;
        let first_frame = frame.with_origin(frame.origin() - frame.z() * 0.5);
        let first = edit
...
fn assert_support_contact(fixture: &Fixture, graph: &BodySectionGraph) {
    assert_support_contact_at(fixture, graph, fixture.frame.point_at(0.0, 1.0, 0.0));
}

fn assert_support_contact_at(fixture: &Fixture, graph: &BodySectionGraph, expected_point: Point3) {
    assert_eq!(
        graph.completion(),
    ...
    assert!(contact.point().dist(expected_point) <= 1.0e-12);
...
fn assert_double_folded_support_components(fixture: &Fixture, graph: &BodySectionGraph) {
    assert_eq!(
        graph.completion(),
        SectionCompletion::Complete,
        "{graph:#?}"
    ...
    for fragment in graph.curve_fragments() {
        ...
        assert!(branch.embedding_certificate().is_none());
        let SectionCurveFragmentSpan::FoldedSupport { endpoints } = fragment.span() else {
            panic!("double folded support retained a non-folded fragment")
        };
        for endpoint in endpoints.iter() {
            endpoint_incidence[endpoint.endpoint()] += 1;
            let local = fixture.frame.to_local(endpoint.point());
            assert!((local.x * local.x + local.y * local.y - 1.0).abs() <= 1.0e-12);
            assert!(local.z.abs() <= 0.03125 + 1.0e-12);
            assert!((local.y * local.y + local.z * local.z - 0.03125_f64.powi(2)).abs() <= 1.0e-12);
        }
    }
    assert!(
        endpoint_incidence
            .into_iter()
            .all(|incidence| incidence == 2)
    );
...
fn assert_non_cardinal_mixed_folded_support_components(
    fixture: &Fixture,
    graph: &BodySectionGraph,
    radial: [f64; 2],
    simple: [usize; 2],
    ...
    assert!(graph.isolated_contacts().is_empty());
    assert!(graph.through_contacts().is_empty());
    assert_eq!(graph.branches().len(), members);
    assert_eq!(graph.curve_fragments().len(), members);
    assert_eq!(graph.curve_endpoints().len(), members);
    assert_eq!(graph.curve_components().len(), 2);
    let mut component_sizes = graph
        .curve_components()
        .iter()
        ...
        .collect::<Vec<_>>();
    component_sizes.sort_unstable();
    assert_eq!(component_sizes, vec![2, members - 2]);

    let mut endpoint_incidence = vec![0_usize; members];
    for fragment in graph.curve_fragments() {
        let branch = &graph.branches()[fragment.branch()];
        assert_eq!(branch.topology(), SectionBranchTopology::Open);
        assert!(branch.embedding_certificate().is_none());
        let SectionCurveFragmentSpan::FoldedSupport { endpoints } = fragment.span() else {
            panic!("non-cardinal seam-mixed support retained a non-folded fragment")
        };
        for endpoint in endpoints.iter() {
            endpoint_incidence[endpoint.endpoint()] += 1;
            let local = fixture.frame.to_local(endpoint.point());
            assert!(
                (local.x * local.x + local.y * local.y - 0.078125_f64.powi(2)).abs() <= 1.0e-10
            );
            let second_radial = local.x * radial[0] + local.y * radial[1] - 0.0390625;
            let second_height = local.z - 0.078125;
            assert!(
                (second_radial * second_radial + second_height * second_height
                    - 0.0390625_f64.powi(2))
                .abs()
                    <= 1.0e-10
            );
        }
    }
    assert_eq!(endpoint_incidence, vec![2; members]);
    ...
    for endpoint in graph.curve_endpoints() {
        match endpoint.topology() {
            ...
            SectionCurveEndpointTopology::TouchingSupportRootJoin {
                ...
                assert_eq!(usize::from(*root_ordinal), repeated);
                assert!(root_interval.lo().is_finite());
                assert!(root_interval.hi().is_finite());
                assert!(root_interval.lo() <= root_interval.hi());
                touching_ports.push(*continuation);
            ...
            topology => panic!(
                "non-cardinal seam-mixed support acquired an unexpected endpoint: {topology:?}"
            ),
        }
    }
    simple_roots.sort_unstable();
...
fn assert_short_seam_root_folded_support_component(fixture: &Fixture, graph: &BodySectionGraph) {
    assert_eq!(
        graph.completion(),
        SectionCompletion::Complete,
        "{graph:#?}"
    ...
    for &fragment_index in component.fragments() {
        ...
        assert!(branch.embedding_certificate().is_none());
        let SectionCurveFragmentSpan::FoldedSupport { endpoints } = fragment.span() else {
            panic!("short seam-root support retained a non-folded fragment")
        };
        assert!(endpoints[0].carrier_parameter() < endpoints[1].carrier_parameter());
        for endpoint in endpoints.iter() {
            endpoint_incidence[endpoint.endpoint()] += 1;
            ...
            assert!(local.z >= -1.0e-12 && local.z <= 1.0 + 1.0e-12);
        }
    }
    assert_eq!(endpoint_incidence, [2, 2]);
    let mut root_ordinals = graph
        ...
        .map(|endpoint| match endpoint.topology() {
            ...
            topology => {
                panic!("short seam-root support acquired an unexpected endpoint: {topology:?}")
            }
        })
        .collect::<Vec<_>>();
    root_ordinals.sort_unstable();
...
fn assert_seam_root_folded_support_component(
    ...
    for &fragment_index in component.fragments() {
        ...
        for endpoint in endpoints.iter() {
            ...
            let local = fixture.frame.to_local(endpoint.point());
            assert!((local.x * local.x + local.y * local.y - 0.0625_f64.powi(2)).abs() <= 1.0e-12);
            assert!((-max_abs_height..=max_abs_height).contains(&local.z));
        }
    }
    assert_eq!(endpoint_incidence, vec![2, 2, 2, 2]);
...
#[test]
fn seam_support_contact_section_is_complete_replay_swap_and_frame_stable() {
    for frame in exact_frames() {
        let fixture = seam_support_fixture(frame);
        let forward = section(&fixture, false);
        let replay = section(&fixture, false);
        let swapped = section(&fixture, true);
        let swapped_replay = section(&fixture, true);
...
#[test]
fn seam_support_contact_boolean_refuses_distinctly_without_mutation() {
    for frame in exact_frames() {
        for swapped in [false, true] {
            let mut fixture = seam_support_fixture(frame);
            let bodies = if swapped {
                [fixture.second.clone(), fixture.first.clone()]
            } else {
                [fixture.first.clone(), fixture.second.clone()]
...
#[test]
fn non_cardinal_mixed_section_is_complete_replay_swap_and_frame_stable() {
    for (radial, simple, repeated, members) in [
        ([0.6, 0.8], [1, 2], 0, 6),
        ([-0.6, 0.8], [0, 2], 1, 4),
        ([-0.6, -0.8], [0, 2], 1, 4),
        ([0.6, -0.8], [0, 1], 2, 6),
        ...
        for frame in folded_exact_frames() {
            let fixture = non_cardinal_mixed_folded_support_fixture(frame, radial);
            let forward = section(&fixture, false);
            let replay = section(&fixture, false);
            let swapped = section(&fixture, true);
            let swapped_replay = section(&fixture, true);
            assert_eq!(forward, replay);
            assert_eq!(swapped, swapped_replay);
            assert_non_cardinal_mixed_folded_support_components(
                &fixture, &forward, radial, simple, repeated, members,
            );
            assert_non_cardinal_mixed_folded_support_components(
                &fixture, &swapped, radial, simple, repeated, members,
            );
        }
    }
}
...
#[test]
fn non_cardinal_mixed_subtract_refuses_without_mutation() {
    for (radial, simple, repeated, members) in [
        ([0.6, 0.8], [1, 2], 0, 6),
        ([-0.6, 0.8], [0, 2], 1, 4),
        ([-0.6, -0.8], [0, 2], 1, 4),
        ...
        for frame in folded_exact_frames() {
            for swapped in [false, true] {
                let mut fixture = non_cardinal_mixed_folded_support_fixture(frame, radial);
                let bodies = if swapped {
                    [fixture.second.clone(), fixture.first.clone()]
                } else {
                    [fixture.first.clone(), fixture.second.clone()]
...
#[test]
fn double_folded_support_section_is_complete_replay_swap_and_frame_stable() {
    for frame in exact_frames() {
        let fixture = double_folded_support_fixture(frame);
        let forward = section(&fixture, false);
        let replay = section(&fixture, false);
        let swapped = section(&fixture, true);
        let swapped_replay = section(&fixture, true);
        assert_eq!(forward, replay);
        assert_eq!(swapped, swapped_replay);
        assert_double_folded_support_components(&fixture, &forward);
        assert_double_folded_support_components(&fixture, &swapped);
    }
}

#[test]
fn double_folded_support_subtract_refuses_without_mutation() {
    for frame in exact_frames() {
        for swapped in [false, true] {
            let mut fixture = double_folded_support_fixture(frame);
            let bodies = if swapped {
                [fixture.second.clone(), fixture.first.clone()]
            } else {
                [fixture.first.clone(), fixture.second.clone()]
            ...
            assert!(part.body(fixture.first.clone()).is_ok());
            assert!(part.body(fixture.second.clone()).is_ok());
            assert_double_folded_support_components(&fixture, &section(&fixture, swapped));
        }
    }
}
...
#[test]
fn long_seam_root_across_folded_support_section_is_complete_replay_swap_and_frame_stable() {
    for (frame_index, frame) in folded_exact_frames().into_iter().enumerate() {
        let fixture = long_seam_root_across_folded_support_fixture(frame);
        let forward = section(&fixture, false);
        let replay = section(&fixture, false);
        let swapped = section(&fixture, true);
        let swapped_replay = section(&fixture, true);
...
#[test]
fn long_seam_root_across_folded_support_boolean_refuses_distinctly_without_mutation() {
    for frame in folded_exact_frames() {
        for swapped in [false, true] {
            let mut fixture = long_seam_root_across_folded_support_fixture(frame);
            let bodies = if swapped {
                [fixture.second.clone(), fixture.first.clone()]
            } else {
                [fixture.first.clone(), fixture.second.clone()]
...
#[test]
fn short_seam_root_folded_support_section_is_complete_replay_swap_and_frame_stable() {
    for (frame_index, frame) in folded_exact_frames().into_iter().enumerate() {
        let fixture = short_seam_root_folded_support_fixture(frame);
        let forward = section(&fixture, false);
        let replay = section(&fixture, false);
        let swapped = section(&fixture, true);
        let swapped_replay = section(&fixture, true);
        assert_eq!(forward, replay, "frame {frame_index} forward replay");
        assert_eq!(swapped, swapped_replay, "frame {frame_index} swap replay");
        assert_short_seam_root_folded_support_component(&fixture, &forward);
        assert_short_seam_root_folded_support_component(&fixture, &swapped);
    }
}

#[test]
fn short_seam_root_folded_support_boolean_refuses_distinctly_without_mutation() {
    for frame in folded_exact_frames() {
        for swapped in [false, true] {
            let mut fixture = short_seam_root_folded_support_fixture(frame);
            let bodies = if swapped {
                [fixture.second.clone(), fixture.first.clone()]
            } else {
                [fixture.first.clone(), fixture.second.clone()]
            ...
            assert!(part.body(fixture.first.clone()).is_ok());
            assert!(part.body(fixture.second.clone()).is_ok());
            assert_short_seam_root_folded_support_component(&fixture, &section(&fixture, swapped));
        }
    }
}

#[test]
fn short_seam_root_across_folded_support_section_is_complete_replay_swap_and_frame_stable() {
    for (frame_index, frame) in folded_exact_frames().into_iter().enumerate() {
        let fixture = short_seam_root_across_folded_support_fixture(frame);
        let forward = section(&fixture, false);
        let replay = section(&fixture, false);
        let swapped = section(&fixture, true);
        let swapped_replay = section(&fixture, true);
...
#[test]
fn short_seam_root_across_folded_support_boolean_refuses_distinctly_without_mutation() {
    for frame in folded_exact_frames() {
        for swapped in [false, true] {
            let mut fixture = short_seam_root_across_folded_support_fixture(frame);
            let bodies = if swapped {
                [fixture.second.clone(), fixture.first.clone()]
            } else {
                [fixture.first.clone(), fixture.second.clone()]
