...
#[test]
fn public_curved_boss_realization_budget_is_exact_and_denial_is_failure_atomic() {
    let baseline = run_boolean(
        &mut one_ring_cylindrical_boss_fixture(),
        BooleanOperation::Unite,
        OperationSettings::new(),
    );
    assert!(matches!(
    ...
    let mut denied_fixture = one_ring_cylindrical_boss_fixture();
    let before = boolean_topology_counts(&denied_fixture);
    let denied = run_boolean(
        &mut denied_fixture,
        BooleanOperation::Unite,
