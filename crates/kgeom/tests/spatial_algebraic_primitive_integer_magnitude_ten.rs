//! Coverage for magnitude-ten primitive-integer spatial-root lifts.
...
#[test]
fn magnitude_ten_carrier_certifies_a_new_partial_range_family() {
    let (first, second) = pair(None);
    assert_no_projected_carrier_through(&first, &second, 9);
    assert_genuinely_noncoplanar(&first, &second);
    assert!(controls_correspond(&first, &second, [10, -1, 0], false));
    ...
    // Corresponding control differences are `(1,10,21)*(1-3t)`.
    // The smallest projected carrier is `10x-y`; no coordinate or two-axis
    // form bounded by magnitude nine corresponds. The residual `-x-2y+z`
    // completes the exact spatial lift.
    let certificate =
        certify_curve_pair_unique_root(&first, full_range(), &second, partial_range())
            .unwrap()
            .expect("the primitive magnitude-ten carrier lifts the normalized one-third root");
...
#[test]
fn primitive_magnitude_ten_forms_support_swap_and_affine_reversal() {
    let (first, second) = pair(None);
...
#[test]
fn proportional_positive_rational_weights_preserve_magnitude_ten_forms() {
    ...
    let certificate =
        certify_curve_pair_unique_root(&first, partial_range(), &second, partial_range())
            .unwrap()
            .expect("globally proportional rational weights preserve magnitude-ten forms");
    assert_eq!(certificate.projection_plane(), CurvePairProjectionPlane::Xy);
    assert!(certificate.determinant_lower_bound() > 0.0);
}

#[test]
fn broken_magnitude_ten_forms_and_nonproportional_weights_fail_closed() {
    let (first, second) = pair(None);
...
fn magnitude_thirteen_carriers_overflow_and_nonfinite_sources_remain_rejected() {
    ...
    let overflow_second = curve(
        [
            Point3::new(f64::MAX, 10.0, 21.0),
            Point3::new(f64::MAX, -5.0, -9.5),
            Point3::new(f64::MAX, -20.0, -42.0),
        ],
        None,
    );
    assert!(
