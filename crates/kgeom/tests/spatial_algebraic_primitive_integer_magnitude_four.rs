//! Coverage for magnitude-four primitive-integer spatial-root lifts.
...
fn assert_no_magnitude_three_projected_carrier_corresponds(
    first: &NurbsCurve,
    second: &NurbsCurve,
) {
    for first_axis in 0..3 {
        ...
        for second_axis in first_axis + 1..3 {
            for first_coefficient in -3_i8..=3 {
                for second_coefficient in -3_i8..=3 {
                    ...
                    assert!(
                        ...
                        "same-orientation magnitude-three carrier {coefficients:?}"
                    );
                    assert!(
                        !controls_correspond(first, second, coefficients, true),
                        "reversed magnitude-three carrier {coefficients:?}"
                    );
                }
            }
        }
...
#[test]
fn magnitude_four_carrier_certifies_a_new_partial_range_family() {
    let (first, second) = pair(None);
    assert_no_magnitude_three_projected_carrier_corresponds(&first, &second);
    assert_genuinely_noncoplanar(&first, &second);
    assert!(controls_correspond(&first, &second, [4, -1, 0], false));
    ...
    // Corresponding control differences are `(1,4,9)*(1-3t)`.
    // The smallest projected carrier is `4x-y`; no coordinate or form
    // bounded by magnitude three corresponds. The residual `-x-2y+z`
    // completes the exact spatial lift.
    let certificate =
        certify_curve_pair_unique_root(&first, full_range(), &second, partial_range())
            .unwrap()
            .expect("the primitive magnitude-four carrier lifts the normalized one-third root");
...
#[test]
fn primitive_magnitude_four_forms_support_swap_and_affine_reversal() {
    let (first, second) = pair(None);
...
#[test]
fn proportional_positive_rational_weights_preserve_magnitude_four_forms() {
    ...
    let certificate =
        certify_curve_pair_unique_root(&first, partial_range(), &second, partial_range())
            .unwrap()
            .expect("globally proportional rational weights preserve magnitude-four forms");
    assert_eq!(certificate.projection_plane(), CurvePairProjectionPlane::Xy);
    assert!(certificate.determinant_lower_bound() > 0.0);
}

#[test]
fn broken_magnitude_four_forms_and_nonproportional_weights_fail_closed() {
    let (first, second) = pair(None);
