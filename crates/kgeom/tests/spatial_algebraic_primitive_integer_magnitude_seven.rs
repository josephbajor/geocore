//! Coverage for magnitude-seven primitive-integer spatial-root lifts.
...
fn assert_no_magnitude_six_projected_carrier_corresponds(first: &NurbsCurve, second: &NurbsCurve) {
    for first_axis in 0..3 {
        ...
        for second_axis in first_axis + 1..3 {
            for first_coefficient in -6_i8..=6 {
                for second_coefficient in -6_i8..=6 {
                    ...
                    assert!(
                        ...
                        "same-orientation magnitude-six carrier {coefficients:?}"
                    );
                    assert!(
                        !controls_correspond(first, second, coefficients, true),
                        "reversed magnitude-six carrier {coefficients:?}"
                    );
                }
            }
        }
...
#[test]
fn magnitude_seven_carrier_certifies_a_new_partial_range_family() {
    let (first, second) = pair(None);
    assert_no_magnitude_six_projected_carrier_corresponds(&first, &second);
    assert_genuinely_noncoplanar(&first, &second);
    assert!(controls_correspond(&first, &second, [7, -1, 0], false));
    ...
    // Corresponding control differences are `(1,7,15)*(1-3t)`.
    // The smallest projected carrier is `7x-y`; no coordinate or form
    // bounded by magnitude six corresponds. The residual `-x-2y+z`
    // completes the exact spatial lift.
    let certificate =
        certify_curve_pair_unique_root(&first, full_range(), &second, partial_range())
            .unwrap()
            .expect("the primitive magnitude-seven carrier lifts the normalized one-third root");
...
#[test]
fn primitive_magnitude_seven_forms_support_swap_and_affine_reversal() {
    let (first, second) = pair(None);
...
#[test]
fn proportional_positive_rational_weights_preserve_magnitude_seven_forms() {
    ...
    let certificate =
        certify_curve_pair_unique_root(&first, partial_range(), &second, partial_range())
            .unwrap()
            .expect("globally proportional rational weights preserve magnitude-seven forms");
    assert_eq!(certificate.projection_plane(), CurvePairProjectionPlane::Xy);
    assert!(certificate.determinant_lower_bound() > 0.0);
}

#[test]
fn broken_magnitude_seven_forms_and_nonproportional_weights_fail_closed() {
    let (first, second) = pair(None);
...
#[test]
fn magnitude_thirteen_carriers_and_nonfinite_sources_remain_rejected() {
    let first = curve(
        [
            Point3::new(0.0, 0.0, 0.0),
            Point3::new(4.0, 0.0, 1.0),
    ...
    let magnitude_thirteen = curve(
        [
            Point3::new(1.0, 13.0, 27.0),
            Point3::new(3.5, -6.5, -12.5),
            Point3::new(6.0, -26.0, -54.0),
        ],
        None,
    );
    assert!(controls_correspond(
        &first,
        &magnitude_thirteen,
        [13, -1, 0],
        false
    ...
    assert!(
        certify_curve_pair_unique_root(&first, full_range(), &magnitude_thirteen, partial_range())
            .unwrap()
            .is_none()
    );
